...
| Output | None |
| Returns | None |
| Constraints | `dims` currently supports only 128. `nlist` ∈ {1024, 2048, 4096, 8192, 16384, 262144, 524288}. `msubs` ∈ {2, 4, 8, 16, 32}. `nbits` currently supports only 8. `config.useKmeansPP`: when `true`, NPU K-Means is used for coarse clustering; when `false`, CPU clustering is used. For large `nlist`, use `resourceSize` ≥ 512 MB (`nlist`=262144) or ≥ 1 GB (`nlist`=524288), and training sample count ≥ `nlist` × 40. |
