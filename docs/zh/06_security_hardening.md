...
用户需要合理规划内存使用，确保使用量不要超过系统资源限制。同时，检索业务特征底库存储于昇腾AI处理器DDR内，特征维度和数量（入库或查询等操作）以及计算过程中，业务临时内存和系统临时内存的使用决定总内存占用大小，输入过大会导致设备侧内存申请失败错误。当前单个Index（底库）支持最大库容视具体昇腾AI处理器Device侧内存大小而定，业务侧需要根据实际需求规划Index个数，防止内存超限情形发生。
