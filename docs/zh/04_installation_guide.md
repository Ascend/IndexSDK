...
在进行源码编译安装时，除了前述依赖外，还需要根据是否需要使用 ILFlat 算法选择安装**开放态场景包**。当前默认是不使用 ILFlat 算法。

- 如需要使用 ILFlat 算法，请在编译前将 `feature_retrieval/src/ascendfaiss/CMakeLists.txt` 中 `BUILD_ASCENDDEVICE` 选项设为 `ON`；

```bash
# 将 BUILD_ASCENDDEVICE 默认值改为 ON
sed -i 's/^OPTION(BUILD_ASCENDDEVICE.*/OPTION(BUILD_ASCENDDEVICE "Build ascenddevice" ON)/' feature_retrieval/src/ascendfaiss/CMakeLists.txt
```

- 同时，请下载并安装 AscendMiniOS。
