...
- Index SDK软件包支持基于不同Faiss版本构建的业务动态库多版本共存。默认兼容Faiss 1.10.x业务链路；如果使用IVFRaBitQ/RaBitQ等依赖Faiss 1.14的特性，需要使用Faiss 1.14.1对应的业务动态库、头文件和libfaiss.so。
...
6. 获取安装命令。请在下方根据实际情况单击对应选项，获取Index SDK的安装命令。
    ...
    **表 1**  快速安装详细信息
    ...
    Index SDK run包中包含基于Faiss 1.10.x和Faiss 1.14.1分别构建的业务动态库。安装时可通过`--faiss-version`选择安装后默认激活的Faiss ABI版本；如未指定，默认激活Faiss 1.10.x版本，兼容老业务链路。

    ```bash
    # 激活Faiss 1.10.x版本业务动态库，适用于不使用IVFRaBitQ/RaBitQ特性的场景
    ...
    # 激活Faiss 1.14.1版本业务动态库，适用于使用IVFRaBitQ/RaBitQ特性的场景
    ...
    安装完成后，软件包会根据“--faiss-version”配置以下软链接：
    ...
    > [!NOTE]
    >
    > run包只负责切换Index SDK提供的业务动态库和头文件软链接，不会安装或替换用户环境中的libfaiss.so。用户编译、运行应用程序时，需要将对应版本Faiss的include和lib目录加入编译参数和"LD_LIBRARY_PATH"。使用IVFRaBitQ/RaBitQ特性时，应选择Faiss 1.14.1；使用非IVFRaBitQ/RaBitQ特性时，可继续选择Faiss 1.10.x。
...
|--faiss-version=*\<version>*|（可选）选择安装后默认激活的Faiss ABI版本。支持“1.10”、“1.10.0”、“faiss1.10”、“1.14”、“1.14.1”和“faiss1.14”等取值，默认值为“1.10”。选择“1.10”时，激活基于Faiss 1.10.x构建的业务动态库和头文件；选择“1.14”时，激活基于Faiss 1.14.1构建的业务动态库和头文件。|
...
|--faiss-version=*\<version>*|（可选）选择升级后默认激活的Faiss ABI版本。支持“1.10”、“1.10.0”、“faiss1.10”、“1.14”、“1.14.1”和“faiss1.14”等取值，默认值为“1.10”。如果升级后需要继续使用IVFRaBitQ/RaBitQ特性，请指定“--faiss-version=1.14”。|
