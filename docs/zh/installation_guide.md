...
|--install-path=*\<path>*|（可选）自定义特征检索软件包安装根目录。如未设置，默认为当前命令执行所在目录。配置的路径必须以/或~开头，路径取值仅支持大小写字母、数字、-_./字符。<br>若不指定，将安装到默认路径下：<li>若使用root用户安装，默认安装路径为：/usr/local/Ascend。</li><li>若使用非root用户安装，则默认安装路径为：\$\{HOME}/Ascend，${HOME}指用户目录。</li><br>若通过该参数指定了安装目录，该目录other用户不能有写权限，如果指定普通用户安装，安装目录属主必须为当前安装用户。|
