...
本需求将扩展 `AscendIndexCagra` 的生命周期方法，支持从训练、索引构建、入库到在线检索的完整流程，并在 Ascend950 卡上完成性能评估，确保距离计算误差 ≤ 1e-5（与 CPU 浮点基线对齐），在典型推荐场景（100w 向量、128维 FP32、Top200）下综合性能达到 1.2 倍对照 GPU 基线（基于 NVIDIA 5090/4090/A100 换算）。

## 1.2 动机

当前 `AscendIndexCagra` 仅支持原始浮点向量存储，存在以下痛点：

- 内存开销大：大规模向量若使用原始浮点存储，单卡内存难以支撑亿级数据；即使采用 Cagra 图结构，邻接表 + 原始向量仍可能超出 Ascend950 等设备内存。
...
- 性能对标：在 Ascend950 单卡、100w 向量（128维 FP32）、Top200 场景中，QPS / 延迟综合性能 ≥ 1.2 × 对照 GPU 基线（取 NVIDIA 5090/4090/A100 上最优 Cagra 实现的实测值）。
...
2. **构建**：调用 `BuildGraph` 时，若配置为 RabitQ 模式，则：
   ...
   - 释放原始向量内存。
3. **入库/序列化**：将图结构（CSR）、压缩码、码本序列化到磁盘；加载时直接映射到 Ascend950 设备内存。
...
1. 由性能实验室在同等数据集（100w 128d，SIFT1M）下，分别测试：
   ...
   - Ascend950 运行本方案（`AscendIndexCagra` + RabitQ 模式），调优参数（`graphDegree=32`，`beam_width=64`，开启随机正交矩阵）达到相同召回率，记录 QPS。
2. 预期目标：`QPS_Ascend950 ≥ 1.2 × QPS_gpu`。

优化手段（基于现有 `AscendIndexCagra` 内核修改）：

- **算子融合**：将 RabitQ 距离计算（随机正交变换 + 归一化 + 汉明距离计算）融合为单个昇腾核函数，减少核函数启动和数据搬运开销。
- **缓存优化**：利用 Ascend950 的 L2/SRAM，将热点邻居的压缩码预先加载到缓存中。
...
性能基准报告（包含 Ascend950 vs 4090 对比数据）。
