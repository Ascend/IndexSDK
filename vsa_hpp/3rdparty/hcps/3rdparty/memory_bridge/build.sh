...
function read_input_args()
{
    ...
    set -- $(getopt -o h::t:c:s:m: --long xsan:,isa:,os:,compiler:,instrument:,entry-point:,help -n "$0" -- "$@")
    while true
    do
        case "$1" in
            -t)
...
function clean_build_files()
{
    ...
    rm -rf "${HMM_ROOT_PATH}/output"
    mkdir -p "${HMM_ROOT_PATH}/output"
    touch "${HMM_ROOT_PATH}/output/.gitkeep"
    rm -rf "${HMM_ROOT_PATH}/install"
    mkdir -p "${HMM_ROOT_PATH}/install"
    touch "${HMM_ROOT_PATH}/install/.gitkeep"
    rm -rf "${HMM_ROOT_PATH:?}/build"
    mkdir -p "${HMM_ROOT_PATH}/build"
    touch "${HMM_ROOT_PATH}/build/.gitkeep"
    echo "Clean succeed!"
}
function main()
{
