...
class OckHmmHMObjectExtImpl : public OckHmmHMObjectExt
{
   public:
    ...
    OckHmmHMObjectExtImpl(OckHmmHMOObjectID objectId, OckHmmSubMemoryAllocDispatcher &memAllocDispatcher,
                          ...
                          std::shared_ptr<HmoCsnGenerator> csnGen, std::unique_ptr<OckHmmMemoryGuard> &&memGuard)
        : allocDispatcher(memAllocDispatcher),
          dataInfo(memGuard->Addr(), memGuard->ByteSize(), 0U, memGuard->Location(), objectId),
          service(taskService),
          csnGenerator(csnGen),
          memoryGuard(std::move(memGuard)),
          released(false)
    {
        OCK_HMM_LOG_DEBUG("OckHmmHMObjectExtImpl" << *this);
    }

    OckHmmHMOObjectID GetId(void) const override { return dataInfo.hmoObjId; }
    OckHmmDeviceId IntimateDeviceId(void) const override
    {
        return OckHmmHMOObjectIDGenerator::ParseDeviceId(dataInfo.hmoObjId);
    }
    uint64_t GetByteSize(void) const override { return dataInfo.byteSize; }
    OckHmmHeteroMemoryLocation Location(void) const override { return dataInfo.location; }
    ...
    std::shared_ptr<OckHmmHMOBuffer> GetBuffer(OckHmmHeteroMemoryLocation location, uint64_t offset, uint64_t length,
                                               uint32_t timeout) override
    {
        auto task = GetBufferAsync(location, offset, length);
        return task->WaitResult(timeout);
    }
    std::shared_ptr<OckHmmAsyncResult<OckHmmHMOBuffer>> GetBufferAsync(OckHmmHeteroMemoryLocation location,
                                                                       uint64_t offset, uint64_t length) override
    {
        ...
        return std::make_shared<OckDefaultAsyncResult<
            OckHmmHMOBuffer, acladapter::OckAsyncResultInnerBridge<OckHmmHMOBufferExt>, OckHmmHMOBufferOutter>>(bridge);
    }
    ...
    uintptr_t Addr(void) const override { return dataInfo.addr; }
    std::shared_ptr<acladapter::OckAsyncTaskExecuteService> Service(void) override { return service; }
    bool Released(void) const override { return released.load(); }
...
std::shared_ptr<OckHmmHMObjectExt> OckHmmHMObjectExt::Create(
    OckHmmHMOObjectID objectId, OckHmmSubMemoryAllocDispatcher &allocDispatcher,
    std::shared_ptr<acladapter::OckAsyncTaskExecuteService> service, std::shared_ptr<HmoCsnGenerator> csnGenerator,
    std::unique_ptr<OckHmmMemoryGuard> &&memoryGuard)
{
    return std::make_shared<OckHmmHMObjectExtImpl>(objectId, allocDispatcher, service, csnGenerator,
...
OckHmmHMObjectOutter::OckHmmHMObjectOutter(std::shared_ptr<OckHmmHMObjectExt> hmoExt,
                                           OckHmmSubMemoryAllocDispatcher &memAllocDispatcher)
...
std::shared_ptr<OckHmmHMOBuffer> OckHmmHMObjectOutter::GetBuffer(OckHmmHeteroMemoryLocation location, uint64_t offset,
                                                                 uint64_t length, uint32_t timeout)
{
    if (hmo.get() == nullptr || hmo->Released())
    {
    ...
    if (OckHmmHeteroMemoryMgrParamCheck::CheckGetBuffer(location, offset, length, *hmo, allocDispatcher) != HMM_SUCCESS)
    {
        return std::shared_ptr<OckHmmHMOBuffer>();
    }
...
OckHmmHMObjectExt *OckHmmHMObjectOutter::GetExtHmo(void) { return hmo.get(); }
...
std::shared_ptr<OckHmmHMObject> OckHmmHMObjectOutter::Create(std::shared_ptr<OckHmmHMObjectExt> hmo,
                                                             OckHmmSubMemoryAllocDispatcher &allocDispatcher)
{
