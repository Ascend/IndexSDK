...
class TestOckDistMaskGenOpRun : public testing::Test {
public:
    ...
    {
        times.resize(DEFAULT_CODE_BLOCK_SIZE, 0);
        for (uint32_t i = 0; i < times.size(); ++i) {
            times[i] = i % 4U;
        }
        tokenIdQs.resize(DEFAULT_CODE_BLOCK_SIZE, 0);
        tokenIdRs.resize(DEFAULT_CODE_BLOCK_SIZE * OPS_DATA_TYPE_TIMES, 64U);
        for (int32_t i = 0; i < DEFAULT_CODE_BLOCK_SIZE; ++i) {
    ...
    void PrepareData()
    {
        maskHmoGroups = std::make_shared<OckDistMaskGenOpHmoGroups>();
        ...
        maskHmoGroups->mask = AllocHmo(((DEFAULT_CODE_BLOCK_SIZE * 2U) / OPS_DATA_TYPE_ALIGN) * batch);
        maskHmoGroups->tokenNum = TOKEN_NUM;
        maskHmoGroups->featureAttrBlockSize = DEFAULT_CODE_BLOCK_SIZE;
        maskHmoGroups->maskLen = (DEFAULT_CODE_BLOCK_SIZE * 2U) / OPS_DATA_TYPE_ALIGN;
    }

    bool CheckResults(std::shared_ptr<OckDistMaskGenOpHmoGroups> hmoGroups)
    {
    ...
    std::shared_ptr<OckDistMaskGenOpHmoGroups> maskHmoGroups;
...
private:
    ...
    template <typename T>
    void WriteHmo(const std::shared_ptr<hmm::OckHmmSubHMObject> &hmo, const std::vector<T> &vec)
    {
        aclrtMemcpy(reinterpret_cast<void *>(hmo->Addr()),
            hmo->GetByteSize(),
            vec.data(),
            vec.size() * sizeof(T),
    ...
    template <typename T>
    void ReadHmo(const std::shared_ptr<hmm::OckHmmSubHMObject> &hmo, std::vector<T> &vec)
    {
        aclrtMemcpy(vec.data(),
            vec.size() * sizeof(T),
            reinterpret_cast<void *>(hmo->Addr()),
            vec.size() * sizeof(T),
...
TEST_F(TestOckDistMaskGenOpRun, run)
{
    PrepareHostData();
    PrepareData();
    OckDistMaskGenOpRun::AddMaskOpsMultiBatches(maskHmoGroups, streamBase);
