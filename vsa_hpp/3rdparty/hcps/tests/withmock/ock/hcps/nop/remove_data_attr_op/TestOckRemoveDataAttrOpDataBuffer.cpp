...
namespace nop {
namespace test {
class TestOckRemoveDataAttrOpDataBuffer : public acladapter::WithEnvAclMock<testing::Test> {
public:
    ...
    {
        hmoBlock = std::make_shared<OckRemoveDataAttrOpHmoBlock>();
        auto hmoRet =
            devMgr->Alloc(opSpec.removeSize * sizeof(uint64_t), hmm::OckHmmMemoryAllocatePolicy::DEVICE_DDR_ONLY);
        OCK_CHECK_RETURN_ERRORCODE(hmoRet.first);
        hmoBlock->srcHmo = hmoRet.second;
        hmoRet = devMgr->Alloc(opSpec.removeSize * sizeof(uint64_t), hmm::OckHmmMemoryAllocatePolicy::DEVICE_DDR_ONLY);
        OCK_CHECK_RETURN_ERRORCODE(hmoRet.first);
    ...
    std::shared_ptr<OckRemoveDataAttrOpHmoBlock> hmoBlock;
...
TEST_F(TestOckRemoveDataAttrOpDataBuffer, alloc_buffers_from_hmo_block)
{
    BuildHmoBlock();
    auto buffer = ock::hcps::nop::OckRemoveDataAttrOpDataBuffer::Create(opSpec, bufferSpec);
