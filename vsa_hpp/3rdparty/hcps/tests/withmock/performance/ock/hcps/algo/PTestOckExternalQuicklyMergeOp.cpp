...
const uint64_t ITEMS_PER_LINE = 1024ULL;
const uint64_t PRECISIONT = (1U << BIT_NUM) - 1U;
const uint64_t DATA_COUNT = 16777216ULL; // 8G
const uint64_t LINE_NUM = DATA_COUNT / ITEMS_PER_LINE;
} // namespace
class PTestOckExternalQuicklyMergeOp : public handler::WithEnvOckHeteroHandler<testing::Test> {
public:
    ...
    void GenRandData(std::vector<FromDataT> &randData, uint64_t randDataNum)
    {
    ...
    void InitRandData(OckHeteroStreamBase &stream)
    {
        ...
        GenRandData(randData, randDataNum);
    ...
    {
        auto ops = MakeOckSplitGroupAtmoicOpsNoReturn<uint64_t, acladapter::OckTaskResourceType::HOST_CPU>(0ULL,
            LINE_NUM, 64ULL, [this](uint64_t i) {
                for (uint64_t j = 0; j < ITEMS_PER_LINE; ++j) {
                    needSortedExtFeatureData[i][j] = &extFeatureDatas[i];
                }
                std::sort(needSortedExtFeatureData[i].begin(), needSortedExtFeatureData[i].end(),
                    utils::PtrCompareAdapter());
            });
        stream.AddOps(*ops);
        stream.WaitExecComplete();
    }
    ...
    std::vector<std::vector<BitDataT *>> needSortedExtFeatureData;
    std::vector<BitDataT *> sortedExtFeatureData;
};
TEST_F(PTestOckExternalQuicklyMergeOp, external_quickly_merge)
{
    ...
    needSortedExtFeatureData.resize(LINE_NUM, std::vector<BitDataT *>(ITEMS_PER_LINE));
    std::vector<BitDataT *> sortedTmpData(sortedExtFeatureData);

    HashFeatureGen(*stream);
    BuildExtFeaturePtrContainer(*stream);

    auto timeGuardSort = fast::hdt::TestTimeGuard();
    ExternalQuicklyMerge(*stream, needSortedExtFeatureData, sortedExtFeatureData, sortedTmpData,
        utils::PtrCompareAdapter(), splitThreshold);
    EXPECT_TRUE(FAST_PTEST().Test("OCK.HCPS.ALGO.ExternMergeSort", "UsedTime", timeGuardSort.ElapsedMicroSeconds()));
}
} // namespace hop
} // namespace hcps
} // namespace ock