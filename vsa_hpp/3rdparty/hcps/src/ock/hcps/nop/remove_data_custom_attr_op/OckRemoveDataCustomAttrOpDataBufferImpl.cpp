...
namespace hcps {
namespace nop {
class OckRemoveDataCustomAttrOpDataBufferImpl : public OckOpDataBufferGen<OckRemoveDataCustomAttrOpDataBuffer> {
public:
    virtual ~OckRemoveDataCustomAttrOpDataBufferImpl() noexcept = default;
    OckRemoveDataCustomAttrOpDataBufferImpl(const OckRemoveDataCustomAttrOpMeta &opSpec,
        const OckRemoveDataCustomAttrBufferMeta &bufferSpec)
    {
        numInputs = 2U;
        numOutputs = 1U;
        AddParamInfo<uint8_t>({ opSpec.removeSize });
        AddParamInfo<int64_t>({ REMOVEDATA_CUSTOM_ATTR_IDX_COUNT });
        AddParamInfo<uint8_t>({ opSpec.removeSize });
        inputParams.resize(numInputs);
        outputParams.resize(numOutputs);
    }
    OckHcpsErrorCode AllocBuffersFromHmoBlock(std::shared_ptr<OckRemoveDataCustomAttrOpHmoBlock> hmoBlock,
        std::shared_ptr<hmm::OckHmmHeteroMemoryMgrBase> devMgr) override
    {
        ...
        if (ret.first != hmm::HMM_SUCCESS) {
            OCK_HCPS_LOG_ERROR("RemoveDataCustomAttrOp attr hmo alloc failed, the errorCode is " << ret.first);
            return ret.first;
        }
        inputParams[1] = std::make_shared<OckDataBuffer>(ret.second, paramsShapes[1U]);
        std::vector<int64_t> attr = { Type::UINT8, static_cast<int64_t>(hmoBlock->customAttrLen),
            static_cast<int64_t>(hmoBlock->customAttrBlockSize) };
...
std::shared_ptr<OckRemoveDataCustomAttrOpDataBuffer> OckRemoveDataCustomAttrOpDataBuffer::Create(
    const OckRemoveDataCustomAttrOpMeta &opSpec, const OckRemoveDataCustomAttrBufferMeta &bufferSpec)
{
    return std::make_shared<OckRemoveDataCustomAttrOpDataBufferImpl>(opSpec, bufferSpec);
}
} // namespace nop
} // namespace hcps
} // namespace ock