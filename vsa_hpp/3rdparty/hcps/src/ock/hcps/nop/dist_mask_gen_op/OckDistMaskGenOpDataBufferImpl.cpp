...
namespace hcps {
namespace nop {
class OckDistMaskGenOpDataBufferImpl : public OckOpDataBufferGen<OckDistMaskGenOpDataBuffer> {
public:
    virtual ~OckDistMaskGenOpDataBufferImpl() noexcept = default;
    explicit OckDistMaskGenOpDataBufferImpl(const OckDistMaskGenOpMeta &opSpec)
    {
        numInputs = 5U;
        numOutputs = 1U;
        AddParamInfo<int32_t>({OPS_DATA_TYPE_ALIGN});
        AddParamInfo<uint8_t>({utils::SafeDivUp(opSpec.tokenNum, OPS_DATA_TYPE_ALIGN) * OPS_DATA_TYPE_TIMES});
        AddParamInfo<int32_t>({opSpec.featureAttrBlockSize * opSpec.blockCount});
        AddParamInfo<int32_t>({opSpec.featureAttrBlockSize * opSpec.blockCount});
        AddParamInfo<uint8_t>({opSpec.featureAttrBlockSize * opSpec.blockCount * OPS_DATA_TYPE_TIMES});
        AddParamInfo<uint8_t>({utils::SafeDivUp(opSpec.featureAttrBlockSize * opSpec.blockCount, OPS_DATA_TYPE_ALIGN)});
    ...
    OckHcpsErrorCode AllocBuffersFromHmoGroup(std::shared_ptr<OckDistMaskGenOpHmoGroup> hmoGroup,
        uint64_t offsetInSingleMask, uint64_t groupMaskLen) override
    {
        ...
        outputParams[0U] = std::make_shared<OckDataBuffer>(maskBuffer, paramsShapes[5U]);

        return hmm::HMM_SUCCESS;
    }
};

std::shared_ptr<OckDistMaskGenOpDataBuffer> OckDistMaskGenOpDataBuffer::Create(
    const OckDistMaskGenOpMeta &opSpec)
{
    return std::make_shared<OckDistMaskGenOpDataBufferImpl>(opSpec);
}
}  // namespace nop
}  // namespace hcps
}  // namespace ock
