...
TOP_DIR=../../../../ # this is the directory where ivfsp_impl and ivfsp_utils are at

ASAN_LOG_DIR=$TOP_DIR/ivfsp_impl/ascendfaiss/ut/output/

function modify_source_code_for_ut() {
    sed -i '/if (!ASCEND_310_SOC) {/,/^    }$/d' $TOP_DIR/ivfsp_utils/src/ascenddaemon/utils/MemorySpace.cpp

    # remove OrthCodebook(); line since it tries to do SVD, which is slow on UT machine
    sed -i '/OrthCodeBook();/s/^/\/\//' $TOP_DIR/ivfsp_impl/ascendfaiss/ascenddaemon/IVFSPCodeBookTrainer.cpp

    # remove the omp iteration within NzFormatReshape which causes performance degrade
    sed -i '577,587s/^/\/\//' $TOP_DIR/ivfsp_impl/ascendfaiss/ascenddaemon/IVFSPCodeBookTrainer.cpp

    # replace content within SingleFloatSVD impl to return 0; directly
    sed -i '435,443s/^/\/\//; 443a return 0;' $TOP_DIR/ivfsp_impl/ascendfaiss/ascenddaemon/IVFSPCodeBookTrainer.cpp
}

# this function prepares retrieval_common repo to be compiled into ivfsp_impl
function prepare_retrieval_common() {
    pushd ./

    cp -r $TOP_DIR/ivfsp_utils $TOP_DIR/ivfsp_impl/Retrieval_Common
    cd $TOP_DIR/ivfsp_impl/Retrieval_Common
    cp -r ./src/* . # Copy content within src to the same level as src itself
    if [ -L ascendsearch ]; then
        rm ascendsearch
    fi
    ln -s . ascendsearch

    # Compilation of Retreival_2022 needs symbols defined in op_vstar's kernel_shared_def.h
    cp -r ./src/ops_vstar/CpuOps ../ascendfaiss/ops
...
function gen_report() {
    ...
    lcov -r ./output/coverage/summary/total.info \
        ...
        '*opensource*' \
        '*/src/faiss/ascend/*.h' \
        '*/src/faiss/ascend/custom/impl/*.h' \
        '*/ascend/custom/impl/*.h' \
        '*/ascenddaemon/*.h' \
        '*/ascendsearch/*.h' \
        '*/common/*.h' \
