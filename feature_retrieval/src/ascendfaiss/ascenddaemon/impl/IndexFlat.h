...
struct topkOpParams
{
    inline topkOpParams(AscendTensor<float16_t, DIMS_3, size_t> &dists,
                        AscendTensor<float16_t, DIMS_3, size_t> &maxdists, AscendTensor<uint32_t, DIMS_3> &sizes,
...
class IndexFlat : public Index
{
   public:
    ...
    inline int getSize() const { return ntotal; }
    ...
    inline int getDim() const { return dims; }

    inline int getBlockSize() const { return blockSize; }

    inline const std::vector<std::unique_ptr<DeviceVector<float16_t>>> &getBaseShaped() const { return baseShaped; }

    inline const std::vector<std::unique_ptr<DeviceVector<float16_t>>> &getNormBase() const { return normBase; }
   ...
   protected:
    ...
    virtual APP_ERROR searchPaged(size_t pageId, size_t pageNum, AscendTensor<float16_t, DIMS_2> &queries,
                                  ...
                                  AscendTensor<int64_t, DIMS_2> &maxIndices) = 0;
    ...
    void execTransdataShapedOp(std::string &opName, aclrtStream stream, AscendTensor<float16_t, DIMS_2> &src1,
