...
class IndexIVFFlat : public IndexIVF
{
   public:
    ...
    APP_ERROR assign(AscendTensor<float, DIMS_2> &queries, AscendTensor<int64_t, DIMS_2> &l1TopNprobeIndicesHost);
   ...
   protected:
    ...
    APP_ERROR searchImplL2(AscendTensor<float, DIMS_2> &queries, AscendTensor<int64_t, DIMS_2> &l1TopNprobeIndicesHost,
                           int k, float *distances, idx_t *labels);
    APP_ERROR searchWithBatch(int n, const float *x, int k, float *distances, idx_t *labels);
    void runL2DistOp(AscendTensor<float, DIMS_2, size_t> &subQuery, AscendTensor<float, DIMS_2, size_t> &codeVec,
                     AscendTensor<uint64_t, DIMS_1, size_t> &subOffset,
                     AscendTensor<uint32_t, DIMS_1, size_t> &subBaseSize, AscendTensor<float, DIMS_2, size_t> &subDis,
    ...
    void runL2TopkOp(AscendTensor<float, DIMS_3, size_t> &distResult, AscendTensor<float, DIMS_3, size_t> &vmdistResult,
                     AscendTensor<int64_t, DIMS_3, size_t> &ids, AscendTensor<uint32_t, DIMS_3, size_t> &sizes,
    ...
    APP_ERROR fillDisOpInputData(int k, size_t batch, size_t tileNum, size_t segNum, size_t coreNum,
                                 ...
                                 AscendTensor<int64_t, DIMS_3, size_t> &ids, AscendTensor<int64_t, DIMS_1> &attrs,
                                 AscendTensor<int64_t, DIMS_2> &l1TopNprobeIndicesHost);
    void fillDisOpInputDataByBlock(size_t qIdx, size_t tIdx, size_t segIdx, size_t segNum, size_t coreNum,
                                   size_t ivfFlatBlockSize, AscendTensor<uint32_t, DIMS_3, size_t> &baseSizeHostVec,
    ...
    void callL2DistanceOp(size_t batch, size_t tileNum, size_t segNum, size_t coreNum, size_t vcMaxLen,
                          AscendTensor<float, DIMS_2, size_t> &queryVec, AscendTensor<uint64_t, DIMS_3, size_t> &offset,
                          AscendTensor<uint32_t, DIMS_3, size_t> &baseSize,
                          AscendTensor<uint16_t, DIMS_3, size_t> &opFlag, AscendTensor<float, DIMS_3, size_t> &disVec,
