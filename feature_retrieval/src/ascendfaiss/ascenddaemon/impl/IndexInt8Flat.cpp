...
IndexSearchTensorShare::IndexSearchTensorShare(AscendResourcesProxy &resources, IndexSearchContext &ctx,
                                               const HeteroBlockGroupMgr &grpSpliter,
                                               const IndexSchemaBase &indexSchema)
    : groupCount(grpSpliter.Count())
{
    ...
    {
        distResult =
            std::shared_ptr<AscendTensor<float16_t, DIMS_3, size_t>>(new AscendTensor<float16_t, DIMS_3, size_t>(
                mem, {maxBlockCount, static_cast<size_t>(nq), static_cast<size_t>(indexSchema.getBlockSize())},
                stream));
        minDistResult =
            std::shared_ptr<AscendTensor<float16_t, DIMS_3, size_t>>(new AscendTensor<float16_t, DIMS_3, size_t>(
                mem, {maxBlockCount, static_cast<size_t>(nq), static_cast<size_t>(indexSchema.getBurstsOfBlock())},
                stream));
    }
    ...
    lastDistResult =
        std::shared_ptr<AscendTensor<float16_t, DIMS_3, size_t>>(new AscendTensor<float16_t, DIMS_3, size_t>(
            mem,
            {static_cast<size_t>(blockNum), static_cast<size_t>(nq), static_cast<size_t>(indexSchema.getBlockSize())},
            stream));
    lastMinDistResult = std::shared_ptr<AscendTensor<float16_t, DIMS_3, size_t>>(
        new AscendTensor<float16_t, DIMS_3, size_t>(mem,
                                                    {static_cast<size_t>(blockNum), static_cast<size_t>(nq),
                                                     static_cast<size_t>(indexSchema.getBurstsOfBlock())},
                                                    stream));
...
template <typename P>
APP_ERROR IndexInt8Flat<P>::setHeteroParam(uint32_t deviceId, size_t deviceCapacity, size_t deviceBuffer,
                                           size_t hostCapacity)
{
    ...
    APPERR_RETURN_IF_NOT_FMT(deviceBuffer >= devBufferMin, APP_ERR_INVALID_PARAM, "deviceBuffer %zu should >= %zu",
                             deviceBuffer, devBufferMin);
...
APP_ERROR IndexInt8Flat<P>::tryToSychResultAdvanced(int &hasCopiedCount, int &indexDoneCount, int indexId, int n,
                                                    int batchSize, int k, float16_t *distances, idx_t *labels,
                                                    ...
                                                    AscendTensor<idx_t, DIMS_3, size_t> &srcIndices)
{
    for (int j = hasCopiedCount; j < indexDoneCount; ++j)
    {
        ...
        auto ret = aclrtMemcpy(distances + totalIndex, totalBatchSize * sizeof(float16_t), srcDistances[j].data(),
        ...
        ret = aclrtMemcpy(labels + totalIndex, totalBatchSize * sizeof(idx_t), srcIndices[j].data(),
...
APP_ERROR IndexInt8Flat<P>::addVectorsAicpu(size_t startOffset, AscendTensor<int8_t, DIMS_2> &rawData)
{
    ...
    for (size_t i = 0; i < static_cast<size_t>(n);)
    {
        ...
        AscendTensor<int8_t, DIMS_4> dst(baseShaped[blockIdx]->data(),
                                         {utils::divUp(this->codeBlockSize, CUBE_ALIGN),
...
template <typename P>
APP_ERROR IndexInt8Flat<P>::addVectors(AscendTensor<int8_t, DIMS_2> &rawData, int num, int dim, int vecSize,
                                       int addVecNum)
{
...
APP_ERROR IndexInt8Flat<P>::addVectors(AscendTensor<int8_t, DIMS_2> &rawData)
{
    ...
    APPERR_RETURN_IF_NOT_FMT(dim == this->dims, APP_ERR_INVALID_PARAM,
                             "the dim of add vectors is %d, not equal to base", dim);
...
APP_ERROR IndexInt8Flat<P>::searchPaged(int pageId, AscendTensor<int8_t, DIMS_2> &queries, int k,
                                        ...
                                        AscendTensor<int64_t, DIMS_2> &minIndices, AscendTensor<uint8_t, DIMS_2> &mask)
{
    auto streamPtr = resources.getDefaultStream();
    auto stream = streamPtr->GetStream();
    auto streamAicpuPtr = resources.getAlternateStreams()[0];
    ...
    AscendTensor<float16_t, DIMS_3, size_t> minDistResult(
        mem, {static_cast<size_t>(blockNum), static_cast<size_t>(nq), static_cast<size_t>(this->burstsOfBlock)},
        stream);
                                       ...
                                       float16_t *distances, idx_t *labels)
{
    ...
    if (this->int8FlatIndexType == Int8FlatIndexType::INT8_FLAT_COS)
    {
        ...
        auto ret = synchronizeStream(stream);
        APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "synchronizeStream default stream: %i", ret);
        queriesNorm = std::move(tmpQueriesNorm);
    }
    AscendTensor<float16_t, DIMS_3, size_t> distResult(
        mem, {static_cast<size_t>(PAGE_BLOCKS), static_cast<size_t>(batchSize), static_cast<size_t>(codeBlockSize)},
        stream);
    AscendTensor<float16_t, DIMS_3, size_t> mvDistResult(
        mem, {static_cast<size_t>(PAGE_BLOCKS), static_cast<size_t>(batchSize), static_cast<size_t>(burstsOfBlock)},
        stream);
    ...
    uint8_t *data = continuousValue.data();
    int64_t *attrs = reinterpret_cast<int64_t *>(data + opFlagSize);
    attrs[aicpu::TOPK_MULTISEARCH_ATTR_ASC_IDX] = this->int8FlatIndexType == Int8FlatIndexType::INT8_FLAT_COS ? 0 : 1;
    ...
    // 1.3 resultTemp
    AscendTensor<float16_t, DIMS_3, size_t> mvDistances(
        mem, {static_cast<size_t>(indexSize), static_cast<size_t>(batchSize), static_cast<size_t>(k)}, streamAicpu);
    AscendTensor<idx_t, DIMS_3, size_t> mvIndices(
        mem, {static_cast<size_t>(indexSize), static_cast<size_t>(batchSize), static_cast<size_t>(k)}, streamAicpu);
    ...
    for (int indexId = 0; indexId < indexSize; ++indexId)
    {
        ...
        for (int i = 0; i < blocks; ++i)
        {
            ...
            if (blockIdx == 0)
            {
                ...
                AscendTensor<uint32_t, DIMS_1> indexOffset(indexOffsetInputs.data() + blockOffset, {actualPageBlocks});
                AscendTensor<uint32_t, DIMS_1> labelOffset(labelOffsetInputs.data() + blockOffset, {actualPageBlocks});
                AscendTensor<uint16_t, DIMS_1> reorderFlag(reorderFlagInputs.data() + blockOffset, {actualPageBlocks});
                AscendTensor<uint16_t, DIMS_3> flag(opFlag.data() + blockOffset * flagNum * FLAG_SIZE,
                                                    {actualPageBlocks, flagNum, FLAG_SIZE});

                runMultisearchTopkCompute(distResult, mvDistResult, opSize, flag, attrsInputs, indexOffset, labelOffset,
                                          reorderFlag, mvDistances, mvIndices, streamAicpu);
            ...
            uint32_t actualNum =
                std::min(static_cast<uint32_t>(index->ntotal - offset), static_cast<uint32_t>(codeBlockSize));
            ...
            auto ret =
                aclrtMemcpy(actualSize[0].data(), static_cast<size_t>(SIZE_ALIGN) * sizeof(uint32_t), actualVec.data(),
                            static_cast<size_t>(SIZE_ALIGN) * sizeof(uint32_t), ACL_MEMCPY_HOST_TO_DEVICE);
            APPERR_RETURN_IF_NOT_LOG(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "Failed to copy actualSize[0] to device");

            std::vector<const AscendTensorBase *> input{&queries, &mask, &shaped, &codesNorm, &actualSize};
            if (this->int8FlatIndexType == Int8FlatIndexType::INT8_FLAT_COS)
            {
                int posAtInput = 3;
    ...
    ret = synchronizeStream(streamAicpu);
    APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "synchronizeStream aicpu stream failed: %i", ret);
    // memcpy data back from dev to host
    for (int indexId = copyCount; indexId < indexSize; ++indexId)
    {
        ret = aclrtMemcpy(
            distances + static_cast<size_t>(indexId) * static_cast<size_t>(n) * static_cast<size_t>(k),
        ...
        ret = aclrtMemcpy(
            labels + static_cast<size_t>(indexId) * static_cast<size_t>(n) * static_cast<size_t>(k),
                                       ...
                                       AscendTensor<idx_t, DIMS_3, size_t> &indices) const
{
    ...
    APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "Mem operator error %i", ret);
    ret = aclrtMemcpy(indices.data(), indices.getSizeInBytes(), indicesInit.data(), indicesInit.size() * sizeof(idx_t),
                      ACL_MEMCPY_HOST_TO_DEVICE);
    APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "Mem operator error %i", ret);
...
void IndexInt8Flat<P>::computeNorm(AscendTensor<int8_t, DIMS_2> &rawData)
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        if (idx2 + 1 == codeBlockSize || i == num - 1)
        {
            ...
            if (isFirst)
            {
                ivecNormsL2sqr(pNormBaseSlice + (int)this->ntotal % codeBlockSize, rawData[idx][0].data(), dim, i + 1);
                idx += (i + 1);
                isFirst = false;
            }
            else
...
void IndexInt8Flat<P>::moveNormForward(idx_t srcIdx, idx_t dstIdx)
{
    ...
    auto err = aclrtMemcpy(normBase[dstIdx1]->data() + dstIdx2, sizeof(P), normBase[srcIdx1]->data() + srcIdx2,
                           sizeof(P), ACL_MEMCPY_DEVICE_TO_DEVICE);
...
void IndexInt8Flat<P>::moveShapedForward(idx_t srcIdx, idx_t dstIdx)
{
    ...
    int srcOffset =
        (srcIdx2 / CUBE_ALIGN) * (dim2 * CUBE_ALIGN * CUBE_ALIGN_INT8) + (srcIdx2 % CUBE_ALIGN) * (CUBE_ALIGN_INT8);
    int dstOffset =
        (dstIdx2 / CUBE_ALIGN) * (dim2 * CUBE_ALIGN * CUBE_ALIGN_INT8) + (dstIdx2 % CUBE_ALIGN) * (CUBE_ALIGN_INT8);
    for (int i = 0; i < dim2; i++)
    {
        auto err = deviceMemMng.MemoryCopy(*baseShaped[dstIdx1], static_cast<size_t>(dstOffset), *baseShaped[srcIdx1],
                                           static_cast<size_t>(srcOffset), CUBE_ALIGN_INT8);
        ASCEND_THROW_IF_NOT_FMT(err == EOK, "Mem error %d", (int)err);
...
void IndexInt8Flat<P>::runTopkCompute(AscendTensor<float16_t, DIMS_3, size_t> &dists,
                                      AscendTensor<float16_t, DIMS_3, size_t> &mindists,
...
template <typename P>
void IndexInt8Flat<P>::runMultisearchTopkCompute(
    AscendTensor<float16_t, DIMS_3, size_t> &dists, AscendTensor<float16_t, DIMS_3, size_t> &maxDists,
    AscendTensor<uint32_t, DIMS_3> &sizes, AscendTensor<uint16_t, DIMS_3> &flags, AscendTensor<int64_t, DIMS_1> &attrs,
    AscendTensor<uint32_t, DIMS_1> &indexOffset, AscendTensor<uint32_t, DIMS_1> &pageOffset,
    AscendTensor<uint16_t, DIMS_1> &reorderFlag, AscendTensor<float16_t, DIMS_3, size_t> &outDists,
...
APP_ERROR IndexInt8Flat<P>::computeMultisearchTopkParam(AscendTensor<uint32_t, DIMS_1> &indexOffsetInputs,
                                                        ...
                                                        std::vector<idx_t> &ntotals,
                                                        std::vector<idx_t> &offsetBlocks) const
{
