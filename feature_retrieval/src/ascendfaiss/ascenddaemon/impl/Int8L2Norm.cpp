...
Int8L2Norm::Int8L2Norm(int d) : dims(d)
{
    ...
    transform(trans.begin(), trans.end(), trans16.begin(), [](float tmp) { return faiss::ascend::fp16(tmp).data; });
    ...
    auto ret = memcpy_s(transDevice.data(), transDevice.getSizeInBytes(), trans.data(),
                        TRANSFER_SIZE * CUBE_ALIGN * sizeof(float16_t));
...
APP_ERROR Int8L2Norm::init() { return resetL2NormOperator(); }

void Int8L2Norm::runL2NormOperator(AscendTensor<int8_t, DIMS_2> &vectors, AscendTensor<float16_t, DIMS_2> &transfer,
                                   AscendTensor<uint32_t, DIMS_1> &actualNum, AscendTensor<float16_t, DIMS_1> &result,
                                   aclrtStream stream)
{
    ASCEND_THROW_IF_NOT(l2NormOp);

    std::shared_ptr<std::vector<const aclDataBuffer *>> l2NormOpInput(new std::vector<const aclDataBuffer *>(),
    ...
    std::shared_ptr<std::vector<aclDataBuffer *>> l2NormOpOutput(new std::vector<aclDataBuffer *>(),
...
APP_ERROR Int8L2Norm::resetL2NormOperator()
{
    ...
    APPERR_RETURN_IF_NOT_LOG(l2NormOpReset(l2NormOp, COMPUTE_BATCH), APP_ERR_ACL_OP_LOAD_MODEL_FAILED,
                             "op init failed");

    return APP_ERR_OK;
}

void Int8L2Norm::dispatchL2NormTask(AscendTensor<int8_t, DIMS_2> &codesData, AscendTensor<float16_t, DIMS_1> &normData,
                                    AscendTensor<uint32_t, DIMS_2> &actualNum, aclrtStream stream)
{
