...
public:
    ...
    explicit Index(idx_t d = 0, int64_t resourceSize = -1);
    ...
    APP_ERROR searchFilter(int64_t n, const float16_t* x, int64_t k, float16_t* distances, idx_t* labels,
        uint8_t* mask);

    APP_ERROR searchFilter(int64_t n, const float16_t* x, int64_t k, float16_t* distances, idx_t* labels,
        uint64_t filterSize, uint32_t* filters);

    APP_ERROR searchFilter(std::vector<Index *> indexes, int64_t n, const float16_t *x, int64_t k, float16_t *distances,
        idx_t *labels, uint32_t filterSize, std::vector<void *> &filters, bool isMultiFilterSearch);
...
protected:
    ...
    virtual APP_ERROR searchBatched(std::vector<Index *> indexes, int64_t n, const float16_t *x, int64_t k,
    ...
    virtual APP_ERROR searchBatched(int64_t n, const float16_t* x, int64_t k, float16_t* distance, idx_t* labels);
    ...
    void fvecNormsL2sqrAicpu(AscendTensor<float16_t, DIMS_1> &nr, AscendTensor<float16_t, DIMS_2> &x);
    void appendTensorDesc(std::vector<aclTensorDesc *> &desc, std::vector<int64_t> &shape,
                          aclDataType type, int64_t range[][2], aclFormat format);
    void appendTensorDesc(std::vector<aclTensorDesc *> &desc, std::vector<int64_t> &shape,
                          aclDataType type, aclFormat format);
    void destroyOpResource(aclopAttr *opAttr,
    ...
    void resetVecL2sqronlineOp();
    bool isVecL2Sqrcompile {false};
