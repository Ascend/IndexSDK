...
{
    static_assert(Dim > 0, "must have > 0 dimensions");
    ASCEND_THROW_IF_NOT(sizes.size() == Dim);
    ...
    const AscendTensor<T, Dim, IndexT, PtrTraits> &t)
{
    ...
    ASCEND_THROW_IF_NOT_FMT(ret == EOK, "Mem operator error %d", (int)ret);
...
void AscendTensor<T, Dim, IndexT, PtrTraits>::copyFrom(
    AscendTensor<T, Dim, IndexT, PtrTraits> &t, aclrtStream stream, aclrtMemcpyKind kind)
{
    // Size must match
    ...
    if (t.numElements() > 0) {
        ...
        ACL_REQUIRE_OK(aclrtMemcpyAsync((void *)this->data(), this->getSizeInBytes(),
            (void *)t.data(), t.getSizeInBytes(), kind, stream));
    }
}
    ...
    AscendTensor<T, Dim, IndexT, PtrTraits> &t, aclrtStream stream, aclrtMemcpyKind kind)
{
    ...
    if (this->numElements() > 0) {
        ...
        ACL_REQUIRE_OK(aclrtMemcpyAsync((void *)t.data(), t.getSizeInBytes(),
            (void *)this->data(), this->getSizeInBytes(), kind, stream));
    }
}

template<typename T, int Dim, typename IndexT, template<typename U> class PtrTraits>
void AscendTensor<T, Dim, IndexT, PtrTraits>::copyFromSync(
    AscendTensor<T, Dim, IndexT, PtrTraits> &t, aclrtMemcpyKind kind)
{
    // Size must match
    ASCEND_THROW_IF_NOT(this->getSizeInBytes() == t.getSizeInBytes());
    ...
    if (t.numElements() > 0) {
        ...
        auto ret = aclrtMemcpy((void *)this->data(), this->getSizeInBytes(),
            (void *)t.data(), t.getSizeInBytes(), kind);
    ...
    AscendTensor<T, Dim, IndexT, PtrTraits> &t, aclrtMemcpyKind kind)
{
    ...
    if (this->numElements() > 0) {
        ...
        auto ret = aclrtMemcpy((void *)t.data(), t.getSizeInBytes(), (void *)this->data(),
            this->getSizeInBytes(), kind);
        ASCEND_THROW_IF_NOT_FMT(ret == ACL_SUCCESS, "Mem operator error %d", (int)ret);
    }
}
...
template<typename T, int Dim, typename IndexT, template<typename U> class PtrTraits>
void *AscendTensor<T, Dim, IndexT, PtrTraits>::getVoidData() const
{
    ...
    typename TensorType::IndexType index)
{
    if (SubDim == 1) {
        ...
        return SubTensor<TensorType, SubDim - 1, PtrTraits>(tensor,
            dataPtr + static_cast<size_t>(index) * tensor.getStride(TensorType::NUM_DIM - SubDim));
    }
}
