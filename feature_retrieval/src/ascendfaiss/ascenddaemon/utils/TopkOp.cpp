...
#include "ascenddaemon/utils/HeapSort.h"
...
bool TopkOp<T, E, D, ASC, IDX>::exec(AscendTensor<D, DIMS_2> &distance, AscendTensor<IDX, DIMS_2> &indices,
    AscendTensor<D, DIMS_2> &topkDistance, AscendTensor<IDX, DIMS_2> &topkIndices, const uint32_t indexOffset)
{
    ...
    const uint32_t indexOffset)
{
    ...
    const int extremeSize = realSize / burstLen * 2;
    ...
    for (int i = 0; i < extremeSize; i += 2) {
        ...
        pushHeap(kVal, tmpDisResult, tmpIdResult, extremeDisArray[i],
            static_cast<uint32_t>(i * halfMinBatch), compare);
    }
    ...
    const int lastNum = realSize % burstLen;
    if (lastNum != 0) {
        const uint32_t startIdx =  realSize - lastNum;
        for (uint32_t j = startIdx; j < realSize; ++j) {
            if (compareEqual(disArray[j], disResult[0])) {
                continue;
            }
...
template<typename T, typename E, typename D, bool ASC, typename IDX>
bool TopkOp<T, E, D, ASC, IDX>::exec(std::tuple<D *, D *, IDX *> &opOutTp, std::tuple<D *, IDX *, int> &topKHeapTp,
    std::tuple<D *, uint32_t *> &, IDX *extremeIdxBak, const uint32_t realSize, const int blockSize,
    const int burstLen, const uint32_t indexOffset)
{
    ...
    for (int i = 0; i < extremeSize; i += 2) {
        ...
        if (realIdx >= realSize) {
            pushHeap(kVal, disResult, idResult, disArray[i * halfMinBatch],
                IDX(i * halfMinBatch + indexOffset), compare);
            continue;
        }
        realIdx = idFlag ? *(idArray + realIdx) : (indexOffset + realIdx);
        pushHeap(kVal, disResult, idResult, extremeDisArray[i],
            realIdx, compare);
    }
...
bool TopkOp<T, E, D, ASC, IDX>::exec(std::tuple<D *, D **, IDX **, int> &opOutTp,
    std::tuple<D *, IDX *, int> &topKHeapTp, std::tuple<D *, uint32_t *> &tmpHeapTp, std::tuple<int, int> &lenTp,
    ...
    const uint32_t realSize, const uint32_t indexOffset, const int burstLen)
{
    ...
    const int kVal = std::get<2>(topKHeapTp);
    bool idFlag = idArray != nullptr;
    const uint32_t burstNum = realSize - realSize % burstLen;

    // 2 reps skip index, only handle distance
    for (uint32_t i = 0, k = 0; i < burstNum; i += burstLen, k += 2) {
        const uint32_t endIdx = i + (uint32_t)burstLen;
    ...
    for (uint32_t j = burstNum; j < realSize; ++j) {
        if (compareEqual(disArray[j], disResult[0])) {
            continue;
        }
