...
class DeviceMemMng
{
   public:
    ...
    template <typename T>
    void DevVecFullProc(std::vector<std::unique_ptr<DeviceVector<T>>> &blockBase, size_t idx, bool isBlockFull,
                        aclrtStream &stream)
    {
        if (!isBlockFull)
        {
        ...
        std::for_each(fullBlockIdx.begin(), fullBlockIdx.end(),
                      [&blockBase](size_t idx) { blockBase[idx]->pushData(); });
        fullBlockIdx.clear();
        fullBlockIdx.emplace_back(idx);
    }
    ...
    template <typename T>
    void AddFinishProc(std::vector<std::unique_ptr<DeviceVector<T>>> &blockBase)
    {
        if (strategy == DevMemStrategy::PURE_DEVICE_MEM)
        {
            return;
    ...
    template <typename T>
    void PushDataAfterSearch(std::vector<std::unique_ptr<DeviceVector<T>>> &blockBase, size_t blockOffsetIdx,
                             size_t curPageBlockIdx, size_t curPageBlockNum, aclrtStream &stream)
    {
        if (strategy == DevMemStrategy::PURE_DEVICE_MEM)
        {
            return;
    ...
    APP_ERROR MemoryCopy(DeviceVector<T> &dst, size_t dstOffset, DeviceVector<T> &src, size_t srcOffset, size_t copyCnt)
    {
        if (strategy == DevMemStrategy::PURE_DEVICE_MEM)
        {
            return aclrtMemcpy(dst.data() + dstOffset, (dst.size() - dstOffset) * sizeof(T), src.data() + srcOffset,
                               copyCnt * sizeof(T), ACL_MEMCPY_DEVICE_TO_DEVICE);
        }
