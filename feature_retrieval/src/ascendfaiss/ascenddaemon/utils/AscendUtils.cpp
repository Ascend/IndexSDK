...
void AscendMultiThreadManager::GetMultiThreadMode(bool &multiThreadFlag)
{
    ...
    multiThreadFlag =
        (multiThreadEnv != nullptr) && (std::strlen(multiThreadEnv) == envLen) && (multiThreadEnv[0] == envVal);
...
std::optional<std::lock_guard<std::mutex>> AscendMultiThreadManager::LockGetBaseMtx(
    int deviceId, std::unordered_map<int, std::mutex> &getBaseMtx)
{
    if (!IsMultiThreadMode())
    {
        return std::nullopt;
...
{
    ASCEND_THROW_IF_NOT_FMT(device <= MAX_DEVICEID, "deviceId[%zu] not in range[0, %zu]", device, MAX_DEVICEID);
