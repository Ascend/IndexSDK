...
class TSBase : public TSSuperBase
{
   ...
   public:
    ...
    APP_ERROR AddFeatureByIndice(int64_t n, const void *features, const faiss::ascend::FeatureAttr *attrs,
    ...
    APP_ERROR GetFeatureByIndice(int64_t count, const int64_t *indices, int64_t *labels, void *features,
                                 faiss::ascend::FeatureAttr *attributes,
                                 faiss::ascend::ExtraValAttr *extraVal) const override;
    ...
    void generateMask(int batch, int blockOffset, int blockNum, AscendTensor<int32_t, DIMS_2> &queryTimes,
                      AscendTensor<uint8_t, DIMS_2> &tokenIds, AscendTensor<uint8_t, DIMS_3> &masks);
    void generateMaskWithExtra(int batch, int blockOffset, int blockNum, AscendTensor<int32_t, DIMS_2> &queryTimes,
   ...
   protected:
    ...
    void runBatchMaskGenerateCompute(int batch, const std::vector<const AscendTensorBase *> &input,
                                     const std::vector<const AscendTensorBase *> &output, aclrtStream stream) const;
    void runBatchMaskValGenerateCompute(int batch, const std::vector<const AscendTensorBase *> &input,
                                        const std::vector<const AscendTensorBase *> &output, aclrtStream stream) const;
    // extra mask op
    ...
    void runBatchExtraMaskGenerateCompute(int batch, const std::vector<const AscendTensorBase *> &input,
    ...
    void runBatchExtraAndBaseMaskGenerateCompute(int batch, const std::vector<const AscendTensorBase *> &input,
                                                 const std::vector<const AscendTensorBase *> &output,
                                                 aclrtStream stream) const;
    void runVectorRemoveOp(const std::vector<uint64_t> &src, const std::vector<uint64_t> &dst, int dataType,
                           int copyNum);
    ...
    template <typename T>
    T *calcAttrStartAddress(const std::vector<std::unique_ptr<DeviceVector<T>>> &attrVec, size_t blockId,
    ...
    void ReshapeAttrsSpace(int64_t newAddCount, const uint8_t *customAttr, const faiss::ascend::ExtraValAttr *extraVal);
   ...
   private:
    ...
    void copyLastFeatureAttrs(int64_t cpyNum, int64_t oldBlockNum, int lastBlockOffset, std::vector<int32_t> &tmpTimes,
                              std::vector<int32_t> &tmpTokenQs, std::vector<uint8_t> &tmpTokenRs);
    void copyLastExtraValAttrs(int64_t cpyNum, int64_t oldBlockNum, int lastBlockOffset, std::vector<int16_t> &tmpVals);

    void copyBlockFeatureAttrs(int64_t copyNum, int offset, int64_t i, std::vector<int32_t> &tmpTimes,
                               std::vector<int32_t> &tmpTokenQs, std::vector<uint8_t> &tmpTokenRs);
