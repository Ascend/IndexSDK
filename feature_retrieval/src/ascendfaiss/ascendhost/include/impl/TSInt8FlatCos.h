...
class TSInt8FlatCos : public TSBase, public IndexInt8FlatCosAicpu
{
   public:
    ...
    APP_ERROR GetFeatureByIndice(int64_t count, const int64_t *indices, int64_t *labels, void *features,
                                 faiss::ascend::FeatureAttr *attributes, faiss::ascend::ExtraValAttr *extraVal) const;
   ...
   private:
    APP_ERROR searchPagedWithMasks(int pageIdx, int batch, const int8_t *features, int topK,
                                   ...
                                   const float16_t *extraScore = nullptr);
    ...
    void runAscendcInt8CosDistCompute(int batch, bool shareMask, const std::vector<const AscendTensorBase *> &input,
                                      const std::vector<const AscendTensorBase *> &output, aclrtStream stream) const;

    void runInt8CosDistCompute(int batch, bool shareMask, const std::vector<const AscendTensorBase *> &input,
                               const std::vector<const AscendTensorBase *> &output, aclrtStream stream) const;
    ...
    APP_ERROR copyExtraScoreToDev(AscendTensor<float16_t, DIMS_2, idx_t> &extraScoreTensor, const float16_t *extraScore,
                                  int count);
    void calOpSize(std::vector<uint32_t> &opSizeHost, int computeNum, int blockNum);
    APP_ERROR callSearchWithShareMaskByBatch(int32_t queryNum, const int8_t *features, uint32_t topk, int64_t *labels,
                                             float *distances, AscendTensor<uint8_t, DIMS_3> &masks);
