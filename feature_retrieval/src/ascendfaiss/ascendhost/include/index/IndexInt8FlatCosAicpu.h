...
class IndexInt8FlatCosAicpu : public IndexInt8Flat<float16_t>
{
   ...
   private:
    APP_ERROR prepareIndexSearchTensorShare(const HeteroBlockGroupMgr &grpSpliter, IndexSearchContext &ctx,
                                            IndexSearchTensorShare &outCond);
    APP_ERROR searchInGroups(const HeteroBlockGroupMgr &grpSpliter, IndexSearchContext &ctx,
                             AscendTensor<uint8_t, DIMS_2> &mask);
    APP_ERROR searchInGroup(size_t grpId, const HeteroBlockGroupMgr &grpSpliter, IndexSearchContext &ctx,
                            IndexSearchTensorShare &tensorCond, AscendTensor<uint8_t, DIMS_2> &mask);
    ...
    APP_ERROR getThreshold(AscendTensor<float16_t, DIMS_2> &topKThreshold, int nq, int k, int pageId,
