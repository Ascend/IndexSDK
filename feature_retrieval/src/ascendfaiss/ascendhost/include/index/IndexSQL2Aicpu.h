...
#ifndef ASCEND_INDEXSQ_L2_AICPU_INCLUDED
#define ASCEND_INDEXSQ_L2_AICPU_INCLUDED

#include "ascenddaemon/impl/IndexSQ.h"

namespace ascend {
class IndexSQL2Aicpu : public IndexSQ {
public:
    IndexSQL2Aicpu(int dim, bool filterable, int64_t resource = -1, int blockSize = SQ_DEFAULT_DIST_COMPUTE_BATCH);

    ~IndexSQL2Aicpu();

    APP_ERROR init() override;

    APP_ERROR addVectors(size_t numVecs, const uint8_t *data, const float *preCompute) override;
...
private:
    ...
    APP_ERROR savePreComputeHostCpu(int numVecs, const float *preCompute);

    void moveVectorForward(idx_t srcIdx, idx_t dstIdx);
...
#endif // ASCEND_INDEXSQ_L2_AICPU_INCLUDED