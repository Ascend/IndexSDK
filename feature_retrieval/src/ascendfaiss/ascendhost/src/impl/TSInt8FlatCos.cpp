...
APP_ERROR TSInt8FlatCos::addFeatureWithLabels(int64_t n, const void *features, const faiss::ascend::FeatureAttr *attrs,
                                              const int64_t *labels, const uint8_t *customAttr,
                                              const faiss::ascend::ExtraValAttr *extraVal)
{
    auto getAddModeFunctor = [this, &extraVal]() { this->isInt8FirstUseExtraVal = (extraVal != nullptr); };
...
APP_ERROR TSInt8FlatCos::AddFeatureByIndice(int64_t n, const void *features, const faiss::ascend::FeatureAttr *attrs,
...
APP_ERROR TSInt8FlatCos::GetFeatureByIndice(int64_t count, const int64_t *indices, int64_t *labels, void *features,
                                              ...
                                              const int8_t *features)
{
    ...
    int64_t singleAddMax =
        static_cast<int64_t>(utils::divDown(UPPER_LIMIT_FOR_ADD, this->codeBlockSize) * this->codeBlockSize);
    // 新增部分的空间申请
                                              ...
                                              const float16_t *extraScore)
{
    ...
    for (int i = 0; i < blockNum; ++i)
    {
        ...
        {
            AscendTensor<float16_t, DIMS_3, size_t> extraScoreDev(
                const_cast<float16_t *>(extraScore),
        ...
        else
        {
            std::vector<const AscendTensorBase *> input{&queries,     &mask,      &shaped,
                                                        &queriesNorm, &codesNorm, &actualSize};
            if (faiss::ascend::SocUtils::GetInstance().IsAscend910B())
            {
                                                   ...
                                                   const float16_t *extraScore)
{
    ...
    for (int pageIdx = 0; pageIdx < pageNum; ++pageIdx)
    {
        ...
        auto ret = searchPagedWithMasks(pageIdx, batch, features, topK, subMasks, outDistanceOnDevice,
...
APP_ERROR TSInt8FlatCos::searchBatchWithExtraNonshareMasks(int batch, const int8_t *features, int topK,
                                                           ...
                                                           AscendTensor<int32_t, DIMS_2> &queryTimes,
                                                           AscendTensor<uint8_t, DIMS_2> &tokenIds,
                                                           const uint8_t *extraMask, const float16_t *extraScore)
{
...
APP_ERROR TSInt8FlatCos::searchInPureDev(uint32_t count, const int8_t *features,
                                         ...
                                         float *distances, const faiss::ascend::ExtraValFilter *extraValFilter)
{
    auto streamPtr = resources.getDefaultStream();
    auto stream = streamPtr->GetStream();
    auto &mem = resources.getMemoryManager();
    ...
    for (auto batch : this->searchBatchSizes)
    {
        for (; queryNum >= batch; queryNum -= batch)
        {
            ...
            auto ret = searchBatchWithNonshareMasks(batch, features + offset * this->code_size, static_cast<int>(topk),
                                                    distances + offset * topk, labels + offset * topk, queryTimes,
                                                    tokenIds, ValFilter);
            APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR,
                                ...
                                const faiss::ascend::ExtraValFilter *extraValFilter)
{
    ...
    else
    {
        ...
        ret = searchBatched(static_cast<idx_t>(count), tensorDevQueries.data(), attrFilter, static_cast<idx_t>(topk),
                            fp16Distance.data(), fp16Indexs.data(), nullptr, 0, false, extraValFilter);
                                                        ...
                                                        AscendTensor<uint8_t, DIMS_3> &masks)
{
    ...
    for (auto batch : this->searchBatchSizes)
    {
        while (queryNum >= batch)
        {
            ...
            APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "searchBatchWithShareMasks failed: %i.\n",
                                     ret);
                                                      ...
                                                      const float16_t *extraScore)
{
    ...
    if (this->shareAttrFilter)
    {
        APPERR_RETURN_IF_NOT_LOG(extraScore == nullptr, APP_ERR_INVALID_PARAM,
                                 "TSInt8FlatCos is not suport Extra Score when shareAttrFilter is true!");
    ...
    for (auto batch : this->searchBatchSizes)
    {
        while (queryNum >= batch)
        {
            ...
            {
                extraScoreDev =
                    AscendTensor<float16_t, DIMS_2, idx_t>(mem, {static_cast<idx_t>(batch), ntotalPad}, stream);
                auto ret =
                    aclrtMemcpy(extraScoreDev.data(), extraScoreDev.getSizeInBytes(), extraScore + offset * ntotalPad,
                                batch * ntotalPad * sizeof(float16_t), ACL_MEMCPY_HOST_TO_DEVICE);
                APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "aclrtMemcpy error %d", ret);
                extraScoreAddr = static_cast<const float16_t *>(extraScoreDev.data());
            }
            auto ret = searchBatchWithExtraNonshareMasks(
                batch, features + offset * this->code_size, static_cast<int>(topk), distances + offset * topk,
                labels + offset * topk, queryTimes, tokenIds, extraMask + offset * extraMaskLen, extraScoreAddr);
            APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR,
                                ...
                                idx_t *inLabels, int64_t *outLabels, uint32_t *validNums)
{
    ...
    {
        std::transform(inDistances + i * topK, inDistances + (i + 1) * topK, outDistances + i * topK,
                                ...
                                AscendTensor<int64_t, DIMS_2> &outIndicesOnDevice, float *distances, int64_t *labels)
{
    ...
    for (uint64_t i = 0; i < searchNum; ++i)
    {
        std::transform(outDistances.data() + i * topK, outDistances.data() + (i + 1) * topK, distances + i * topK,
                       [&](const float16_t temp) -> float { return static_cast<float>(faiss::ascend::fp16(temp)); });
        std::transform(labels + i * topK, labels + (i + 1) * topK, labels + i * topK, [&](const idx_t temp) -> int64_t
                       { return static_cast<int64_t>(temp) == -1 ? -1 : static_cast<int64_t>(ids.at(temp)); });
    }
    APP_LOG_INFO("TSInt8FlatCos postProcess operation end.\n");
}
...
void TSInt8FlatCos::add_with_ids(idx_t n, const int8_t *x, const idx_t *xids)
{
    ...
    deviceMemMng.AddFinishProc(baseShaped);
...
void TSInt8FlatCos::queryVectorByIdx(int64_t idx, uint8_t *dis) const
{
    if (faiss::ascend::SocUtils::GetInstance().IsAscend910B())
    {
        ...
        auto ret = aclrtMemcpy(xb.data(), this->code_size * sizeof(uint8_t), baseShaped[blockIdx]->data() + srcOffset,
                               this->code_size * sizeof(uint8_t), ACL_MEMCPY_DEVICE_TO_HOST);
                                         ...
                                         const std::vector<const AscendTensorBase *> &output, aclrtStream stream) const
{
    IndexTypeIdx indexType =
        shareMask ? IndexTypeIdx::ITI_INT8_COS_SHARE_MASK_EXTRASCORE : IndexTypeIdx::ITI_INT8_COS_MASK_EXTRASCORE;
    std::vector<int> keys({batch, dims, static_cast<int>(tokenNum)});
    OpsMngKey opsKey(keys);
    auto ret = DistComputeOpsManager::getInstance().runOp(indexType, opsKey, input, output, stream);
    ASCEND_THROW_IF_NOT_FMT(ret == APP_ERR_OK, "run operator failed: %i\n", ret);
...
APP_ERROR TSInt8FlatCos::resetAscendcInt8CosDistCompute(int codeNum) const
{
    ...
    for (auto batch : searchBatchSizes)
    {
        ...
        std::vector<std::pair<aclDataType, std::vector<int64_t>>> input{
            {ACL_INT8, queryShape},          {ACL_UINT8, maskShape},        {ACL_INT8, codeShape},
            {ACL_FLOAT16, queriesNormShape}, {ACL_FLOAT16, codesNormShape}, {ACL_UINT32, sizeShape},
        };
        std::vector<std::pair<aclDataType, std::vector<int64_t>>> output{
            {ACL_FLOAT16, resultShape}, {ACL_FLOAT16, minResultShape}, {ACL_UINT16, flagShape}};
        std::vector<int> keys({batch, dims, static_cast<int>(tokenNum)});
        OpsMngKey opsKey(keys);
