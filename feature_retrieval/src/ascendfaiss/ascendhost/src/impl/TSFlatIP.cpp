...
{
    FAISS_THROW_IF_NOT_FMT(std::find(DIM_RANGE.begin(), DIM_RANGE.end(), dim) != DIM_RANGE.end(), "Unsupported dims %u",
                           dim);
...
APP_ERROR TSFlatIP::addFeatureWithLabels(int64_t n, const void *features, const faiss::ascend::FeatureAttr *attrs,
...
APP_ERROR TSFlatIP::AddFeatureByIndice(int64_t n, const void *features, const faiss::ascend::FeatureAttr *attrs,
                                       const int64_t *indices, const uint8_t *customAttr,
                                       const faiss::ascend::ExtraValAttr *)
{
...
APP_ERROR TSFlatIP::GetFeatureByIndice(int64_t count, const int64_t *indices, int64_t *labels, void *features,
                                       faiss::ascend::FeatureAttr *attributes, faiss::ascend::ExtraValAttr *) const
{
...
APP_ERROR TSFlatIP::AddFeatureImpl(int64_t singleAdd, const float *features, int64_t offset, int64_t startOffset)
{
    ...
    APPERR_RETURN_IF_NOT_LOG(featureInt8 != nullptr, APP_ERR_ACL_BAD_ALLOC, "Memory allocation fail for featureInt8");
...
void TSFlatIP::removeIdsImpl(const std::vector<int64_t> &indices)
{
    ...
    for (int i = 0; i < removedCnt; i++)
    {
        ...
        float16_t *srcDataPtr = baseShaped[srcIdx1]->data() +
                                (srcIdx2 / zRegionHeight) * (dimAlignSize * zRegionHeight * CUBE_ALIGN) +
                                (srcIdx2 % zRegionHeight) * CUBE_ALIGN;
        float16_t *dstDataPtr = baseShaped[dstIdx1]->data() +
                                (dstIdx2 / zRegionHeight) * (dimAlignSize * zRegionHeight * CUBE_ALIGN) +
                                (dstIdx2 % zRegionHeight) * CUBE_ALIGN;
...
void TSFlatIP::resetAscendcDistMaskCompOp(int numLists)
{
    ...
    auto distCompMaskOpReset = [&](int batch, bool shareMask)
    {
        IndexTypeIdx indexMaskType =
            shareMask ? IndexTypeIdx::ITI_ASCENDC_FLAT_IP_SHARE_MASK : IndexTypeIdx::ITI_ASCENDC_FLAT_IP_MASK;
        ...
        std::vector<std::pair<aclDataType, std::vector<int64_t>>> output{
            {ACL_FLOAT16, distResultShape}, {ACL_FLOAT16, maxResultShape}, {ACL_UINT16, flagShape}};
...
void TSFlatIP::resetDistMaskExtraScoreCompOp(int numLists)
{
    ...
    {
        IndexTypeIdx indexMaskType =
            shareMask ? IndexTypeIdx::ITI_FLAT_IP_SHARE_MASK_EXTRA_SCORE : IndexTypeIdx::ITI_FLAT_IP_MASK_EXTRA_SCORE;
        std::vector<int64_t> queryShape({batch, dims});
        std::vector<int64_t> coarseCentroidsShape(
            {utils::divUp(numLists, CUBE_ALIGN), utils::divUp(dims, CUBE_ALIGN), CUBE_ALIGN, CUBE_ALIGN});
        std::vector<int64_t> sizeShape({CORE_NUM, SIZE_ALIGN});
        ...
        std::vector<std::pair<aclDataType, std::vector<int64_t>>> input{
            {ACL_FLOAT16, queryShape}, {ACL_FLOAT16, coarseCentroidsShape}, {ACL_UINT32, sizeShape},
            {ACL_UINT8, maskShape},    {ACL_FLOAT16, extraScoreShape},
        };
        std::vector<std::pair<aclDataType, std::vector<int64_t>>> output{
            {ACL_FLOAT16, distResultShape}, {ACL_FLOAT16, maxResultShape}, {ACL_UINT16, flagShape}};
...
void TSFlatIP::resetDistMaskWithScaleCompOp(int numLists)
{
    ...
    {
        IndexTypeIdx indexMaskType = isUsedExtraScore ? IndexTypeIdx::ITI_FLAT_IP_EXTRA_SCORE_AND_SCALE
                                                      : IndexTypeIdx::ITI_FLAT_IP_NOSCORE_AND_SCALE;
        std::string opTypeName =
            isUsedExtraScore ? "DistanceFlatIPMaxsWithScale" : "DistanceFlatIPMaxsNoScoreWithScale";
        std::vector<int64_t> queryShape({batch, dims});
        std::vector<int64_t> coarseCentroidsShape(
        ...
        std::vector<std::pair<aclDataType, std::vector<int64_t>>> input{
            {ACL_FLOAT16, queryShape}, {ACL_INT8, coarseCentroidsShape}, {ACL_UINT32, sizeShape},
            {ACL_UINT8, maskShape},    {ACL_FLOAT16, scaleShape},
        };
        if (isUsedExtraScore)
        {
            input = {
                {ACL_FLOAT16, queryShape}, {ACL_INT8, coarseCentroidsShape}, {ACL_UINT32, sizeShape},
                {ACL_UINT8, maskShape},    {ACL_FLOAT16, extraScoreShape},   {ACL_FLOAT16, scaleShape},
            };
        }
        std::vector<std::pair<aclDataType, std::vector<int64_t>>> output{
            {ACL_FLOAT16, distResultShape}, {ACL_FLOAT16, maxResultShape}, {ACL_UINT16, flagShape}};
                                           ...
                                           const std::vector<const AscendTensorBase *> &output, aclrtStream stream)
{
    ...
    IndexTypeIdx indexType = isUsedExtraScore ? IndexTypeIdx::ITI_FLAT_IP_EXTRA_SCORE_AND_SCALE
                                              : IndexTypeIdx::ITI_FLAT_IP_NOSCORE_AND_SCALE;
    std::vector<int> keys({batch, dims, static_cast<int>(tokenNum)});
    OpsMngKey opsKey(keys);
    auto ret = DistComputeOpsManager::getInstance().runOp(indexType, opsKey, input, output, stream);
                           ...
                           bool enableTimeFilter, const faiss::ascend::ExtraValFilter *)
{
    ...
    {
        APPERR_RETURN_IF_NOT_LOG(scale.empty(), APP_ERR_INVALID_PARAM,
                                 "TSFlatIP is not suport scale when shareAttrFilter is true!");
    ...
    else
    {
        for (auto batch : searchBatchSizes)
        {
            while (queryNum >= batch)
            {
                ...
                APP_ERROR ret =
                    searchBatchWithNonshareMasks(batch, query.data(), static_cast<int>(topk), distances + offset * topk,
                                                 labels + offset * topk, queryTimes, tokenIds);
                APPERR_RETURN_IF_NOT_FMT(ret == 0, APP_ERR_INNER_ERROR, "searchBatchWithNonshareMasks failed(%d).",
                                         ret);
                offset += batch;
                queryNum -= batch;
            }
                                                 ...
                                                 AscendTensor<uint8_t, DIMS_2> &tokenIds)
{
    ...
    for (size_t pageId = 0; pageId < pageNum; ++pageId)
    {
        ...
        AscendTensor<uint8_t, DIMS_3> masks(mem, {blockNum, batch, utils::divUp(this->blockSize, MASK_ALIGN)}, stream);
                                                      ...
                                                      float16_t *extraScore)
{
    ...
    for (size_t pageId = 0; pageId < pageNum; ++pageId)
    {
        ...
        {
            baseMaskDev =
                AscendTensor<uint8_t, DIMS_3>(mem, {blockNum, 1, utils::divUp(this->blockSize, MASK_ALIGN)}, stream);
            auto ret =
                aclrtMemcpy(baseMaskDev.data(), baseMaskDev.getSizeInBytes(), baseMask.data() + pageOffset / MASK_ALIGN,
                            utils::divUp(computeNum, MASK_ALIGN), ACL_MEMCPY_HOST_TO_DEVICE);
            APPERR_RETURN_IF_NOT_FMT(ret == 0, APP_ERR_INNER_ERROR, "copy baseMask failed(%d).", ret);
        }
        AscendTensor<uint8_t, DIMS_3> subMasks(
            mem, {blockNum, this->shareAttrFilter ? 1 : batch, utils::divUp(this->blockSize, MASK_ALIGN)}, stream);
                                         ...
                                         AscendTensor<int64_t, DIMS_2> &maxIndices, float16_t *extraScore)
{
    ...
    for (int i = 0; i < blockNum; ++i)
    {
        ...
        opSizeData[opSizeHostIdx + 1] = static_cast<uint32_t>(pageOffset) + static_cast<uint32_t>(offset);
    ...
    for (int i = 0; i < blockNum; ++i)
    {
        ...
        if (extraScore != nullptr)
        {
            ...
            if (!scale.empty())
            {
                ...
                std::vector<const AscendTensorBase *> input = {&queries, &shapedInt8,    &actualSize,
                                                               &mask,    &extraScoreDev, &scaleReciprocal};
        ...
        else
        {
            if (!scale.empty())
            {
                ...
                AscendTensor<int8_t, DIMS_4> shapedInt8(baseShapedInt8[blockOffset + static_cast<size_t>(i)]->data(),
                                                        {dim1, int8Dim2, CUBE_ALIGN, CUBE_ALIGN_INT8});
                std::vector<const AscendTensorBase *> input = {&queries, &shapedInt8, &actualSize, &mask,
                                                               &scaleReciprocal};
    ...
    ret = synchronizeStream(stream);
    APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "synchronizeStream default stream failed: %i\n",
                             ret);
...
void TSFlatIP::postProcess(int64_t searchNum, int topK, float16_t *outDistances, float *distances, int64_t *labels)
{
    ...
    for (int64_t i = 0; i < searchNum; ++i)
    {
        std::transform(outDistances + i * topK, outDistances + (i + 1) * topK, distances + i * topK,
                       [&](const float16_t temp) -> float { return static_cast<float>(faiss::ascend::fp16(temp)); });
        std::transform(labels + i * topK, labels + (i + 1) * topK, labels + i * topK, [&](const int64_t temp) -> int64_t
                       { return temp == -1 ? -1 : static_cast<int64_t>(ids.at(temp)); });
    }
                                           ...
                                           float *distances)
{
    ...
    for (auto batch : searchBatchSizes)
    {
        while (queryNum >= batch)
        {
            ...
            AscendTensor<int32_t, DIMS_2> queryTimes(mem, {batch, OPS_DATA_TYPE_ALIGN}, stream);
            AscendTensor<uint8_t, DIMS_2> tokenIds(
                mem, {batch, static_cast<int32_t>(utils::divUp(tokenNum, OPS_DATA_TYPE_ALIGN) * OPS_DATA_TYPE_TIMES)},
                stream);
            buildAttr(attrFilter + offset, batch, queryTimes, tokenIds);
            ...
            if (extraScore != nullptr)
            {
                ...
                auto ret =
                    aclrtMemcpy(extraScoreDev.data(), extraScoreDev.getSizeInBytes(),
            ...
            APP_ERROR ret = searchBatchWithExtraNonshareMasks(
                batch, query.data(), static_cast<int>(topk), distances + offset * topk, labels + offset * topk,
                queryTimes, tokenIds, extraMask + offset * extraMaskLen, extraScoreDev.data());
            APPERR_RETURN_IF_NOT_FMT(ret == 0, APP_ERR_INNER_ERROR, "searchBatchWithExtraNonshareMasks failed(%d).",
                                     ret);
            offset += batch;
            queryNum -= batch;
        }
                                        ...
                                        uint32_t *validNums, bool enableTimeFilter, const float16_t *extraScore)
{
    ...
    if (this->shareAttrFilter)
    {
        APPERR_RETURN_IF_NOT_LOG(extraScore == nullptr, APP_ERR_INVALID_PARAM,
                                 "TSFlatIP is not suport Extra Score when shareAttrFilter is true!");
        APPERR_RETURN_IF_NOT_LOG(scale.empty(), APP_ERR_INVALID_PARAM,
                                 "TSFlatIP is not suport scale when shareAttrFilter is true!");
    ...
    {
        APP_ERROR ret = runNonSharedAttrFilter(queryNum, topk, extraMaskLen, attrFilter, extraMask, queryFeatures,
                                               extraScore, labels, distances);
...
void TSFlatIP::queryVectorByIdx(int64_t idx, float *dis) const
{
    if (faiss::ascend::SocUtils::GetInstance().IsAscend910B())
    {
        ...
        auto ret = aclrtMemcpy(xb.data(), this->code_size * sizeof(uint16_t), baseShaped[blockIdx]->data() + srcOffset,
                               this->code_size * sizeof(uint16_t), ACL_MEMCPY_DEVICE_TO_HOST);
    ...
    else
    {
        ...
        for (int i = 0; i < reshapeDim2; ++i)
        {
            auto ret = aclrtMemcpy(xb.data() + disOffset, CUBE_ALIGN * sizeof(uint16_t),
                                   baseShaped[blockIdx]->data() + srcOffset, CUBE_ALIGN * sizeof(uint16_t),
...
void TSFlatIP::queryInt8VectorByIdx(int64_t idx, float *dis) const
{
    if (faiss::ascend::SocUtils::GetInstance().IsAscend910B())
    {
        ...
        auto ret = aclrtMemcpy(xb.data(), this->code_size * sizeof(int8_t), baseShaped[blockIdx]->data() + srcOffset,
                               this->code_size * sizeof(int8_t), ACL_MEMCPY_DEVICE_TO_HOST);
        FAISS_THROW_IF_NOT_MSG(ret == ACL_SUCCESS, "Failed to copy to host");
...
APP_ERROR TSFlatIP::SetScale(const std::vector<float> &scale)
{
    ...
    auto ret =
        aclrtMemcpy(scaleReciprocalDevice.data(), scaleReciprocalDevice.getSizeInBytes(), scaleReciprocalHost.data(),
                    scaleReciprocalHost.size() * sizeof(float16_t), ACL_MEMCPY_HOST_TO_DEVICE);
