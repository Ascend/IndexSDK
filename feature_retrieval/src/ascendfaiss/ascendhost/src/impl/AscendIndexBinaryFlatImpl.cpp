...
AscendIndexBinaryFlatImpl::AscendIndexBinaryFlatImpl(int dims, AscendIndexBinaryFlatConfig config, bool usedFloat,
    ...
    : resourceSize(config.resourceSize)
{
    CheckParam(config, allowAscend910);
...
void AscendIndexBinaryFlatImpl::CheckParam(AscendIndexBinaryFlatConfig config, bool allowAscend910)
{
    ...
    if (allowAscend910)
    {
        FAISS_THROW_IF_NOT_MSG(aclrtSetDevice(config.deviceList.front()) == ACL_ERROR_NONE,
                               "Failed to set device for AscendIndexBinaryFlat.");
    }
    ...
    const bool supportedSoc = deviceName.find("Ascend310P") != std::string::npos ||
                              (allowAscend910 && SocUtils::GetInstance().IsAscend910B());
    const char *errorMessage = allowAscend910
                                   ? "AscendIndexBinaryFlat can only be used on Ascend310P* or Atlas A2/A3 device."
                                   : "AscendIndexBinaryFlat can only be used on Ascend310P* device.";
    FAISS_THROW_IF_NOT_FMT(supportedSoc, "%s", errorMessage);
}
...
void AscendIndexBinaryFlatImpl::setRemoveFast(bool useRemoveFast) { isRemoveFast = useRemoveFast; }
...
void AscendIndexBinaryFlatImpl::addVectors(int n, const uint8_t *x)
{
    ...
    for (size_t i = 0; i < static_cast<size_t>(n);)
    {
        ...
        AscendTensor<uint8_t, DIMS_4> dst(baseShaped[blockIdx]->data(), {reshapeDim1, reshapeDim2, align1, align2});
...
void AscendIndexBinaryFlatImpl::searchPaged(int pageIdx, int batch, const uint8_t *x, int topK,
...
void AscendIndexBinaryFlatImpl::searchPaged(int pageIdx, int batch, AscendTensor<float16_t, DIMS_2> &queries, int topK,
                                            AscendTensor<float16_t, DIMS_2> &outDistanceOnDevice,
                                            AscendTensor<idx_t, DIMS_2> &outIndicesOnDevice)
{
    ...
    for (int i = 0; i < blockNum; ++i)
    {
        ...
        opSizeData[opSizeHostIdx] =
            std::min(static_cast<uint32_t>(computeNum - offset), static_cast<uint32_t>(BLOCK_SIZE));
    }
...
void AscendIndexBinaryFlatImpl::searchBatch(int batch, const float *x, int topK, float *distances, idx_t *labels)
{
    ...
    auto ret = aclrtMemcpy(queries.data(), queries.getSizeInBytes(), query.data(), batch * this->d * sizeof(uint16_t),
                           ACL_MEMCPY_HOST_TO_DEVICE);
...
void AscendIndexBinaryFlatImpl::search(idx_t n, const float *x, idx_t k, float *distances, idx_t *labels)
{
    ...
    for (auto batch : BATCH_SIZES)
    {
        while (n >= batch)
        {
            searchBatch(batch, x + offset * this->d, k, distances + offset * static_cast<int64_t>(k),
...
void AscendIndexBinaryFlatImpl::copyCode(const faiss::IndexBinaryFlat *index, const idx_t *xids)
{
    ...
    FAISS_THROW_IF_NOT_FMT(
        index->xb.size() == static_cast<size_t>(index->ntotal) * static_cast<size_t>(this->code_size),
...
void AscendIndexBinaryFlatImpl::getVectors(uint32_t offset, uint32_t num, std::vector<uint8_t> &xb) const
{
    ...
    for (size_t i = 0; i < num;)
    {
        ...
        AscendTensor<uint8_t, DIMS_4> src(baseShaped[blockIdx]->data(), {reshapeDim1, reshapeDim2, align1, align2});
...
size_t AscendIndexBinaryFlatImpl::removeIdsImpl(const std::vector<idx_t> &indices)
{
    ...
    for (int i = 0; i < removedCnt; i++)
    {
        ...
        uint8_t *srcDataPtr = baseShaped[srcIdx1]->data() + (srcIdx2 / align1) * (dimAlignSize * align1 * align2) +
                              (srcIdx2 % align1) * align2;
        uint8_t *dstDataPtr = baseShaped[dstIdx1]->data() + (dstIdx2 / align1) * (dimAlignSize * align1 * align2) +
                              (dstIdx2 % align1) * align2;
...
void AscendIndexBinaryFlatImpl::setRemoveAttr(AscendTensor<int64_t, DIMS_1> &attrsInput, int dimAlignSize, int align1,
                                              int align2) const
{
