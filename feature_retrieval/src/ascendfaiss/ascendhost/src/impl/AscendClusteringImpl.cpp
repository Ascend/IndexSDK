...
const idx_t CLUSTERING_MAX_N = 10000001;
...
AscendClusteringImpl::AscendClusteringImpl(int d, int k, MetricType metricType, AscendClusteringConfig config,
                                           AscendClustering *intf)
    : intf_(intf),
      ...
      updateNtotal(0)
{
    ...
    FAISS_THROW_IF_NOT_FMT(k > 0 && k <= MAX_NLISTS, "number of centroids(%d) should be in range (0, %d]", k,
                           MAX_NLISTS);
    ...
    FAISS_THROW_IF_NOT_MSG(
        config.resourceSize == -1 || (config.resourceSize >= 0 && config.resourceSize <= CLUSTERING_MAX_MEM),
...
void AscendClusteringImpl::init()
{
    ...
    FAISS_THROW_IF_NOT_FMT(static_cast<uint32_t>(deviceId) < devCount, "Device %d is invalid, total device %u",
                           deviceId, devCount);
...
void AscendClusteringImpl::add(idx_t n, const float *x)
{
    ...
    ASCEND_THROW_IF_NOT_FMT((n > 0) && (n < CLUSTERING_MAX_N), "n must be > 0 and n < %ld", CLUSTERING_MAX_N);
    ASCEND_THROW_IF_NOT_FMT((n > 0) && (ntotal + static_cast<size_t>(n) < CLUSTERING_MAX_N),
                            "n must be > 0 and ntotal+n < %ld", CLUSTERING_MAX_N);
    // Check the actual add size; CLUSTERING_MAX_N*dim would reject dim>=256 after raising MAX_N to 10M+.
    FAISS_THROW_IF_NOT_FMT(static_cast<size_t>(n) * static_cast<size_t>(intf_->d) < static_cast<size_t>(INT_MAX),
    ...
    try
    {
        for_each(transFunctorRet.begin(), transFunctorRet.end(),
                 [&](std::future<void> &ret)
                 {
                     transWait++;
                     ret.get();
    ...
    {
        // if exception occures, waiting for the rest topkFunctor to quit.
...
void AscendClusteringImpl::distributedTrain(int niter, float *centroids, const std::vector<int> &deviceList,
                                            bool clearData)
{
    ...
    APP_LOG_INFO("AscendClustering distributedTrain start: ntotal=%zu, ncentroids=%d, dim=%d, niter=%d\n", ntotal,
                 intf_->k, intf_->d, niter);
    ASCEND_THROW_IF_NOT_MSG(centroids, "distances can not be nullptr.");
    ASCEND_THROW_IF_NOT_FMT(niter > 0 && niter <= MAX_NITER, "Niter must be in range (0, %d], but given %d \n",
                            MAX_NITER, niter);
    ASCEND_THROW_IF_NOT_FMT((ntotal > 0) && (ntotal < CLUSTERING_MAX_N), "ntotal %zu must be > 0 and < %ld", ntotal,
                            CLUSTERING_MAX_N);
    ...
    auto ret = aclrtMemcpy(codesDeviceAll.data(), codesDeviceAll.getSizeInBytes(), codes.data(),
                           codes.size() * sizeof(float16_t), ACL_MEMCPY_HOST_TO_DEVICE);
    ...
    auto trainFunctor = [&](int idx)
    {
        ...
        for (int i = 0; i < niter; i++)
        {
            ...
            ASCEND_THROW_IF_NOT_FMT(ret == ACL_SUCCESS, "Failed to copy centrodata to device failed, id %d, ret %d",
                                    idx, ret);
            ...
            ASCEND_THROW_IF_NOT_FMT(ret == ACL_SUCCESS, "Failed to copy assign to host, device id %d, ret %d", idx,
                                    ret);
            count++;
            ...
            if (idx == 0)
            {
                ...
                ret = aclrtMemcpy(assignDevice.data(), assignDevice.getSizeInBytes(), assignHost.data(),
                                  assignHost.size() * sizeof(uint64_t), ACL_MEMCPY_HOST_TO_DEVICE);
                ASCEND_THROW_IF_NOT_FMT(ret == ACL_SUCCESS, "Failed to copy assignHost to device, device id %d, ret %d",
                                        idx, ret);
                ...
                ASCEND_THROW_IF_NOT_FMT(ret == ACL_SUCCESS, "update centroids compute synchronizeStream failed: %d",
                                        ret);
...
// fp32 distributed coarse clustering.
//
// Unlike distributedTrain (which relies on the fp16 IndexFlatATAicpu operators
// L2Norm / DistanceFlatL2MinsAt), this variant reuses the IndexIVFPQ training
// assignment path (DistanceFlatL2MinsAtFP32 / TopkFlat) that is always generated
// for IVFPQ, matching the single-device trainFp32 semantics while splitting the
// per-iteration assignment across every device in deviceList. Centroid update
// stays on the host (CPU), exactly like trainFp32.
void AscendClusteringImpl::distributedTrainFp32(int niter, float *centroids, const std::vector<int> &deviceList,
                                                bool clearData)
{
    APP_LOG_INFO("AscendClustering distributedTrainFp32 start: ntotal=%zu, ncentroids=%d, dim=%d, niter=%d\n", ntotal,
                 intf_->k, intf_->d, niter);
    checkParaFp32(niter, centroids);
    ASCEND_THROW_IF_NOT_FMT(ntotal >= static_cast<size_t>(intf_->k),
                            "Number of training points (%zu) should be at least as large as number of clusters (%zu)",
                            ntotal, static_cast<size_t>(intf_->k));
    ASCEND_THROW_IF_NOT_MSG(!deviceList.empty(), "deviceList can not be empty for distributed clustering.");
    ASCEND_THROW_IF_NOT_MSG(!codesFp32.empty(), "distributedTrainFp32 requires fp32 codes added via AddFp32.");

    const int d = static_cast<int>(intf_->d);
    const int k = static_cast<int>(intf_->k);

    ACL_REQUIRE_OK(aclrtSetDevice(deviceList[0]));
    AscendTensor<float, DIMS_2> centroidsHostInit(centroids, {k, d});
    randomCentrodataFp32(centroidsHostInit);
    fvec_renorm_L2(d, k, centroids);

    const int deviceCount = static_cast<int>(deviceList.size());
    std::vector<int64_t> assignHost(ntotal, 0);
    const int64_t fragN = (static_cast<int64_t>(ntotal) + deviceCount - 1) / deviceCount;
    std::atomic_int count(0);

    std::vector<std::unique_ptr<::ascend::IndexIVFPQ>> pqInstances(static_cast<size_t>(deviceCount));
    std::vector<::ascend::IndexIVFPQ *> pqHandles(static_cast<size_t>(deviceCount), nullptr);
    std::vector<std::atomic<int64_t>> iterAssignMsMax(static_cast<size_t>(niter));
    std::vector<int64_t> centroidCpuMs(static_cast<size_t>(niter), 0);
    std::vector<int64_t> centroidSyncMs(static_cast<size_t>(niter), 0);
    std::vector<int64_t> barrierMs(static_cast<size_t>(niter), 0);
    for (auto &ms : iterAssignMsMax)
    {
        ms.store(0);
    }
    ...
    {
        CommonUtils::attachToCpu(idx);
        int64_t start = static_cast<int64_t>(idx) * fragN;
        int actualSize = static_cast<int>(std::min(fragN, static_cast<int64_t>(ntotal) - start));
        ...
        if (actualSize > 0)
        {
            ...
            pqInstances[static_cast<size_t>(idx)] =
                CREATE_UNIQUE_PTR(::ascend::IndexIVFPQ, k, d, 1, 8, 1, static_cast<int64_t>(-1));
            auto *pq = pqInstances[static_cast<size_t>(idx)].get();
            pqHandles[static_cast<size_t>(idx)] = pq;
            auto ret = pq->initClusterTrainOps(k, d);
            ASCEND_THROW_IF_NOT_FMT(ret == ::ascend::APP_ERR_OK,
                                    "Failed to init cluster train ops, device id %d, ret %d", deviceList[idx], ret);

            AscendTensor<float, DIMS_2> codesDevice(mem, {actualSize, d}, stream);
            ret = aclrtMemcpy(
                codesDevice.data(), codesDevice.getSizeInBytes(), codesFp32.data() + start * static_cast<int64_t>(d),
                static_cast<size_t>(actualSize) * static_cast<size_t>(d) * sizeof(float), ACL_MEMCPY_HOST_TO_DEVICE);
            ASCEND_THROW_IF_NOT_FMT(ret == ACL_SUCCESS, "Failed to copy codes to device %d, ret %d", deviceList[idx],
                                    ret);
            ...
            for (int i = 0; i < niter; i++)
            {
                ...
                ret = pq->runKMeans(k, d, actualSize, i, centroids, codesDevice, assignHost, useDeviceCentroids, start);
                ASCEND_THROW_IF_NOT_FMT(ret == ::ascend::APP_ERR_OK, "Failed to assign shard, device id %d, ret %d",
                                        deviceList[idx], ret);
                const auto assignMs = std::chrono::duration_cast<std::chrono::milliseconds>(
                                          std::chrono::high_resolution_clock::now() - assignStart)
                                          .count();
                int64_t prev = iterAssignMsMax[static_cast<size_t>(i)].load();
                while (assignMs > prev &&
                       !iterAssignMsMax[static_cast<size_t>(i)].compare_exchange_weak(prev, assignMs))
                {
                }
                ...
                if (idx == 0)
                {
                    ...
                    fvec_renorm_L2(d, k, centroids);
                    centroidCpuMs[static_cast<size_t>(i)] = std::chrono::duration_cast<std::chrono::milliseconds>(
                                                                std::chrono::high_resolution_clock::now() - cpuStart)
                                                                .count();

                    auto syncStart = std::chrono::high_resolution_clock::now();
                    auto syncFunctor = [&](int j)
                    {
                        if (pqHandles[static_cast<size_t>(j)] == nullptr)
                        {
                            return;
                        }
                        CommonUtils::attachToCpu(j + deviceCount);
                        ACL_REQUIRE_OK(aclrtSetDevice(deviceList[j]));
                        auto syncRet = pqHandles[static_cast<size_t>(j)]->syncTrainCentroidsFromHost(k, d, centroids);
                        ASCEND_THROW_IF_NOT_FMT(syncRet == ::ascend::APP_ERR_OK,
                                                "Failed to sync train centroids to device %d, ret %d", deviceList[j],
                                                syncRet);
                    };
                    CALL_PARALLEL_FUNCTOR(static_cast<size_t>(deviceCount), pool, syncFunctor);
                    ACL_REQUIRE_OK(aclrtSetDevice(deviceList[0]));
                    centroidSyncMs[static_cast<size_t>(i)] = std::chrono::duration_cast<std::chrono::milliseconds>(
                                                                 std::chrono::high_resolution_clock::now() - syncStart)
                                                                 .count();
                    ...
                    if (intf_->verbose)
                    {
                        APP_LOG_INFO(
                            ...
                            i, (utils::getMillisecs() - t0) / 1000,
                            static_cast<long long>(iterAssignMsMax[static_cast<size_t>(i)].load()),
                            static_cast<long long>(centroidCpuMs[static_cast<size_t>(i)]),
                            static_cast<long long>(centroidSyncMs[static_cast<size_t>(i)]));
                    }
                    count = 0;
                }
                barrierMs[static_cast<size_t>(i)] += std::chrono::duration_cast<std::chrono::milliseconds>(
                                                         std::chrono::high_resolution_clock::now() - barrierStart)
                                                         .count();
                WAITING_FLAG_READY_TIMEOUT((count == 0), TIMEOUT_MS);
    ...
    if (clearData)
    {
        ...
        if (codesFp32OnDevice)
        {
            codesFp32OnDevice->clear();
            codesFp32OnDeviceCount = 0;
        }
        ntotal = 0;
    }

    APP_LOG_INFO("AscendClustering distributedTrainFp32 finished.\n");
}
...
void AscendClusteringImpl::train(int niter, float *centroids, bool clearData)
{
    ...
    FAISS_THROW_IF_NOT_FMT(niter > 0 && niter <= MAX_NITER, "Niter must be in range (0, %d], but given %d \n",
                           MAX_NITER, niter);
    FAISS_THROW_IF_NOT_FMT((ntotal > 0) && (ntotal < CLUSTERING_MAX_N), "ntotal %zu must be > 0 and < %ld", ntotal,
                           CLUSTERING_MAX_N);
    ...
    auto ret = aclrtMemcpy(codesDevice.data(), codesDevice.getSizeInBytes(), codes.data(),
                           codes.size() * sizeof(float16_t), ACL_MEMCPY_HOST_TO_DEVICE);
                                            ...
                                            float *centroids)
{
    ...
    std::vector<int32_t> hassign(k, 0);
#pragma omp parallel
    {
        const int nt = omp_get_num_threads();
        const int rank = omp_get_thread_num();

        // Each thread owns a centroid index range and scans all vectors once.
        const size_t c0 = (k * rank) / nt;
        const size_t c1 = (k * (rank + 1)) / nt;
        std::vector<int32_t> localHassign(c1 - c0, 0);
...
#pragma omp parallel for schedule(static)
    for (size_t ci = 0; ci < k; ci++)
    {
        ...
        const float norm = 1.0f / static_cast<float>(hassign[ci]);
        float *c = centroids + ci * d;
        for (size_t j = 0; j < d; j++)
        {
            c[j] *= norm;
...
void AscendClusteringImpl::normalizeVecByHost(float *centrodataHost, AscendTensor<float, DIMS_2> &centrodataDev)
{
    fvec_renorm_L2(intf_->d, intf_->k, centrodataHost);
    auto ret = aclrtMemcpy(centrodataDev.data(), centrodataDev.getSizeInBytes(), centrodataHost,
                           intf_->d * intf_->k * sizeof(float), ACL_MEMCPY_HOST_TO_DEVICE);
...
void AscendClusteringImpl::ensureCodesOnDevice()
{
    if (ntotal == 0 || codesFp32.empty())
    {
        return;
    }

    const size_t elemCount = ntotal * static_cast<size_t>(intf_->d);
    if (!codesFp32OnDevice || codesFp32OnDeviceCount < elemCount)
    {
        if (!codesFp32OnDevice)
        {
            codesFp32OnDevice = CREATE_UNIQUE_PTR(DeviceVector<float>, MemorySpace::DEVICE_HUGEPAGE);
        }
        codesFp32OnDevice->resize(elemCount);
        codesFp32OnDeviceCount = elemCount;
    }

    const size_t dataBytes = elemCount * sizeof(float);
    auto upload_start = std::chrono::high_resolution_clock::now();
    auto ret = ::ascend::aclrtMemcpyChunked(codesFp32OnDevice->data(), dataBytes, codesFp32.data(), dataBytes,
                                            ACL_MEMCPY_HOST_TO_DEVICE);
    FAISS_THROW_IF_NOT_FMT(ret == ACL_SUCCESS, "Failed to upload training codes to device, ret=%d", ret);
...
void AscendClusteringImpl::trainFp32(int niter, float *centroids, bool clearData)
{
    // Here k means num of centroids/nlist
    checkParaFp32(niter, centroids);
    FAISS_THROW_IF_NOT_MSG(!codesFp32.empty(), "trainFp32 requires fp32 codes added via AddFp32.");
    APP_LOG_INFO("AscendClustering train start: ntotal=%zu, ncentroids=%d, dim=%d, niter=%d\n", ntotal, intf_->k,
                 intf_->d, niter);
    ACL_REQUIRE_OK(aclrtSetDevice(config.deviceList[0]));
    ...
    const int d = static_cast<int>(intf_->d);
    const int k = static_cast<int>(intf_->k);
    // Reuse IndexIVFPQ L1 assignment (same path as distributedTrainFp32), not IndexIVFFlat.
    auto pq = CREATE_UNIQUE_PTR(::ascend::IndexIVFPQ, k, d, 1, 8, 1, static_cast<int64_t>(-1));
    auto ret = pq->initClusterTrainOps(k, d);
    FAISS_THROW_IF_NOT_FMT(ret == ::ascend::APP_ERR_OK, "Failed to init cluster train ops, result is %d", ret);

    auto streamPtr = resources->getDefaultStream();
    auto stream = streamPtr->GetStream();
    auto &mem = resources->getMemoryManager();

    AscendTensor<float, DIMS_2> centrodata(mem, {k, d}, stream);
    AscendTensor<float, DIMS_2> centroidsHost(centroids, {k, d});
    randomCentrodataFp32(centroidsHost);
    // 聚类中心进行归一化
    normalizeVecByHost(centroids, centrodata);
    ensureCodesOnDevice();
    AscendTensor<float, DIMS_2> codesTensor(codesFp32OnDevice->data(), {static_cast<int>(ntotal), d});
    std::vector<int64_t> assignHost(ntotal, 0);
    double tSearchTotal = 0.0;
    double t0 = 0.0;
    double t0s = 0.0;
    ...
    for (int i = 0; i < niter; i++)
    {
        ...
        ret = pq->runKMeans(k, d, static_cast<int>(ntotal), i, centroids, codesTensor, assignHost,
                            /*useDeviceCentroids=*/false, /*assignOffset=*/0);
        FAISS_THROW_IF_NOT_FMT(ret == ::ascend::APP_ERR_OK, "Failed to exec search centroids, ret %d", ret);
...
size_t AscendClusteringImpl::getNTotal() { return ntotal; }
...
void AscendClusteringImpl::corrPreProcess(float *corr)
{
    ...
    ASCEND_THROW_IF_NOT_FMT((ntotal >= intf_->k) && (ntotal < CLUSTERING_MAX_N), "ntotal must be >= %zu and < %ld",
                            intf_->k, CLUSTERING_MAX_N);
    ACL_REQUIRE_OK(aclrtSetDevice(config.deviceList[0]));
    if (!corrComputeOp)
    {
...
void AscendClusteringImpl::computeCorr(float *corr, bool clearData)
{
    ...
    int ret = aclrtMemcpy(codesShaped.data(), codesShaped.getSizeInBytes(), codesShapedHost.data(),
                          codesShapedHost.getSizeInBytes(), ACL_MEMCPY_HOST_TO_DEVICE);
    ...
    transform(corrResultHost.begin(), corrResultHost.end(), corr,
              [](float16_t temp) { return static_cast<float>(fp16(temp)); });
...
void AscendClusteringImpl::runCorrComputeOp(const AscendTensor<float16_t, DIMS_4> &codesShaped,
                                            const AscendTensor<uint64_t, DIMS_1> &actualNum,
                                            AscendTensor<float16_t, DIMS_2> &corrResult,
                                            AscendTensor<uint16_t, DIMS_2> &flag, aclrtStream stream)
{
    ASCEND_THROW_IF_NOT(corrComputeOp);

    std::shared_ptr<std::vector<const aclDataBuffer *>> corrOpInput(new std::vector<const aclDataBuffer *>(),
    ...
    std::shared_ptr<std::vector<aclDataBuffer *>> corrOpOutput(new std::vector<aclDataBuffer *>(),
...
void AscendClusteringImpl::subClusAddInt8(idx_t n, const uint8_t *x, int seq, int k)
{
    ...
    FAISS_THROW_IF_NOT_MSG(x, "x can not be nullptr.");
    FAISS_THROW_IF_NOT_FMT((n > 0) && (n < CLUSTERING_MAX_N), "n must be > 0 and < %ld", CLUSTERING_MAX_N);
...
void AscendClusteringImpl::subClusExecInt8(int niter, uint16_t *labels, float *centroids, int batchSubNlist,
                                           size_t batchNTotal)
{
                                                ...
                                                std::vector<float16_t> &centrodata)
{
    ...
    for (size_t i = 0; i < size_t(nlist); i++)
    {
        ...
        {
            initCentroids[i * static_cast<size_t>(intf_->d) + d] = static_cast<float>(
                (static_cast<float>(data[c * intf_->d + d]) + SQ_DECODE_PARAM) / UINT8_UPPER_BOUND * vDiff[d] +
                vMin[d]);
        }
    }

    transform(begin(initCentroids), end(initCentroids), begin(centrodata), [](float temp) { return fp16(temp).data; });
}
...
void AscendClusteringImpl::execClusteringInt8(int niter, uint16_t *labels, float *centroids)
{
    ...
    std::vector<std::vector<float16_t>> centrodataInit(
        NUM_BUCK_PER_BATCH, std::vector<float16_t>(intf_->d * SUBCENTER_NUM, 0));  // 初始化为0代表k-mean不参与计算
...
#pragma omp parallel for num_threads(CommonUtils::GetThreadMaxNums())
    for (size_t i = 0; i < NUM_BUCK_PER_BATCH; i++)
    {
        ...
        randomCentrodataInt8(codesInt8.data() + curCodePosAccum[i] * intf_->d, curNtotal, numCentroidsEachBuck[i],
                             centrodataInit[i]);
    }
    ...
    for (size_t i = 0; i < NUM_BUCK_PER_BATCH; i++)
    {
        ...
        trainInt8(centrodataInit[i], curCent, curNtotal, niter, SUBCENTER_NUM, codesInt8.data() + curCodePos * intf_->d,
                  labels + curCodePos, centroids + curCentPos * intf_->d);
...
void AscendClusteringImpl::trainInt8(std::vector<float16_t> &centrodata, int ncentroids, int n, int niter,
                                     int reqCentroids, const uint8_t *data, uint16_t *labels, float *subCentroids)
{
    auto streamPtr = resources->getDefaultStream();
    auto stream = streamPtr->GetStream();
    auto &mem = resources->getMemoryManager();
    ...
    int ret = aclrtMemcpy(codesDevice.data(), n * dims * sizeof(uint8_t), data, n * dims * sizeof(uint8_t),
                          ACL_MEMCPY_HOST_TO_DEVICE);
    FAISS_THROW_IF_NOT_MSG(ret == ACL_SUCCESS, "Failed to copy codes to device");
    ...
    AscendTensor<int16_t, DIMS_1> outHassign(mem,
                                             {
                                                 SUBCENTER_NUM,
                                             },
                                             stream);
    AscendTensor<uint16_t, DIMS_1> inputActualSize(mem,
                                                   {
                                                       FLAG_SIZE,
                                                   },
                                                   stream);
    ...
    ret = aclrtMemcpy(inputActualSize.data(), inputActualSize.getSizeInBytes(), inputActualSizeHost.data(),
                      inputActualSize.getSizeInBytes(), ACL_MEMCPY_HOST_TO_DEVICE);
    ...
    ret = aclrtMemcpy(centrodataDevice.data(), centrodataDevice.getSizeInBytes(), centrodata.data(),
                      centrodata.size() * sizeof(float16_t), ACL_MEMCPY_HOST_TO_DEVICE);
    ...
    for (int iter = 0; iter < niter - 1; iter++)
    {
        ...
        ASCEND_THROW_IF_NOT_FMT(ret == APP_ERR_OK, "Failed to addVectors, ret %d", ret);
        ret = npuFlatAicpu->search(n, codesDevice.data(), 1, ncentroids, labelSubClus.data(), subCentroidsCores,
                                   hassignCores, vDM);
        ASCEND_THROW_IF_NOT_FMT(ret == APP_ERR_OK, "Failed to search, ret %d", ret);

        runSubCntCompute(curSubcentroidsCores, inputHassignCores, inputActualSize, centrodataDevice, outHassign,
                         outOpFlag, stream);
        ret = synchronizeStream(stream);
    ...
    ret = aclrtMemcpy(centrodata.data(), centrodataDevice.getSizeInBytes(), centrodataDevice.data(),
                      centrodataDevice.getSizeInBytes(), ACL_MEMCPY_DEVICE_TO_HOST);
    ...
    ret = aclrtMemcpy(labels, size_t(n) * sizeof(uint16_t), labelSubClus.data(), size_t(n) * sizeof(uint16_t),
                      ACL_MEMCPY_DEVICE_TO_HOST);
    FAISS_THROW_IF_NOT_FMT(ret == ACL_SUCCESS, "Failed to copy labels back to host %i\n", ret);
}

void AscendClusteringImpl::checkParaInt8(int, uint16_t *labels, float *centroids, int batchSubNlist, size_t batchNTotal)
{
    ...
    for (int i = 0; i < NUM_BUCK_PER_BATCH; ++i)
    {
        FAISS_THROW_IF_NOT_FMT(numCentroidsEachBuck[i] <= SUBCENTER_NUM,
                               ...
                               static_cast<int>(numCentroidsEachBuck[i]));
        ...
        FAISS_THROW_IF_NOT_FMT((numCodes >= LOWER_BOUND && numCodes <= MAX_CLUS_POINTS) || (numCodes == 0),
                               ...
                               MAX_CLUS_POINTS, static_cast<int>(numCodesEachBuck[i]));
    }
}
...
APP_ERROR AscendClusteringImpl::resetSubcentsAccumOp()
{
    ...
    std::vector<int64_t> actualSizeShape({
        FLAG_SIZE,
    });
    std::vector<int64_t> subcentsShape({SUBCENTER_NUM, dims});
    std::vector<int64_t> hassignShape({
        SUBCENTER_NUM,
    });
    std::vector<int64_t> flagShape({CORE_NUM, FLAG_SIZE});
...
void AscendClusteringImpl::updateVDM(uint16_t *vmin, uint16_t *vdiff)
{
    ...
    int ret = aclrtMemcpy(dmTensor[0].data(), dims * sizeof(float16_t), vmin, dims * sizeof(uint16_t),
                          ACL_MEMCPY_HOST_TO_DEVICE);
    FAISS_THROW_IF_NOT_MSG(ret == ACL_SUCCESS, "Failed to copy vmin to dmTensor");
    ret = aclrtMemcpy(dmTensor[1].data(), dims * sizeof(float16_t), vdiff, dims * sizeof(uint16_t),
                      ACL_MEMCPY_HOST_TO_DEVICE);
                                                       ...
                                                       AscendTensor<uint64_t, DIMS_1> &assign, aclrtStream stream)
{
    ...
    std::shared_ptr<std::vector<aclDataBuffer *>> kmUpdateCentroidsOpOutput(new std::vector<aclDataBuffer *>(),
                                                                            CommonUtils::AclOutputBufferDelete);
...
void AscendClusteringImpl::randomCentrodata(AscendTensor<float16_t, DIMS_2> &centrodata)
{
    ...
    auto ret = aclrtMemcpy(centrodata.data(), centrodata.getSizeInBytes(), centrodataTmp.data(),
                           centrodataTmp.size() * sizeof(float16_t), ACL_MEMCPY_HOST_TO_DEVICE);
...
void AscendClusteringImpl::randomCentrodataFp32(AscendTensor<float, DIMS_2> &centrodata)
{
    ...
    auto ret = aclrtMemcpy(centrodata.data(), centrodata.getSizeInBytes(), centrodataTmp.data(),
                           centrodataTmp.size() * sizeof(float), ACL_MEMCPY_HOST_TO_DEVICE);
...
void AscendClusteringImpl::trainPostProcess(AscendTensor<float16_t, DIMS_2> &centrodata, float *centroids)
{
    ...
    auto ret = aclrtMemcpy(centroResult.data(), centroResult.size() * sizeof(float16_t), centrodata.data(),
                           centrodata.getSizeInBytes(), ACL_MEMCPY_DEVICE_TO_HOST);
