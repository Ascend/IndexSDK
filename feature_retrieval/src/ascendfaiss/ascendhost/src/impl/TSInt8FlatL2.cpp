...
TSInt8FlatL2::TSInt8FlatL2(uint32_t deviceId, uint32_t dim, uint32_t tokenNum, uint64_t resources,
    ...
    : TSBase(tokenNum, customAttrLen, customAttrBlockSize), IndexInt8FlatL2Aicpu(dim, resources)
{
    FAISS_THROW_IF_NOT_FMT(std::find(DIMS_INT8L2.begin(), DIMS_INT8L2.end(), dim) != DIMS_INT8L2.end(),
    ...
    {
        this->searchBatchSizes = {128, 64, 48, 36, 32, 24, 18, 16, 12, 8, 6, 4, 2, 1};
        this->setPageSize(16);
    }
...
APP_ERROR TSInt8FlatL2::addFeatureWithLabels(int64_t n, const void *features, const faiss::ascend::FeatureAttr *attrs,
...
void TSInt8FlatL2::removeIdsImpl(const std::vector<int64_t> &indices)
{
    ...
    for (int i = 0; i < removedCnt; i++)
    {
        ...
        int8_t *srcDataPtr = baseShaped[srcIdx1]->data() +
                             (srcIdx2 / zRegionHeight) * (dimAlignSize * zRegionHeight * CUBE_ALIGN_INT8) +
                             (srcIdx2 % zRegionHeight) * CUBE_ALIGN_INT8;
        int8_t *dstDataPtr = baseShaped[dstIdx1]->data() +
                             (dstIdx2 / zRegionHeight) * (dimAlignSize * zRegionHeight * CUBE_ALIGN_INT8) +
                             (dstIdx2 % zRegionHeight) * CUBE_ALIGN_INT8;
                                             ...
                                             AscendTensor<int64_t, DIMS_2> &outIndicesOnDevice)
{
    ...
    AscendTensor<float16_t, DIMS_3, size_t> distResult(
        mem, {static_cast<size_t>(blockNum), static_cast<size_t>(batch), static_cast<size_t>(codeBlockSize)}, stream);
    const size_t minDistElementCount = static_cast<size_t>(blockNum) * batch * this->burstsOfBlock;
    // AscendC DMA writes one 32-byte block for the final 16 valid bytes.
    constexpr size_t minDistDmaTailElements = 16 / sizeof(float16_t);
    AscendTensor<float16_t, DIMS_1, size_t> minDistStorage(mem, {minDistElementCount + minDistDmaTailElements}, stream);
    ...
    const bool isAscend910B = faiss::ascend::SocUtils::GetInstance().IsAscend910B();
    const int coreNum = isAscend910B ? faiss::ascend::SocUtils::GetInstance().GetCoreNum() : CORE_NUM;
    const int flagNum = isAscend910B ? faiss::ascend::SocUtils::GetInstance().GetCoreNum() : FLAG_NUM;
    uint32_t opFlagSize = static_cast<uint64_t>((blockNum * flagNum * FLAG_SIZE)) * sizeof(uint16_t);
    uint32_t attrsSize = aicpu::TOPK_FLAT_ATTR_IDX_COUNT * sizeof(int64_t);
    uint32_t opSizeLen = static_cast<uint32_t>(blockNum * coreNum * SIZE_ALIGN) * sizeof(uint32_t);
    ...
    {
        int opSizeHostIdx = i * coreNum * SIZE_ALIGN;
        ...
        if (isAscend910B)
        {
            opSizeData[opSizeHostIdx + IDX_COMP_OFFSET] = 0;
            opSizeData[opSizeHostIdx + IDX_MASK_LEN] = utils::divUp(this->codeBlockSize, MASK_ALIGN);
            opSizeData[opSizeHostIdx + IDX_USE_MASK] = 1;
            opSizeData[opSizeHostIdx + IDX_SHARE_MASK] = this->shareAttrFilter ? 1 : 0;
        }
    ...
    AscendTensor<uint32_t, DIMS_3> opSize(opSizeMem, {blockNum, coreNum, SIZE_ALIGN});
    ...
    for (int i = 0; i < blockNum; ++i)
    {
        ...
        uint32_t actualNum = opSizeData[i * coreNum * SIZE_ALIGN + IDX_ACTUAL_NUM];
...
APP_ERROR TSInt8FlatL2::searchBatchWithExtraNonshareMasks(int batch, const int8_t *features, int topK, float *distances,
                                                          int64_t *labels, AscendTensor<int32_t, DIMS_2> &queryTimes,
                                                          AscendTensor<uint8_t, DIMS_2> &tokenIds,
...
APP_ERROR TSInt8FlatL2::searchInPureDev(uint32_t count, const int8_t *features,
                                        const faiss::ascend::AttrFilter *attrFilter, uint32_t topk, int64_t *labels,
                                        float *distances)
{
    auto streamPtr = resources.getDefaultStream();
    auto stream = streamPtr->GetStream();
    ...
    if (this->shareAttrFilter)
    {
        ...
        AscendTensor<uint8_t, DIMS_2> tokenIds(
            mem, {1, utils::divUp(static_cast<int32_t>(tokenNum), OPS_DATA_TYPE_ALIGN) * OPS_DATA_TYPE_TIMES}, stream);
        ...
        for (auto batch : this->searchBatchSizes)
        {
            ...
            {
                auto ret = searchBatchWithShareMasks(batch, features + offset * static_cast<uint32_t>(this->code_size),
                                                     static_cast<int>(topk), distances + offset * topk,
                                                     labels + offset * topk, masks);
            ...
            while (queryNum >= batch)
            {
                ...
                AscendTensor<uint8_t, DIMS_2> tokenIds(
                    ...
                    {static_cast<int32_t>(batch),
                     utils::divUp(static_cast<int32_t>(tokenNum), OPS_DATA_TYPE_ALIGN) * OPS_DATA_TYPE_TIMES},
                    stream);
                buildAttr(attrFilter + offset, batch, queryTimes, tokenIds);
                auto ret = searchBatchWithNonshareMasks(
                    batch, features + offset * static_cast<uint32_t>(this->code_size), static_cast<int>(topk),
...
APP_ERROR TSInt8FlatL2::search(uint32_t count, const void *features, const faiss::ascend::AttrFilter *attrFilter,
                               ...
                               uint32_t *validNums, bool enableTimeFilter, const faiss::ascend::ExtraValFilter *)
{
...
APP_ERROR TSInt8FlatL2::searchInPureDevWithExtraMask(uint32_t count, const int8_t *features,
                                                     const faiss::ascend::AttrFilter *attrFilter, uint32_t topk,
                                                     const uint8_t *extraMask, int64_t *labels, float *distances)
{
    auto streamPtr = resources.getDefaultStream();
    auto stream = streamPtr->GetStream();
    auto &mem = resources.getMemoryManager();
    ...
    for (auto batch : this->searchBatchSizes)
    {
        while (queryNum >= batch)
        {
            ...
            AscendTensor<uint8_t, DIMS_2> tokenIds(
                ...
                stream);
            buildAttr(attrFilter + offset, batch, queryTimes, tokenIds);
            auto ret = searchBatchWithExtraNonshareMasks(
                batch, features + offset * static_cast<uint32_t>(this->code_size), static_cast<int>(topk),
                distances + offset * topk, labels + offset * topk, queryTimes, tokenIds,
                                            ...
                                            uint32_t *validNums, bool enableTimeFilter, const float16_t *)
{
    ...
    ret = searchInPureDevWithExtraMask(count, tensorDevQueries.data(), attrFilter, topk, extraMask, labels, distances);
                               ...
                               AscendTensor<int64_t, DIMS_2> &outIndicesOnDevice, float *distances, int64_t *labels)
{
    ...
    for (uint64_t i = 0; i < searchNum; ++i)
    {
        std::transform(outDistances.data() + i * static_cast<uint64_t>(topK),
                       outDistances.data() + (i + 1) * static_cast<uint64_t>(topK),
                       distances + i * static_cast<uint64_t>(topK), [&](const float16_t temp) -> float
                       { return std::sqrt(static_cast<float>(faiss::ascend::fp16(temp)) / scale); });

        std::transform(labels + i * static_cast<uint64_t>(topK), labels + (i + 1) * static_cast<uint64_t>(topK),
                       labels + i * static_cast<uint64_t>(topK), [&](const idx_t temp) -> int64_t
                                        ...
                                        const std::vector<const AscendTensorBase *> &output, aclrtStream stream) const
{
    if (faiss::ascend::SocUtils::GetInstance().IsAscend910B())
    {
        ...
        IndexTypeIdx indexType = shareMask ? IndexTypeIdx::ITI_INT8_L2_SHARE_MASK : IndexTypeIdx::ITI_INT8_L2;
...
APP_ERROR TSInt8FlatL2::resetInt8L2DistCompute(int codeNum, bool shareMask) const
{
    if (faiss::ascend::SocUtils::GetInstance().IsAscend910B())
    {
        const std::string opTypeName = "AscendcDistInt8FlatL2";
        const IndexTypeIdx indexMaskType = shareMask ? IndexTypeIdx::ITI_INT8_L2_SHARE_MASK : IndexTypeIdx::ITI_INT8_L2;
        const int flagNum = faiss::ascend::SocUtils::GetInstance().GetCoreNum();
        for (auto batch : searchBatchSizes)
        {
            std::vector<int64_t> queryShape({batch, dims});
            std::vector<int64_t> maskShape({shareMask ? 1 : batch, utils::divUp(codeNum, 8)});
            ...
            std::vector<int64_t> sizeShape({flagNum, SIZE_ALIGN});
