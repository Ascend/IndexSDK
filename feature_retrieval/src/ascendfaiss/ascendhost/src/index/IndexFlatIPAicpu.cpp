                                       ...
                                       AscendTensor<idx_t, DIMS_2> &outIndices)
{
    ...
    auto ret = aclrtMemcpy(outDistances.data(), outDistances.getSizeInBytes(), maxDistances.data(),
    ...
    ret = aclrtMemcpy(outIndices.data(), outIndices.getSizeInBytes(), maxIndices.data(), maxIndices.getSizeInBytes(),
                                        ...
                                        AscendTensor<int64_t, DIMS_2> &maxIndices)
{
    ...
    AscendTensor<float16_t, DIMS_3, size_t> distResult(
        mem, {static_cast<size_t>(blockNum), static_cast<size_t>(nq), static_cast<size_t>(blockSize)}, stream);
    AscendTensor<float16_t, DIMS_3, size_t> maxDistResult(
        mem, {static_cast<size_t>(blockNum), static_cast<size_t>(nq), static_cast<size_t>(curBurstsOfBlock)}, stream);
...
void IndexFlatIPAicpu::runTopkCompute(AscendTensor<float16_t, DIMS_3> &dists, AscendTensor<float16_t, DIMS_3> &maxdists,
...
APP_ERROR IndexFlatIPAicpu::searchImpl(std::vector<Index *> indexes, int n, int batchSize, const float16_t *x, int k,
                                       float16_t *distances, idx_t *labels)
{
    ...
    AscendTensor<float16_t, DIMS_3, size_t> distResult(
        mem, {static_cast<size_t>(PAGE_BLOCKS), static_cast<size_t>(batchSize), static_cast<size_t>(blockSize)},
        stream);
    AscendTensor<float16_t, DIMS_3, size_t> maxDistResult(
        mem, {static_cast<size_t>(PAGE_BLOCKS), static_cast<size_t>(batchSize), static_cast<size_t>(curBurstsOfBlock)},
        stream);
    ...
    for (size_t indexId = 0; indexId < indexSize; ++indexId)
    {
        ...
        for (int i = 0; i < blocks; ++i)
        {
            ...
            if (blockIdx == 0 && (offsetBlocks[indexId] + static_cast<idx_t>(i)) > 0)
            {
                ...
                {
                    ret = aclrtMemcpy(
                        distances + static_cast<size_t>(j) * static_cast<size_t>(n) * static_cast<size_t>(k),
                        static_cast<size_t>(batchSize) * static_cast<size_t>(k) * sizeof(float16_t),
                        maxDistances[j].data(),
                        static_cast<size_t>(batchSize) * static_cast<size_t>(k) * sizeof(float16_t),
                        ACL_MEMCPY_DEVICE_TO_HOST);
                    APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "Mem operator error %d", ret);
                    ret = aclrtMemcpy(labels + static_cast<size_t>(j) * static_cast<size_t>(n) * static_cast<size_t>(k),
            ...
            if (blockIdx == 0)
            {
                ...
                topkFlatIpMultisearchParams params(batchSize, distResult, maxDistResult, opSize, flag, attrsInputs,
                                                   indexOffset, labelOffset, reorderFlag, maxDistances, maxIndices);
                                                ...
                                                aclrtStream streamAicpu)
{
    ...
    auto ret =
        aclopExecuteV2("TopkMultisearch", desc.inputDesc.size(), const_cast<aclTensorDesc **>(desc.inputDesc.data()),
                       topkOpInput.data(), desc.outputDesc.size(), const_cast<aclTensorDesc **>(desc.outputDesc.data()),
...
void IndexFlatIPAicpu::runDistCompute(int batch, const std::vector<const AscendTensorBase *> &input,
                                      const std::vector<const AscendTensorBase *> &output, aclrtStream stream)
{
    ...
    else
    {
        ...
        aclopAttr *opAttr = aclopCreateAttr();
        auto ret = aclopExecuteV2(opType, inputDesc.size(), const_cast<aclTensorDesc **>(inputDesc.data()),
                                  distOpInput.data(), outputDesc.size(),
                                  const_cast<aclTensorDesc **>(outputDesc.data()), distOpOutput.data(), opAttr, stream);
        aclopDestroyAttr(opAttr);
                                              ...
                                              std::vector<std::pair<aclDataType, std::vector<int64_t>>> &output)
{
    ...
    {
        aclTensorDesc *desc =
            aclCreateTensorDesc(input[i].first, input[i].second.size(), input[i].second.data(), ACL_FORMAT_ND);
        if (desc == nullptr)
        {
    ...
    {
        aclTensorDesc *desc =
            aclCreateTensorDesc(output[i].first, output[i].second.size(), output[i].second.data(), ACL_FORMAT_ND);
        if (desc == nullptr)
        {
            return APP_ERR_INNER_ERROR;
        }
    ...
    ret = aclopCompile(opType, numInputs, disIPOpInputDesc[batch].data(), numOutputs, disIPOpOutputDesc[batch].data(),
...
APP_ERROR IndexFlatIPAicpu::resetDistCompOp(int numLists)
{
    ...
    for (auto batch : searchBatchSizes)
    {
        ...
        std::vector<std::pair<aclDataType, std::vector<int64_t>>> input{{ACL_FLOAT16, queryShape},
                                                                        {ACL_UINT8, maskShape},
