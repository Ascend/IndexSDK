                                                  ...
                                                  AscendTensor<float16_t, DIMS_1> &precompData)
{
    ...
    while (mantainLen > 0)
    {
        ...
        auto ret = aclrtMemcpy(this->normBase.at(normVecId)->data() + noremVecOffset, cpyNum * sizeof(float16_t),
                               precompData.data() + precompDataOffset, cpyNum * sizeof(float16_t),
...
APP_ERROR IndexInt8FlatCosAicpu::addVectors(AscendTensor<int8_t, DIMS_2> &rawData)
{
    ...
    APPERR_RETURN_IF_NOT_FMT(APP_ERR_OK == l2Ret, APP_ERR_INNER_ERROR, "calculate L2norm for rawData failed! %d",
                             l2Ret);
    ...
    if (lastOffset != 0)
    {
        ...
        auto ret =
            aclrtMemcpy(this->normBase.at(vecSize - 1)->data() + lastOffset, this->codeBlockSize * sizeof(float16_t),
                                               ...
                                               AscendTensor<uint8_t, DIMS_2> &mask)
{
    ...
    runTopkCompute(tensorCond.getDistResult(grpId), tensorCond.getMinDistResult(grpId), *tensorCond.opSizeVec.at(grpId),
                   *tensorCond.opFlagVec.at(grpId), *tensorCond.attrsInputVec.at(grpId), ctx.minDistances,
                   ctx.minIndices, streamAicpuPtr->GetStream());
                                                               ...
                                                               IndexSearchContext &ctx, IndexSearchTensorShare &outCond)
{
    ...
    for (size_t grpId = 0; grpId < grpSpliter.Count(); ++grpId)
    {
        ...
        auto ret =
            aclrtMemcpy(outCond.attrsInputVec.at(grpId)->data(), outCond.attrsInputVec.at(grpId)->getSizeInBytes(),
        ...
        for (size_t i = 0; i < blockNum; i++)
        {
            ...
            opSizeHost[opSizeHostIdx + IDX_COMP_OFFSET] = (uint32_t)(grp.blocks.at(i).blockId * this->codeBlockSize);
            opSizeHost[opSizeHostIdx + IDX_MASK_LEN] = (size_t)idxMaskLen;
            opSizeHost[opSizeHostIdx + IDX_USE_MASK] = (size_t)idxUseMask;
            opSizeHost[opSizeHostIdx + IDX_CODE_SIZE] =
                this->baseShaped[grp.blocks.at(i).blockId]->size() / static_cast<size_t>(this->dims);
                                                 ...
                                                 AscendTensor<uint8_t, DIMS_2> &retMask)
{
    ...
    if (this->maskData != nullptr && this->maskOnDevice)
    {
        ...
        retMask =
            std::move(AscendTensor<uint8_t, DIMS_2>(this->maskData, {n, std::max(idxMaskLen, this->blockMaskSize)}));
        return APP_ERR_OK;
    }
    // operator has verification, mask must be blockMaskSize at least.
                                             ...
                                             AscendTensor<float16_t, DIMS_1> &queriesNorm)
{
    ...
    for (int i = 0; i < blockNum; ++i)
    {
        ...
        std::vector<const AscendTensorBase *> input{&queries, &mask, &shaped, &queriesNorm, &codesNorm, &actualSize};
        if (deviceMemMng.GetStrategy() == DevMemStrategy::HETERO_MEM)
        {
            input.push_back(&topKThreshold);
        }
...
APP_ERROR IndexInt8FlatCosAicpu::resetDistCompOp(int codeNum) const
{
    ...
    IndexTypeIdx indexType = deviceMemMng.GetStrategy() == DevMemStrategy::HETERO_MEM
                                 ? IndexTypeIdx::ITI_INT8_COS_FILTER
                                 : IndexTypeIdx::ITI_INT8_COS;
...
void IndexInt8FlatCosAicpu::runDistCompute(int batch, const std::vector<const AscendTensorBase *> &input,
                                           const std::vector<const AscendTensorBase *> &output, aclrtStream stream,
                                           uint32_t actualNum) const
{
    VALUE_UNUSED(actualNum);
    IndexTypeIdx type = deviceMemMng.GetStrategy() == DevMemStrategy::HETERO_MEM ? IndexTypeIdx::ITI_INT8_COS_FILTER
                                                                                 : IndexTypeIdx::ITI_INT8_COS;
    std::vector<int> keys({batch, dims, codeBlockSize});
    OpsMngKey opsKey(keys);
...
APP_ERROR IndexInt8FlatCosAicpu::getThreshold(AscendTensor<float16_t, DIMS_2> &topKThreshold, int nq, int k, int pageId,
                                              const AscendTensor<float16_t, DIMS_2> &minDistances) const
{
    ...
    if (pageId != 0)
    {
        auto ret = aclrtMemcpy(minDistancesHost.data(), nq * k * sizeof(float16_t), minDistances.data(),
                               minDistances.getSizeInBytes(), ACL_MEMCPY_DEVICE_TO_HOST);
        APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "Mem operator error %d", ret);
    }
    ...
    for (int i = 0; i < nq; ++i)
    {  // 遍历各个查询向量
        std::fill_n(topKThresholdHost.begin() + i * numThresholds, numThresholds, minDistancesHost[i * k]);
    }
