...
APP_ERROR IndexFlatL2Aicpu::searchImpl(std::vector<Index *> indexes, int n, int batchSize, const float16_t *x, int k,
    ...
    for (int indexId = 0; indexId < indexSize; ++indexId)
    {
        ...
        for (int i = 0; i < blocks; ++i)
        {
            ...
            if (blockIdx == 0)
            {
                ...
                topkMultisearchParams params =
                    topkMultisearchParams(batchSize, &distResult, &minDistResult, &opSize, &flag, &attrsInputs,
                                          &indexOffset, &labelOffset, &reorderFlag, &minDistances, &minIndices);
...
APP_ERROR IndexFlatL2Aicpu::tryToSychResultAdvanced(int &hasCopiedCount, int &indexDoneCount, int indexId, int n,
                                                    ...
                                                    AscendTensor<idx_t, DIMS_3, size_t> &minIndices)
{
    for (int j = hasCopiedCount; j < indexDoneCount; ++j)
    {
        ...
        auto ret = aclrtMemcpy(distances + totalIndex, totalBatchSize * sizeof(float16_t), minDistances[j].data(),
                               totalBatchSize * sizeof(float16_t), ACL_MEMCPY_DEVICE_TO_HOST);
        APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "Mem operator error %d", ret);

        ret = aclrtMemcpy(labels + totalIndex, totalBatchSize * sizeof(idx_t), minIndices[j].data(),
                          totalBatchSize * sizeof(idx_t), ACL_MEMCPY_DEVICE_TO_HOST);
        APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "Mem operator error %d", ret);
        ++hasCopiedCount;
    }
...
void IndexFlatL2Aicpu::moveVectorForward(idx_t srcIdx, idx_t dstIdx)
{
    ...
    auto err = aclrtMemcpy(normBase[dstIdx1]->data() + dstIdx2, sizeof(float16_t), normBase[srcIdx1]->data() + srcIdx2,
                           sizeof(float16_t), ACL_MEMCPY_DEVICE_TO_DEVICE);
...
void IndexFlatL2Aicpu::runDistCompute(int batch, const std::vector<const AscendTensorBase *> &input,
                                              ...
                                              std::vector<std::pair<aclDataType, std::vector<int64_t>>> &output)
{
    ...
    ret = aclopCompile(opType, numInputs, disL2OpInputDesc[batch].data(), numOutputs, disL2OpOutputDesc[batch].data(),
