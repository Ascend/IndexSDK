#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
-------------------------------------------------------------------------
This file is part of the MindStudio project.
Copyright (c) 2025 Huawei Technologies Co.,Ltd.

MindStudio is licensed under Mulan PSL v2.
You can use this software according to the terms and conditions of the Mulan PSL v2.
You may obtain a copy of Mulan PSL v2 at:

         http://license.coscl.org.cn/MulanPSL2

THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND,
EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT,
MERCHANTABILITY OR FIT FOR A PARTICULAR PURPOSE.
See the Mulan PSL v2 for more details.
-------------------------------------------------------------------------
"""
import os
import argparse
import common as utils
from common import OpJsonGenerator

# number of code per time in distance_compute_flat_min64
_QUERY_NUM = 1024


def arg_parse():
    """
    Parse arguements to the operator model
    """

    parser = argparse.ArgumentParser(
        description='generate aicore operator model')

    utils.op_common_parse(parser, "--cores", 'core_num', 2, int, "Core number")
    utils.op_common_parse(parser, "-d", 'dim', 64, int, "Feature dimension")
    utils.op_common_parse(parser, "-c", 'code_num', 8192, int, "Number vector of base")
    utils.op_common_parse(parser, "-p", 'process_id', 0, int, "Number of process_id")
    utils.op_common_parse(parser, "-t", 'npu_type', "310", str, "NPU type, 310 or 310P. 310 by default")

    return parser.parse_args()


def generate_l2_norm_at_json(core_num, page_num, dim, file_path):
    # write dist_compute_flat_mins json
    l2_norm_obj = []
    generator = OpJsonGenerator("L2Norm")
    generator.add_input("ND", [page_num, dim], "float16")
    generator.add_input("ND", [256, 16], "float32")
    generator.add_input("ND", [core_num, 8], "uint32")
    generator.add_output("ND", [page_num], "float32")
    generator.add_output("ND", [page_num // 32, dim // 16, 32, 16], "float16")
    l2_norm_obj.append(generator.generate_obj())
    utils.generate_op_config(l2_norm_obj, file_path)


def generate_distance_flat_l2_mins_at_json(core_num, query_num, code_num, dim, file_path):
    # write dist_compute_flat_mins json
    dist_flat_mins_obj = []
    generator = OpJsonGenerator("DistanceFlatL2MinsAt")
    generator.add_input("ND", [query_num // 32, dim // 16, 32, 16], "float16")
    generator.add_input("ND", [code_num // 512, dim // 16, 512, 16], "float16")
    generator.add_input("ND", [query_num], "float32")
    generator.add_input("ND", [code_num // 512, 1, 512, 16], "float32")
    generator.add_output("ND", [query_num, code_num], "float16")
    generator.add_output("ND", [query_num, code_num // 64 * 2], "float16")
    generator.add_output("ND", [core_num, 16], "uint16")
    dist_flat_mins_obj.append(generator.generate_obj())
    utils.generate_op_config(dist_flat_mins_obj, file_path)


def generate_corr_compute_json(core_nums, dim, file_path):
    code_num = 1024
    corr_compute_obj = []
    generator = OpJsonGenerator("CorrCompute")
    generator.add_input("ND", [1, code_num // 16, dim, 16], "float16")
    generator.add_input("ND", [core_nums, ], "uint64")
    generator.add_output("ND", [dim, dim], "float16")
    generator.add_output("ND", [core_nums, 16], "uint16")
    corr_compute_obj.append(generator.generate_obj())
    utils.generate_op_config(corr_compute_obj, file_path)


def generate_flat_offline_model():
    utils.set_env()
    args = arg_parse()
    process_id = args.process_id
    dim = args.dim
    utils.check_param_range(dim, [64, 128, 256], "dim")

    code_num = args.code_num
    utils.check_param_range(code_num, [1024, 2048, 4096, 8192, 16384, 32768], "code_num")
    
    soc_version = utils.get_soc_version_from_npu_type(args.npu_type)
    core_num = utils.get_core_num_by_npu_type(args.core_num, args.npu_type)
    work_dir = '.'
    try:
        page_num = 32768 * 8192 / code_num
    except ZeroDivisionError as e:
        raise Exception("Code_num can not be zero!") from e
    config_path = utils.get_config_path(work_dir)

    op_name_ = f"l2_norm_op_pid{process_id}"
    file_path_ = os.path.join(config_path, f"{op_name_}.json")
    generate_l2_norm_at_json(core_num, page_num, dim, file_path_)
    utils.atc_model(op_name_, soc_version)

    op_name_ = f"flat_l2_mins_at_op_pid{process_id}"
    file_path_ = os.path.join(config_path, f"{op_name_}.json")
    generate_distance_flat_l2_mins_at_json(core_num, _QUERY_NUM, code_num, dim, file_path_)
    utils.atc_model(op_name_, soc_version)

    op_name_ = "corr_compute_op{}_pid{}".format(dim, process_id)
    file_path_ = os.path.join(config_path, '{}.json'.format(op_name_))
    generate_corr_compute_json(core_num, dim, file_path_)
    utils.atc_model(op_name_, soc_version)


if __name__ == '__main__':
    generate_flat_offline_model()
