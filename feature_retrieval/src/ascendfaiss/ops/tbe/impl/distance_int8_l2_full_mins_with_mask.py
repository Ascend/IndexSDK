...
QueriesPara = collections.namedtuple('QueriesPara', ['queries_l1', 'queries_offset', 'queries_num', \
                                                     'queries_square_ub'])


class DistanceInt8L2FullMinsWithMask:
    ...
    def cube_compute_each_loop(self, queries_para, centroids_offset, centroids_offset_in_loop):
        """
        compute centroids of each loop
        """
        queries_l1, queries_offset, queries_num, queries_square_ub = queries_para
            ...
            with self.tik_instance.for_range(0, queries_num) as j:
                ...
                if self.share_distance_mask is not True:
                    self.tik_instance.data_move(sel_ub[j, 0],
                                                self.input_mask_gm[(j + queries_offset) * self.block_mask_len +
                                                                (centroids_offset + centroids_offset_in_loop) // 8],
                                                0, 1, (self.centroids_num_each_loop + 255) // 256, 8, 8)
                else:
                    self.tik_instance.data_move(sel_ub[j, 0],
                                                self.input_mask_gm[(centroids_offset + centroids_offset_in_loop) // 8],
                                                0, 1, (self.centroids_num_each_loop + 255) // 256, 8, 8)
    ...
    def distance_int8_l2_each_loop(self, centroids_offset, centroids_num, queries_offset, queries_num):
        ...
        if self.use_fp16 is True:
            ...
            queries_para = QueriesPara(queries_l1, queries_offset, queries_num, queries_square_ub)

            with self.tik_instance.for_range(0, centroids_num // self.centroids_num_each_loop) as loop_centroids:
                self.cube_compute_each_loop(queries_para, centroids_offset,
                                            loop_centroids * self.centroids_num_each_loop)
    ...
    def get_tik_instance(self):
        ...
        self.tik_instance.BuildCCE(kernel_name=self.kernel_name,
                                   inputs=[self.input_queries_gm, self.input_mask_gm,
                                           self.input_centroids_gm, self.input_precomputed_gm,
                                           self.input_actual_num_gm],
                                   outputs=[self.output_dist_gm, self.output_min_dist_gm, self.output_flag_gm])
        return self.tik_instance


def distance_int8_l2_full_mins_with_mask(input_queries, input_mask, input_centroids,
                          input_precomputed, input_actual_num,
                          output_dist, output_min_dist, output_flag,
                          kernel_name="distance_int8_l2_full_mins_with_mask"):

    distance_l2_full_mins_with_mask = DistanceInt8L2FullMinsWithMask(input_queries, input_mask,
                                                input_centroids, input_precomputed, input_actual_num,
                                                output_dist, output_min_dist, output_flag, False,
                                                kernel_name)
    tik_instance = distance_l2_full_mins_with_mask.get_tik_instance()
