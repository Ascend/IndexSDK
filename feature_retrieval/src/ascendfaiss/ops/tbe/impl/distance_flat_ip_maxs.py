...
class DistanceFlatIPMaxs:
    def __init__(self,
        ...
        self.code_out_div_num = 1 if self.dim >= 2048 else 2
    ...
    def set_tik_instance(self):
        ...
        # dim为2048, code_num_each_loop为512
        # dim为{32,64,128}, nq为{1,2,4,8,16},code_num_each_loop为2048
        # other code_num_each_loop为1024
        self.queries_num_each_loop = min(self.queries_num, 48)
        code_num_base = 512 if self.dim >= 2048 else 1024
        self.code_num_each_loop = min(48 // 16 // ((self.queries_num_each_loop + 15) // 16) * code_num_base,
    ...
    def cube_compute_each_loop(self, queries_l1, aicore_move_offset, code_move_offset, code_move_num,
        ...
        # dim为2048, code_out_num为512; other 512 or 1024
        code_out_num = self.code_num_each_loop // self.code_out_div_num

        if flag == 0:
            with self.tik_instance.for_range(0, self.code_out_div_num, thread_num=1) as i:
...
def distance_flat_ip_maxs(input_queries,
    ...
    None
    """
    distance_flat = DistanceFlatIPMaxs(input_queries, input_mask, input_code, input_actual_num,
