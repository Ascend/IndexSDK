...
#include "transdata_raw_cpu_kernel.h"
...
namespace {
const char* TRANSDATA_RAW = "TransdataRaw";
}

namespace aicpu {
uint32_t TransdataRawCpuKernel::Compute(CpuKernelContext &ctx)
{
    Inputs inputs;
    Outputs outputs;
...
uint32_t TransdataRawCpuKernel::GetInOutAndCheck(const CpuKernelContext &ctx, Inputs &inputs, Outputs &outputs) const
{
    KERNEL_LOG_INFO("TransdataRawCpuKernel GetInOutAndCheck begin");
...
uint32_t TransdataRawCpuKernel::CheckInOutShapes(const Inputs &inputs, const Outputs &outputs)
{
    KERNEL_LOG_INFO("TransdataRawCpuKernel CheckInputShapes begin");
    ...
    KERNEL_CHECK_TRUE(attrCount == TRANSDATA_RAW_ATTR_IDX_COUNT,
        KERNEL_STATUS_PARAM_INVALID, "Num of attrs must be %d", TRANSDATA_RAW_ATTR_IDX_COUNT);

    auto attr = static_cast<int64_t *>(inputs.attrs->GetData());
    offset_ = *(attr + TRANSDATA_RAW_ATTR_OFFSET_IDX);

    return KERNEL_STATUS_OK;
}

template<typename T>
void TransdataRawCpuKernel::DoCompute(size_t start, size_t end, const Inputs &inputs, Outputs &outputs)
{
...
REGISTER_CPU_KERNEL(TRANSDATA_RAW, TransdataRawCpuKernel);
} // namespace aicpu