...
#include "removedata_attr_cpu_kernel.h"
...
namespace {
const char* REMOVEDATA_ATTR = "RemovedataAttr";
}

namespace aicpu {
uint32_t RemovedataAttrCpuKernel::Compute(CpuKernelContext &ctx)
{
    Inputs inputs;
    Outputs outputs;
    ...
    auto computeFunc = [&](size_t start, size_t end) {
        switch (dataType_) {
            case faiss::ascend::UINT8:
                ...
                break;
            case faiss::ascend::INT32:
                DoCompute<int32_t>(start, end, inputs, outputs);
                break;
            case faiss::ascend::FLOAT16:
                DoCompute<float16_t>(start, end, inputs, outputs);
                break;
            case faiss::ascend::INT16:
                DoCompute<int16_t>(start, end, inputs, outputs);
                break;
            default:
...
uint32_t RemovedataAttrCpuKernel::GetInOutAndCheck(const CpuKernelContext &ctx, Inputs &inputs, Outputs &outputs) const
{
    KERNEL_LOG_INFO("RemoveAttrShapedCpuKernel GetInOutAndCheck begin");
...
uint32_t RemovedataAttrCpuKernel::CheckInOutShapes(const Inputs &inputs, const Outputs &outputs)
{
    ...
    KERNEL_CHECK_TRUE(attrCount == REMOVEDATA_ATTR_IDX_COUNT,
        KERNEL_STATUS_PARAM_INVALID, "Num of attrs must be %d", REMOVEDATA_ATTR_IDX_COUNT);

    auto attr = static_cast<int64_t *>(inputs.attrs->GetData());
    dataType_ = *(attr + REMOVEDATA_ATTR_DATA_TYPE);
    copyNums_ = *(attr + REMOVEDATA_ATTR_COPY_NUM);

    return KERNEL_STATUS_OK;
}

template <typename T>
void RemovedataAttrCpuKernel::DoCompute(size_t start, size_t end, const Inputs &inputs, const Outputs &outputs)
{
    ...
    for (size_t i = start; i < end; ++i) {
        ...
        auto err = memcpy_s(dstDataPtr, sizeof(T) * copyNums_, srcDataPtr, sizeof(T) * copyNums_);
        if (err != EOK) {
...
REGISTER_CPU_KERNEL(REMOVEDATA_ATTR, RemovedataAttrCpuKernel);
} // namespace aicpu
