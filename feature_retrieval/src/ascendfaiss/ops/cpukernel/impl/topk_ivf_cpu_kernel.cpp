...
namespace {
const char* TOPK_IVF = "TopkIvf";
const uint32_t THREAD_CNT = 6;
}

namespace aicpu {
uint32_t TopkIvfCpuKernel::Compute(CpuKernelContext &ctx)
{
    Inputs inputs;
    Outputs outputs;
...
uint32_t TopkIvfCpuKernel::GetInOutAndCheck(const CpuKernelContext &ctx, Inputs &inputs, Outputs &outputs) const
{
...
uint32_t TopkIvfCpuKernel::CheckInputShapes(const Inputs &inputs)
{
...
void TopkIvfCpuKernel::UpdateInOutShape(Inputs &inputs, Outputs &outputs) const
{
...
void TopkIvfCpuKernel::InitTopkHeap(Outputs &outputs) const
{
...
template <typename C>
void TopkIvfCpuKernel::DoCompute(size_t tcnt, size_t tid, const Inputs &inputs, Outputs &outputs, C &&cmp)
{
    ...
    for (int64_t qidx = tid; qidx < nq_; qidx += tcnt) {
        for (int64_t bidx = 0; bidx < blockNum_; ++bidx) {
            ...
            for (int64_t hidx = 0; hidx < handleBatch_; ++hidx) {
                bool reorder = (bidx + 1 == blockNum_ && hidx + 1 == handleBatch_);
...
template <typename C>
void TopkIvfCpuKernel::ComputeQuery(int64_t qidx,
                                    int64_t bidx,
                                    int64_t hidx,
...
REGISTER_CPU_KERNEL(TOPK_IVF, TopkIvfCpuKernel);
} // namespace aicpu