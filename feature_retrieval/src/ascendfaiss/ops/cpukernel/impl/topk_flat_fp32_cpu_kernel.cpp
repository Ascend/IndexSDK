...
#include "topk_flat_fp32_cpu_kernel.h"
...
namespace {
const char *TOPK_FLAT_FP32 = "TopkFlatFp32";
}

namespace aicpu {
uint32_t TopkFlatCpuKernelFp32::GetAndCheckInOut(CpuKernelContext &ctx, Inputs &inputs, Outputs &outputs)
{
...
uint32_t TopkFlatCpuKernelFp32::Compute(CpuKernelContext &ctx)
{
    Inputs inputs;
    Outputs outputs;
    
    GetAndCheckInOut(ctx, inputs, outputs);
    ...
    auto funcLess = [](const float a, const float b) -> bool { return a < b; };
    auto funcGreater = [](const float a, const float b) -> bool { return a > b; };
...
uint32_t TopkFlatCpuKernelFp32::GetInOutAndCheck(const CpuKernelContext &ctx, Inputs &inputs, Outputs &outputs) const
{
    KERNEL_LOG_INFO("TopkFlatCpuKernelFp32 GetInOutAndCheck begin");
...
uint32_t TopkFlatCpuKernelFp32::CheckInputShapes(const Inputs &inputs)
{
    KERNEL_LOG_INFO("TopkFlatCpuKernelFp32 CheckInputShapes begin");
...
void TopkFlatCpuKernelFp32::UpdateOutputsShape(Outputs &outputs)
{
    KERNEL_LOG_INFO("TopkFlatCpuKernelFp32 UpdateOutputsShape begin");
...
void TopkFlatCpuKernelFp32::ReorderLastBlock(float *outdists, T *outlabel, C &&cmp)
{
...
template <typename T, typename C>
void TopkFlatCpuKernelFp32::DoCompute(size_t start, size_t end, const Inputs &inputs, Outputs &outputs, C &&cmp)
{
...
template<typename T>
void TopkFlatCpuKernelFp32::InitTopkHeap(Outputs &outputs) const
{
...
template <typename T, typename C>
void TopkFlatCpuKernelFp32::UpdateHeapByPos(int64_t outdisPos, float *outdists, int64_t indisPos, float *indists,
...
template <typename T, typename C>
void TopkFlatCpuKernelFp32::UpdateHeapByBurst(size_t n, int64_t blockIdx, int64_t burstSize, int64_t totalBaseoffset,
                                              ...
                                              KernelTensor<float> &indistsTensor, C &&cmp)
{
    ...
    if (blockIdx + 1 == blockNum_) {
        ...
        std::vector<std::pair<float, int64_t>> topkBurstIdx;
        ...
        std::sort(topkBurstIdx.begin(), topkBurstIdx.end(),
            [&cmp](const std::pair<float, int64_t> p1, const std::pair<float, int64_t> p2) -> bool {
        ...
        for (size_t i = 0; i < topkBurstIdx.size(); ++i) {
            ...
            float *indistsSrc = indistsTensor.GetSubTensorDim1(currentPostion.first, n);
            ...
            for (int64_t j = currentIdx; j < currentIdx + burstLen_; ++j) {
                UpdateHeapByPos(0, outdists, j, indistsSrc, 0, static_cast<T>(pageOffset + blockOffset + j),
                                outlabel, 0, cmp);
            }
        }
    }
...
template <typename T, typename C>
void TopkFlatCpuKernelFp32::ComputeBlock(size_t n, int64_t blockIdx, KernelTensor<float> &indistsTensor,
    KernelTensor<float> &vmdistsTensor, KernelTensor<uint32_t> &sizeTensor, KernelTensor<float> &outdistsTensor,
    KernelTensor<T> &outlabelsTensor, bool reorder, C &&cmp)
{
    float *indists = indistsTensor.GetSubTensorDim1(blockIdx, n);
    float *vmdists = vmdistsTensor.GetSubTensorDim1(blockIdx, n);
    uint32_t *vmlabel = reinterpret_cast<uint32_t *>(vmdists);
    uint32_t *size = sizeTensor.GetSubTensorDim0(blockIdx);
    float *outdists = outdistsTensor.GetSubTensorDim0(n);
        ...
        for (int64_t i = burstIdx; i < burstSize; ++i) {
            ...
            for (int64_t j = 0; j < burstLen_ && idx < ntotal; ++j, ++idx) {
                UpdateHeapByPos(0, outdists, idx, indists, 0,
                                static_cast<T>(baseOffset + idx + pageIdx_ * pageSize_), outlabel, 0, cmp);
            }
        }
    } else {
        UpdateHeapByBurst(n, blockIdx, burstSize, totalBaseoffset, outdists,
                          vmdists, outlabel, vmlabel, indistsTensor, cmp);
...
REGISTER_CPU_KERNEL(TOPK_FLAT_FP32, TopkFlatCpuKernelFp32);
} // namespace aicpu