...
#include "vec_l2sqr_cpu_kernel.h"
...
namespace {
const char* VEC_L2SQR = "VecL2Sqr";
const uint32_t THREAD_CNT = 6;
}

namespace aicpu {
uint32_t VecL2sqrCpuKernel::Compute(CpuKernelContext &ctx)
{
    Inputs inputs;
    Outputs outputs;
...
uint32_t VecL2sqrCpuKernel::GetInOutAndCheck(const CpuKernelContext &ctx, Inputs &inputs, Outputs &outputs) const
{
    KERNEL_LOG_INFO("VecL2sqrCpuKernel GetInOutAndCheck begin");
...
uint32_t VecL2sqrCpuKernel::CheckInOutShapes(const Inputs &inputs, const Outputs &outputs)
{
    KERNEL_LOG_INFO("VecL2sqrCpuKernel CheckInputShapes begin");
...
void VecL2sqrCpuKernel::DoCompute(size_t start, size_t end, const Inputs &inputs, Outputs &outputs)
{
...
REGISTER_CPU_KERNEL(VEC_L2SQR, VecL2sqrCpuKernel);
} // namespace aicpu