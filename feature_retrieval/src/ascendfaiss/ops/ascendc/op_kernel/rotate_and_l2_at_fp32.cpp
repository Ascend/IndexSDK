...
class RotateAndL2AtFP32
{
   public:
    ...
    __aicore__ inline void Init(GM_ADDR vectors, GM_ADDR vectorSize, GM_ADDR matrix, GM_ADDR rotate_result,
                                GM_ADDR l2_result, GM_ADDR usrWorkspace, const RotateAndL2AtFP32TilingData *tiling_data)
    {
   ...
   private:
    ...
    __aicore__ inline void TilingAcrossCores(int32_t formerCoreNum, int32_t formerCoreLength, int32_t tailCoreLength)
    {
        ...
        if (block_id < formerCoreNum)
        {
            ...
            if ASCEND_IS_AIV
            {
                if (formerCoreLength % 2 == 0)
                {
                    ...
                    this->vecOffset =
                        block_id * formerCoreLength + sub_block_id * formerCoreLength / 2;  // 当前core负责的向量偏移
                }
                else
                {
                    this->vecLength = (1 - sub_block_id) + (int32_t)(formerCoreLength / 2);  // 第一个vector多处理一个；
                    this->vecOffset = block_id * formerCoreLength + sub_block_id * formerCoreLength / 2 +
                                      sub_block_id;  // 第二个vector多偏移一个
                }
            }
        }
        else
...
extern "C" __global__ __aicore__ void rotate_and_l2_at_fp32(GM_ADDR vectors, GM_ADDR vectorSize, GM_ADDR matrix,
                                                            GM_ADDR rotate_result, GM_ADDR l2_result, GM_ADDR workspace,
                                                            GM_ADDR tiling)
{
