...
__aicore__ __simt_callee__ inline float SimtSearchTableSum(__ubuf__ float *queryLutUb, __gm__ uint8_t *codeBase,
                                                           uint32_t taskIdx, uint32_t ksub)
{
    ...
    // codeOffset: 起始code
    // taskIdx: code index
    // ksub:子空间中的聚类数量，正好是int8格式，共256个
    uint8_t codeBookIndices[UNROLL_FACTOR];  // 从codeBase中读取的codebook
    float sumDist = 0;                       // 累加的距离和
    uint8_t codeBookIndex = 0;               // 单个codebook
    uint8_t m = 0;

    // 对循环进行展开，提高并行计算效率，一直做到 m+UNROLL_FACTOR
#pragma unroll
    for (; m + UNROLL_FACTOR <= NumSubQuantizers; m += UNROLL_FACTOR)
    {
#pragma unroll
        // 批量读取codebook，减少数据依赖
        for (uint8_t i = 0; i < UNROLL_FACTOR; i++)
        {
            // 读取codebook索引，一次性读UNROLL个子空间数据，初始偏移为 codeOffset + taskIdx * NumSubQuantizers
            codeBookIndices[i] = codeBase[taskIdx * NumSubQuantizers + m + i];
        }
#pragma unroll
        // 批量访问queryPQ和累加，减少数据依赖
        for (int i = 0; i < UNROLL_FACTOR; i++)
        {
            sumDist += queryLutUb[(m + i) * ksub + codeBookIndices[i]];
        }
    }
...
#pragma unroll
    ...
    {
        codeBookIndex = codeBase[taskIdx * NumSubQuantizers + m];
        sumDist += queryLutUb[m * ksub + codeBookIndex];
    }
    return sumDist;
}

template <int NumSubQuantizers>
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM) inline void SimtSearchCompute(__ubuf__ float *queryLutUb,
                                                                              __gm__ float *distResultGM,
                                                                              uint32_t codeSize,
                                                                              __gm__ uint8_t *codeBase, uint32_t ksub)
{
    uint32_t curBlockThreadIdx = static_cast<uint32_t>(Simt::GetThreadIdx<0>());  // 当前thread
    uint32_t threadCount = static_cast<uint32_t>(Simt::GetThreadNum<0>());        // thread数量

    uint32_t startTaskId = 0;
    uint32_t endtaskId = codeSize;
    ...
    for (uint32_t taskIdx = curBlockThreadIdx + startTaskId; taskIdx < endtaskId; taskIdx += threadCount)
    {
        ...
        distResultGM[dstIndex] = SimtSearchTableSum<NumSubQuantizers>(queryLutUb, codeBase, taskIdx, ksub);
    }
...
template <typename T>
class DistanceIVFRabitqL2FP32Simt
{
   public:
    TPipe *pipe_;
    DistanceIVFRabitqL2FP32SimtTilingData tiling_;

   public:
    __aicore__ inline DistanceIVFRabitqL2FP32Simt(TPipe *pipe_in) { pipe_ = pipe_in; };
                                ...
                                GM_ADDR usrWorkspace, const DistanceIVFRabitqL2FP32SimtTilingData *tiling_data)
    {
        ...
        mm_out_gm_.SetGlobalBuffer((__gm__ T *)(usrWorkspace + mmoutBufferSize * this->blockIdx));  // 存储矩阵乘结果

        this->distTileLength = tiling_.distTileLength;  // 每次处理多少codes
        ...
        pipe_->InitBuffer(in_centroidslut_que, 1,
                          RoundUp(this->lutTileLength * this->lutDimLength * sizeof(T), 64));  // centroidslut分tile拉取
    }
    ...
    {
        if (this->actualSizeVal > 0)
        {
            QuerySubtractCentroid();  // 先计算q-c
            PipeBarrier<PIPE_ALL>();
            LookUpTableAndSumSimt();  // 查表求和
            PipeBarrier<PIPE_ALL>();
            SecondInit();   // 在执行最后阶段之前需要释放所有内存，并重新初始化
            DistCompute();  // 计算距离
            PipeBarrier<PIPE_ALL>();
        }
        PipeBarrier<PIPE_ALL>();
   ...
   private:
    ...
    __aicore__ inline void CopyInForCentroidsLUT(int32_t progress, int32_t copyCodesLength)
    {
        ...
        DataCopy(centroidslutLocal, cur_centroidslut_gm_[progress * this->lutDimLength * this->lutTileLength],
                 copyCodesLength * this->lutDimLength);
        in_centroidslut_que.EnQue(centroidslutLocal);
    }

    __aicore__ inline void LookUpTableAndSumSimt()
    {
        LocalTensor<float> distResultLocal =
            in_centroidslut_que.AllocTensor<float>();  // 分配distResultLocal, 长度为codeTileLength

        LocalTensor<T> querylutLocal = in_querylut_que.DeQue<T>();  // 取出q-c LUT结果
        PipeBarrier<PIPE_ALL>();
        ProcessDistResult(querylutLocal);
        in_querylut_que.FreeTensor(querylutLocal);
    }

    __aicore__ inline void ProcessDistResult(LocalTensor<float> querylutLocal)
    {
        __ubuf__ float *queryLutUb = (__ubuf__ float *)querylutLocal.GetPhyAddr();
        __gm__ float *distResultGM = (__gm__ float *)this->mm_out_gm_.GetPhyAddr();
        __gm__ uint8_t *codeBase = (__gm__ uint8_t *)this->code_gm_.GetPhyAddr();

        // 共 dim/8=16 个分段
        Simt::VF_CALL<SimtSearchCompute<16>>(Simt::Dim3{THREAD_NUM, 1, 1}, queryLutUb, distResultGM,
                                             this->actualSizeVal, codeBase, this->ksub);
        PipeBarrier<PIPE_ALL>();
    }
    ...
    __aicore__ inline void DistCompute()
    {
        ...
        for (int32_t i = 0; i < this->distTileNum; i++)
        {
            ...
            CopyInForMatResAndIndexL1(i, copyLength);  // 拷贝matmul结果和L1因子
    ...
    __aicore__ inline void CopyInForMatResAndIndexL1(int32_t progress, int32_t copyCodesLength)
    {
        ...
        DataCopyPad(matResLocal, mm_out_gm_[progress * this->distTileLength], copyParams, padParams);
    ...
    __aicore__ inline void Compute(int32_t progress, int32_t computeCodesLength, LocalTensor<T> minResultLocal)
    {
        ...
        PipeBarrier<PIPE_V>();
        Mul(matResLocal, matResLocal, indexL1Local, computeCodesLength);  // 复用matResLocal保存临时结果
        CopyInForIndexL2(progress, computeCodesLength);
        PipeBarrier<PIPE_V>();
        LocalTensor<T> indexL2Local = in_indexl2_que.DeQue<T>();  // 拷贝L2距离
        Sub(distResultLocal, indexL2Local, matResLocal, computeCodesLength);
        PipeBarrier<PIPE_V>();
        Adds(distResultLocal, distResultLocal, cur_centroidsl2, computeCodesLength);
        PipeBarrier<PIPE_V>();
        if (this->metric == METRIC_INNER_PRODUCT)
        {  // dist ip: - 2 <q, o> = ||o - q||^2 - ||q||^2 - ||o||^2
            PipeBarrier<PIPE_V>();
        ...
        {
            Copy(
                minResultLocal[(progress % this->mask) * this->distTileLength + (computeCodesLength - lastCodesLength)],
                distResultLocal[computeCodesLength - lastCodesLength], lastCodesLength, 1, {1, 1, 8, 8});
        }

        ComputeMinResult(progress, minResultLocal);

        dist_result_que.EnQue(distResultLocal);  // 结果入队
        in_mat_res_que.FreeTensor(matResLocal);
    ...
    __aicore__ inline void ComputeMinResult(int32_t progress, LocalTensor<T> minResultLocal)
    {
        ...
        {  // 当progress为63时，才会进入，需要运行64次才会进行一次计算操作
