...
class IndexCodeAndPrecompute
{
   public:
    ...
    __aicore__ inline void Init(GM_ADDR vectorSize, GM_ADDR indexes, GM_ADDR indexesl2, GM_ADDR centroid,
                                GM_ADDR centroidl2, GM_ADDR codes_result, GM_ADDR l2_result, GM_ADDR l1_result,
                                GM_ADDR usrWorkspace, const IndexCodeAndPrecomputeTilingData *tiling_data)
    {
   ...
   private:
    ...
    __aicore__ inline void VecComputeForCode(int32_t progress, int32_t copyLength)
    {
        ...
        {
            Mul(tmp_Local[i * this->dimLength], indexesLocal[i * this->dimLength], this->centerLocal, this->dimLength);
        }
        PipeBarrier<PIPE_V>();
        ...
        for (int32_t i = 0; i < copyLength; i++)
        {
            Sub(indexesLocal[i * this->dimLength], indexesLocal[i * this->dimLength], this->centerLocal,
                this->dimLength);
        }
        PipeBarrier<PIPE_V>();

        // 双值量化
        CompareScalar(codesResultLocal, indexesLocal, (float32_t)0, AscendC::CMPMODE::GE, this->dimLength * copyLength);
        PipeBarrier<PIPE_V>();
        ...
        for (int32_t i = 0; i < copyLength; i += 255)
        {
            int32_t len = copyLength - i;
            if (len > 255) len = 255;
            WholeReduceSum<T>(l1ResultLocal[i], indexesLocal[i * this->dimLength], this->mask, len, 1, 1, repStride);
        }
    ...
    __aicore__ inline void CopyOutForCode(int32_t progress, int32_t copyLength)
    {
        ...
        DataCopyPad(codes_res_gm_[progress * this->tileLength * this->dimLength / 8], codesResultLocal, copyParams1);
    ...
    __aicore__ inline void VecComputeForPreComp(int32_t progress, int32_t copyLength)
    {
        ...
        if (this->metric == 1)
        {  // L2: ||indexes - centroid||^2; IP: ||indexes - centroid||^2 - ||indexes||^2
            PipeBarrier<PIPE_V>();
                                                                ...
                                                                GM_ADDR l1_result, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    op.Init(vectorSize, indexes, indexesl2, centroid, centroidl2, codes_result, l2_result, l1_result, usrWorkspace,
            &tiling_data);
    op.Process();
}
