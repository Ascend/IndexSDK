...
class DistanceIVFFlatIpFP32
{
   public:
    ...
    __aicore__ inline void InitBuffer(GM_ADDR query, GM_ADDR base, GM_ADDR offset, GM_ADDR actual_size, GM_ADDR result,
    ...
    __aicore__ inline void InitCoreLength()
    {
        ...
        this->core_length = (this->all_codes / this->aiv_num + this->tiling_.burst_len - 1) / this->tiling_.burst_len *
                            this->tiling_.burst_len;
        ...
        if (this->length_arr_block_length > 0)
        {
            ...
            this->length_arr[this->length_arr_core_idx][1] =
                this->length_arr[this->length_arr_core_idx - 1][1] + this->length_arr[this->length_arr_core_idx - 1][0];
        }
    ...
    __aicore__ inline void Process(GM_ADDR base)
    {
        ...
        if (this->tile_core_length > 0)
        {
            while (this->cube_flag)
            {
                ...
                {
                    WaitGemmQB();
                    VecProcess(this->offset_arr[2][this->pingpong_idx % 2], this->offset_arr[0][this->pingpong_idx % 2],
                               this->offset_arr[1][this->pingpong_idx % 2], this->pingpong_idx - 1);
                }
                this->core_offset = this->core_offset + this->length;
            }
            WaitGemmQB();
            VecProcess(this->length, this->offset_arr[0][(this->pingpong_idx - 1) % 2],
                       this->offset_arr[1][(this->pingpong_idx - 1) % 2], this->pingpong_idx);
        }
   ...
   private:
    __aicore__ inline void InitLengthArr(int32_t actual_size_val)
    {
        ...
        int32_t actual_size_val_block =
            (actual_size_val + this->tiling_.burst_len - 1) / this->tiling_.burst_len * this->tiling_.burst_len;
        ...
        else
        {
            ...
            int32_t block_next_length =
                (next_length + this->tiling_.burst_len - 1) / this->tiling_.burst_len * this->tiling_.burst_len;
    ...
    __aicore__ inline bool isLast(int32_t actual_size_val)
    {
        if (this->length_arr_core_idx == this->aiv_num - 1)
        {
            ...
            {
                this->length_arr[this->length_arr_core_idx][1] = this->length_arr[this->length_arr_core_idx - 1][1] +
    ...
    __aicore__ inline void VecProcess(int32_t code_len, int32_t copyout_offset, int32_t dst_max_res_offset,
                                      int32_t pingpong_idx)
    {
        ...
        int32_t align_burst_len =
            (align_code_len + this->tiling_.burst_len - 1) / this->tiling_.burst_len * this->tiling_.burst_len;
        ...
        WholeReduceMax(res_local, mm_res_local, this->tiling_.burst_len, rep_times, 1, 1, this->tiling_.burst_len / 8);
        out_que.EnQue(res_local);
        res_local = out_que.DeQue<T>();
    ...
    __aicore__ inline void LaunchGemmQB(int32_t length, int32_t offset, int32_t pingpong_idx)
    {
        ...
        gemm_qb.template IterateAll<false>(mm_out_gm_[(pingpong_idx % 2) * tiling_.per_loop_process_len], 0, false,
                                           true);
    }
        ...
        if (this->tile_core_length > 0)
        {
            for (int32_t i = 0; i < this->aiv_num; i++)
            {
                    ...
                    this->core_offset < this->actual_size_pro_val + actual_size_gm_.GetValue(i))
                {
                    ...
                    this->offset_arr[1][0] =
                        i * this->tiling_.max_res_stride + this->offset / this->tiling_.burst_len * 2;
...
extern "C" __global__ __aicore__ void distance_ivf_flat_ip_fp32(GM_ADDR query, GM_ADDR base, GM_ADDR offset,
                                                                GM_ADDR actual_size, GM_ADDR result, GM_ADDR max_result,
                                                                GM_ADDR flag, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    op.InitBuffer(query, base, offset, actual_size, result, max_result, flag, usrWorkspace, &tiling_data);
    op.InitCoreLength();
    op.Process(base);
}
