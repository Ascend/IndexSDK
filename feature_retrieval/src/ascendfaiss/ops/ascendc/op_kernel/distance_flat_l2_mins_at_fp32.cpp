...
class DistanceFlatL2MinsAtFP32
{
   public:
    ...
    __aicore__ inline void InitTilingInfo(const DistanceFlatL2MinsAtFP32TilingData &tiling_data)
    {
        ...
        this->minResultColsPerRow = (this->codesNumLength + this->mask - 1) / this->mask * 2;
        this->minResultColsPerCodesTile = (this->codesTileLength + this->mask - 1) / this->mask * 2;
    }
                                      ...
                                      GM_ADDR min_result, GM_ADDR flag_shape, GM_ADDR usrWorkspace)
    {
        ...
        minResultGlobal.SetGlobalBuffer((__gm__ T *)min_result, this->queryNumLength * this->minResultColsPerRow);
        ...
        pipe.InitBuffer(
            minResultQueue, 1,
            this->formerTileLength * ((this->codesTileLength + this->mask - 1) / this->mask * 2) * sizeof(T));
    ...
    __aicore__ inline void ProcessCore(int pIdx)
    {
        ...
        if ((pIdx - 1) % this->codesTileNum == 0)
        {
            ...
            DataCopy(queryLocal,
                     queryGlobal[(pIdx - 1) / this->codesTileNum * this->formerTileLength * this->dimLength],
            ...
            {
                WholeReduceSum<T>(processLocal, queryLocal, this->dimLength, this->queryLength, 1, 1,
                                  this->dimLength / 8);
                PipeBarrier<PIPE_V>();
            }
   ...
   private:
    __aicore__ inline void VecProcess(int32_t queryProcess, int32_t codesProcess, LocalTensor<T> processLocal,
                                      int32_t pingPongId)
    {
    ...
    __aicore__ inline void Compute(LocalTensor<T> processLocal)
    {
        ...
        Sub(distResultLocal, distResultLocal, workspaceLocal, this->queryLength * this->codesLength);
        PipeBarrier<PIPE_V>();
        const uint32_t minColsPerRow = (this->codesLength + this->mask - 1) / this->mask * 2;
        const uint32_t fullSegs = this->codesLength / this->mask;
        const uint32_t tailLen = this->codesLength % this->mask;
        for (int32_t i = 0; i < this->queryLength; i++)
        {
            if (fullSegs != 0)
            {
                WholeReduceMin(minResultLocal[i * minColsPerRow], distResultLocal[i * this->codesLength], this->mask,
                               fullSegs, 1, 1, 8);
            }
        }
        if (tailLen != 0)
        {
            ...
            for (int32_t i = 0; i < this->queryLength; i++)
            {
                WholeReduceMin(minResultLocal[i * minColsPerRow + fullSegs * 2],
                               distResultLocal[i * this->codesLength + fullSegs * this->mask], tailLen, 1, 1, 1, 8);
            }
        }
    ...
    __aicore__ inline void CopyOut(int32_t queryProcess, int32_t codesProcess)
    {
        ...
        ub2gm_datacopy_aligned(
            this->queryLength, this->codesLength, this->codesLength, this->codesNumLength, distResultLocal,
            distResultGlobal[(this->queryCoreOffset + queryProcess * this->formerTileLength) * this->codesNumLength +
                             codesProcess * this->codesTileLength]);
        distResultQueue.FreeTensor(distResultLocal);
        LocalTensor<T> minResultLocal = minResultQueue.DeQue<T>();
        const uint32_t minColsPerRow = (this->codesLength + this->mask - 1) / this->mask * 2;
        ub2gm_datacopy_aligned(this->queryLength, minColsPerRow, minColsPerRow, this->minResultColsPerRow,
                               minResultLocal,
                               minResultGlobal[(this->queryCoreOffset + queryProcess * this->formerTileLength) *
                                                   this->minResultColsPerRow +
                                               codesProcess * this->minResultColsPerCodesTile]);
    ...
    __aicore__ inline void ub2gm_datacopy_aligned(const uint32_t &rows, const uint32_t &cols,
                                                  const uint32_t &src_offset, const uint32_t &dst_offset,
                                                  const AscendC::LocalTensor<T> &src_tensor,
   ...
   private:
    ...
    int32_t dimLength;
    int32_t bufferSize;
    uint32_t minResultColsPerRow;
    uint32_t minResultColsPerCodesTile;
...
extern "C" __global__ __aicore__ void distance_flat_l2_mins_at_fp32(GM_ADDR query, GM_ADDR codes, GM_ADDR codes_double,
                                                                    GM_ADDR dist_result, GM_ADDR min_result,
                                                                    GM_ADDR flag_shape, GM_ADDR workspace,
                                                                    GM_ADDR tiling)
{
