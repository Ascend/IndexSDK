...
#include "cagra_topk.h"

template<int TEAM_SIZE, int DATASET_BLOCK_DIM, int MAX_ITOPK,
         int MAX_CANDIDATES,bool TOPK_BY_BITONIC_SORT,
        typename DATA_T, typename INDEX_T, typename DISTANCE_T>
__simt_vf__ __aicore__ LAUNCH_BOUND(256) inline void Simtsearch(
    __gm__ const float* const queries_ptr,
    const uint32_t dim,
    __ubuf__ DATA_T* query_buffer,
    const uint32_t query_smem_buffer_length,
    __ubuf__ uint32_t* terminate_flag,
    __ubuf__ uint32_t* topk_ws,
    const uint32_t hash_table_size,

    __gm__ const INDEX_T* seed_ptr,
    const uint32_t num_seeds,
    __ubuf__ INDEX_T* result_indices_buffer,
    ...
    __gm__ DISTANCE_T* result_distances_ptr,
    __gm__ INDEX_T* result_indices_ptr,
    __gm__ uint32_t* const num_executed_iterations,

    __ubuf__ uint32_t* smem_work_ptr,
    const uint32_t small_hash_reset_interval,
    const uint32_t hash_bitlen,
    const uint32_t adding_pref,
    __ubuf__ uint32_t* filter_flag,
    __gm__ uint32_t* const visited_hashmap_ptr
)
...
{   none_cagra_sample_filter sample_filter;
    ...
    compute_distance_to_random_nodes<TEAM_SIZE, DATASET_BLOCK_DIM>(result_indices_buffer,
    ...
    while (not_add_count <= patience) {
        if (threadIdx.x == 0){count_flag[0] = 0;}
        ...
        if constexpr (TOPK_BY_BITONIC_SORT) {
            ...
            get_min_k(result_distances_buffer + internal_topk,
            ...
            __sync_workitems();
            if (threadIdx.x == 0) {
                if (result_distances_buffer[parent_list_buffer[0]] < candidate_distances_buffer[0]) {
                    parent_list_buffer[0] = candidate_indices_buffer[0];
                } else {parent_list_buffer[0] = result_indices_buffer[parent_list_buffer[0]];}
            }

            __sync_workitems();

            pickChildren(knn_graph,
                graph_degree,
                temp_indices_buffer,
                parent_list_buffer,
                search_width,
            ...
            topk_by_radix_sort<MAX_ITOPK, INDEX_T>{}(
                ...
                56,
                result_buffer_size,
                reinterpret_cast<__ubuf__ uint32_t*>(result_distances_buffer),
                result_indices_buffer,
                reinterpret_cast<__ubuf__ uint32_t*>(result_distances_buffer),
                result_indices_buffer,
                nullptr,
                topk_ws,
                true,
                reinterpret_cast<__ubuf__ uint32_t*>(smem_work_ptr));

            if ((iter + 1) % small_hash_reset_interval == 0) {
                init(local_visited_hashmap_ptr, hash_bitlen);
            }
        }
        __sync_workitems();

        if (count_flag[0] == 1) {not_add_count++;} else {not_add_count = 0;}
        if (iter + 1 == max_iteration) { break; }
        if (*terminate_flag && iter >= min_iteration) { break; }

        copy_buffer_distances(result_indices_buffer + internal_topk,
                    result_distances_buffer + internal_topk,
                    temp_distances_buffer,
                    temp_indices_buffer,
                    search_width * graph_degree);

    {
        ...
        set_parents(result_indices_buffer, internal_topk, parent_list_buffer, first_tid);
    }
        ...
        for (unsigned p = threadIdx.x; p < search_width; p += blockDim.x) {
            if (parent_list_buffer[p] != invalid_index) {
                ...
                if (!sample_filter(query_id, parent_id)) {
                    result_distances_buffer[parent_list_buffer[p]] = get_max_value<DISTANCE_T>();
    ...
    unsigned ii = i;
    if (TOPK_BY_BITONIC_SORT) { ii = swizzling(i); }
...
extern "C" __global__ __aicore__ void cagra(
    GM_ADDR queries_ptr_gm,
    GM_ADDR knn_graph_gm,
    GM_ADDR visited_hashmap_ptr_gm,
    GM_ADDR ptr_gm,

    GM_ADDR result_distances_ptr_gm,
    GM_ADDR result_indices_ptr_gm,
...

{
	...
	constexpr uint32_t num_distilation = 1;
	...
	constexpr uint32_t hash_bitlen = 8;
	constexpr uint32_t  small_hash_bitlen = 8;
	constexpr uint32_t small_hash_reset_interval = 1;

    const uint32_t small_hash_bitlen = small_hash_bitlen__small_hash_reset_interval >> 32;
    const uint32_t small_hash_reset_interval = small_hash_bitlen__small_hash_reset_interval;
    std::uint32_t adding_pref = 25;
    std::uint32_t patience = 16;

    constexpr uint32_t TEAM_SIZE = 8;
    ...
    const auto small_hash_size = get_size(small_hash_bitlen);

    const uint32_t hash_table_size = (1000000 + 31) / 32;
    ...
    __ubuf__ INDEX_T *result_indices_buffer =
        reinterpret_cast<__ubuf__ INDEX_T *>(query_buffer + query_smem_buffer_length);
    ...
    __ubuf__ INDEX_T *parent_list_buffer = reinterpret_cast<__ubuf__ INDEX_T *>(temp_distances_buffer + small_hash_size);

    __ubuf__ INDEX_T *candidate_indices_buffer =
        reinterpret_cast<__ubuf__ INDEX_T *>(parent_list_buffer + search_width);
    ...
    __ubuf__ uint32_t *smem_work_ptr = reinterpret_cast<__ubuf__ uint32_t *>(count_flag + 1);

    set_smem_ptr(distance_work_buffer_ptr);

    __ubuf__ uint32_t *filter_flag = terminate_flag;

    Simt::VF_CALL<Simtsearch<TEAM_SIZE, DATASET_BLOCK_DIM, MAX_ITOPK,
                            MAX_CANDIDATES, TOPK_BY_BITONIC_SORT,
