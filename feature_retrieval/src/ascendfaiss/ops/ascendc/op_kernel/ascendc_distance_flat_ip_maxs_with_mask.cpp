    ...
    if (leftLoopTime != 0)
    {
        ...
        else
        {
            ...
            aicoreCodeMoveOffset =
                leftLoopTime * (eachCoreLoopTime + 1) * codeNumEachLoop + (blockIdx - leftLoopTime) * codeNumEachCore;
        }
    }
    else
    {
...
__aicore__ inline void AscendcDistanceFlatIPMaxsWithMask::Process()
{
    ...
    {
        DistanceComputeLoop(aicoreCodeMoveOffset, codeNumEachCore, queryLoopIdx * queryNumEachLoop, queryNumEachLoop,
                            false);
    }
...
__aicore__ inline void AscendcDistanceFlatIPMaxsWithMask::DoMask(uint32_t codeMoveOffset, uint32_t codeMoveNum,
                                                                 uint32_t queryMoveOffset, uint32_t queryMoveNum,
...
__aicore__ inline void AscendcDistanceFlatIPMaxsWithMask::CubeComputeLoop(uint32_t codeMoveOffset, uint32_t codeMoveNum,
                                                                          uint32_t queryMoveOffset,
                                                                          uint32_t queryMoveNum, bool isTail)
{
...
extern "C" __global__ __aicore__ void ascendc_distance_flat_ip_maxs_with_mask(GM_ADDR query, GM_ADDR mask,
                                                                              GM_ADDR shaped, GM_ADDR actualSize,
                                                                              GM_ADDR dist, GM_ADDR maxDist,
                                                                              GM_ADDR flag, GM_ADDR workspace,
                                                                              GM_ADDR tiling)
{
