...
class AscendcDistInt8FlatL2
{
   public:
    __aicore__ inline AscendcDistInt8FlatL2(const AscendcDistInt8FlatL2TilingData& tilingData, bool useFp16)
        : queryNum(tilingData.querySize),
          dim(tilingData.dim),
          codeBlockSize(tilingData.codeBlockSize),
          availableCoreNum(tilingData.aivNum),
          blockIdx(GetBlockIdx()),
          useFp16(useFp16),
          cubeTilingSquare(tilingData.cubeTilingSquare),
          cubeTilingIp(tilingData.cubeTilingIp)
    {
    }

    // 初始化，为输入输出分配内存
    __aicore__ inline void Init(GM_ADDR queries, GM_ADDR mask, GM_ADDR codes, GM_ADDR norm, GM_ADDR actualSize,
                                GM_ADDR distance, GM_ADDR distanceMin, GM_ADDR flag);
   ...
   private:
    ...
    __aicore__ inline void SetBuffer(GM_ADDR queries, GM_ADDR mask, GM_ADDR codes, GM_ADDR norm, GM_ADDR distance,
                                     GM_ADDR distanceMin, GM_ADDR flag);
    ...
    __aicore__ inline void ComputeNormSum(uint32_t queryProcNum, LocalTensor<int32_t>& distanceLocal,
                                          LocalTensor<int32_t>& querySquareLocal);
    ...
    __aicore__ inline void CopyOutDist(uint32_t queryLoopIdx, uint32_t codeLoopIdx, uint32_t queryProcNum,
                                       LocalTensor<half>& distanceTensor);

    __aicore__ inline void CopyOutDistMin(uint32_t queryLoopIdx, uint32_t codeLoopIdx, uint32_t queryProcNum,
                                          LocalTensor<half>& distanceMinTensor);
...
__aicore__ inline void AscendcDistInt8FlatL2::Init(GM_ADDR queries, GM_ADDR mask, GM_ADDR codes, GM_ADDR norm,
                                                   GM_ADDR actualSize, GM_ADDR distance, GM_ADDR distanceMin,
                                                   GM_ADDR flag)
{
...
__aicore__ inline void AscendcDistInt8FlatL2::ComputeCoreProcInfo()
{
    ...
    else if (blockIdx != usedCoreNum - 1)
    {
        ...
        {
            codeOffset = (batchTail * codeNumEachCoreHead + (blockIdx - batchTail) * batchNumPerCoreProcess) *
                         CORE_PROC_CODE_GRAIN;
    ...
    {
        codeOffset =
            (batchTail * codeNumEachCoreHead + (blockIdx - batchTail) * batchNumPerCoreProcess) * CORE_PROC_CODE_GRAIN;
...
__aicore__ inline void AscendcDistInt8FlatL2::SetBuffer(GM_ADDR queries, GM_ADDR mask, GM_ADDR codes, GM_ADDR norm,
                                                        GM_ADDR distance, GM_ADDR distanceMin, GM_ADDR flag)
{
    queryTensor.SetGlobalBuffer(reinterpret_cast<__gm__ int8_t*>(queries));
    maskTensor.SetGlobalBuffer(reinterpret_cast<__gm__ uint8_t*>(mask) + (maskBlockOffset + codeOffset) / MASK_BIT_NUM);
...
__aicore__ inline void AscendcDistInt8FlatL2::ProcessEachCodeLoop(uint32_t queryLoopIdx, uint32_t codeLoopIdx,
                                                                  uint32_t queryProcNum, bool isLastQuery,
                                                                  LocalTensor<int32_t>& querySquareLocal)
{
...
__aicore__ inline void AscendcDistInt8FlatL2::ComputeNormSum(uint32_t queryProcNum, LocalTensor<int32_t>& distanceLocal,
                                                             LocalTensor<int32_t>& querySquareLocal)
{
    // y^2
...
__aicore__ inline void AscendcDistInt8FlatL2::DoMask(uint32_t queryLoopIdx, uint32_t codeLoopIdx, uint32_t queryProcNum,
                                                     LocalTensor<half>& distanceTensor)
{
    if (useMask == 0)
    {
    ...
    for (uint32_t i = 0; i < selectLoopTimes; i++)
    {
        Select(distanceTensor[i * SELECT_REPEAT_TIME * VIC_HALF_FULL_MASK],
               ...
               distanceTensor[i * SELECT_REPEAT_TIME * VIC_HALF_FULL_MASK], HALF_MAX, SELMODE::VSEL_TENSOR_SCALAR_MODE,
    ...
    if (selectRemainderTimes != 0)
    {
        Select(distanceTensor[selectLoopTimes * SELECT_REPEAT_TIME * VIC_HALF_FULL_MASK],
               ...
               distanceTensor[selectLoopTimes * SELECT_REPEAT_TIME * VIC_HALF_FULL_MASK], HALF_MAX,
    ...
    if (codeProcNum / BURST_LEN != 0)
    {
        for (uint32_t i = 0; i < queryProcNum; i++)
        {
            WholeReduceMin<half>(distanceMinTensor[i * codeBatchSize * BURST_BLOCK_RATIO],
                                 distanceTensor[i * codeSizeEachLoop], BURST_LEN, codeProcNum / BURST_LEN, 1, 1,
                                 srcRepStride);
        }
    }
    ...
    if (codeProcNum % BURST_LEN != 0)
    {
        for (uint32_t i = 0; i < queryProcNum; i++)
        {
            WholeReduceMin<half>(distanceMinTensor[(i * codeBatchSize + codeProcNum / BURST_LEN) * BURST_BLOCK_RATIO],
                                 ...
                                 codeProcNum % BURST_LEN, 1, 1, 1, srcRepStride);
        }
    }
}
...
{
    auto copyOffset =
        (queryLoopIdx * queryCopyOutBatchOffset + codeLoopIdx * codeCopyOutBatchOffset) * BURST_BLOCK_RATIO;
    DataCopyParams copyParams;
    copyParams.blockCount = queryProcNum;
...
extern "C" __global__ __aicore__ void ascendc_dist_int8_flat_l2(GM_ADDR queries, GM_ADDR mask, GM_ADDR codes,
                                                                GM_ADDR norm, GM_ADDR actualSize, GM_ADDR distance,
                                                                GM_ADDR distanceMin, GM_ADDR flag, GM_ADDR workspace,
                                                                GM_ADDR tiling)
{
