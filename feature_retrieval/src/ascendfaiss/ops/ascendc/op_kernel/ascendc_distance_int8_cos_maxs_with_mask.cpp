...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::SetGlobalMemory(GM_ADDR queryData, GM_ADDR mask,
    GM_ADDR baseData, GM_ADDR queryNormData, GM_ADDR baseNormData, GM_ADDR result, GM_ADDR resultMax, GM_ADDR flag)
{
    ...
    if (loopIndex < loopNum) {
        ...
        if (repeatTimes > 0) {
            for (uint32_t j = 0; j < queryNum; j++) {
                WholeReduceMax(resultMaxLocal[j * burstSizeEachLoop],
                    resultLocal[j * onceComputeBaseNum],
                    BURST_SIZE,
                    repeatTimes,
                    1,
                    1,
        ...
        if (lastNum > 0) {
            for (uint32_t j = 0; j < queryNum; j++) {
                WholeReduceMax(resultMaxLocal[j * burstSizeEachLoop + repeatTimes * BURST_RESULT_SIZE],
                    resultLocal[j * onceComputeBaseNum + repeatTimes * BURST_SIZE],
                    lastNum,
                    1,
                    1,
                    1,
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::DoMask(const uint32_t &loopOffset)
{
    ...
    for (uint32_t i = 0; i < queryNum; i++) {
        DataCopy(maskLocal[i * onceComputeBaseNumMaskLen], maskGlobal[i * static_cast<uint64_t>(blockMaskLen) +
...
extern "C" __global__ __aicore__ void ascendc_distance_int8_cos_maxs_with_mask(GM_ADDR queryData,
                                                                 GM_ADDR mask,
                                                                 GM_ADDR baseData,
