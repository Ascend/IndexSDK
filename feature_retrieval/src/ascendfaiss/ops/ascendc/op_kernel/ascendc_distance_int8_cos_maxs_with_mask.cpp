...
namespace IndexOps
{
class AscendcDistanceInt8CosMaxsWithMask
{
   public:
    __aicore__ inline AscendcDistanceInt8CosMaxsWithMask(const AscendcDistanceInt8CosMaxsWithMaskTilingData &tilingData)
   ...
   private:
    ...
    __aicore__ inline void CalcResultMax(LocalTensor<half> &resultMaxLocal, const uint32_t &loopIndex);
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::ParseActualSize(GM_ADDR actualSize, uint32_t &actualNum)
{
    actualNum = *(reinterpret_cast<__gm__ uint32_t *>(actualSize));
    blockMaskLen = blockSize / MASK_BIT_NUM;
}

__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::CalcComputeNumAndCoreOffset(const uint32_t &actualNum)
{
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::SetFlagGlobalMemory(GM_ADDR flag)
{
    flagGlobal.SetGlobalBuffer(reinterpret_cast<__gm__ uint16_t *>(flag) + coreIdx * FLAG_SIZE);
}

__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::SetGlobalMemory(GM_ADDR queryData, GM_ADDR mask,
                                                                           GM_ADDR baseData, GM_ADDR queryNormData,
                                                                           GM_ADDR baseNormData, GM_ADDR result,
                                                                           GM_ADDR resultMax, GM_ADDR flag)
{
    aGlobal.SetGlobalBuffer(reinterpret_cast<__gm__ int8_t *>(queryData));
    maskGlobal.SetGlobalBuffer(reinterpret_cast<__gm__ uint8_t *>(mask) + (coreOffset) / MASK_BIT_NUM);
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::InitBuffer()
{
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::CalcConstants()
{
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::Init(GM_ADDR queryData, GM_ADDR mask, GM_ADDR baseData,
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::CalcAB(const uint32_t &bMatrixOffset)
{
    mm.SetTensorB(bGlobal[bMatrixOffset], true);
    mm.IterateAll<false>(cubeOutGlobal, 0, false, true);
}

__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::MoveBaseNormFromGM(LocalTensor<half> &baseNormLocal,
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::CalcResult(const uint32_t &baseNumAlign128)
{
    // 1和8均表示内存是连续的
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::CalcResultMax(LocalTensor<half> &resultMaxLocal,
                                                                         const uint32_t &loopIndex)
{
    // 计算循环次数
    uint32_t loopNum = computeNum / onceComputeBaseNum;
    uint32_t remainNum = computeNum % onceComputeBaseNum;
    uint32_t remainNumLast = remainNum % BURST_SIZE;
    ...
    if (loopIndex < loopNum)
    {
        ...
        WholeReduceMax(resultMaxLocal[reduceMaxRemainderDstOffset], resultLocal[reduceMaxRemainderSrcOffset],
                       BURST_SIZE, reduceMaxRemainder, 1, 1, 4);
    }
    else
    {
        uint32_t srcRepStride = BURST_SIZE * sizeof(half) / DEFAULT_C0_SIZE;

        uint32_t repeatTimes = remainNum / BURST_SIZE;

        uint32_t burstSizeEachLoop = onceComputeBaseNum / BURST_SIZE * BURST_RESULT_SIZE;

        if (repeatTimes > 0)
        {
            for (uint32_t j = 0; j < queryNum; j++)
            {
                WholeReduceMax(resultMaxLocal[j * burstSizeEachLoop], resultLocal[j * onceComputeBaseNum], BURST_SIZE,
                               repeatTimes, 1, 1, srcRepStride);
            }
        }

        uint32_t lastNum = remainNum % BURST_SIZE;

        if (lastNum > 0)
        {
            for (uint32_t j = 0; j < queryNum; j++)
            {
                WholeReduceMax(resultMaxLocal[j * burstSizeEachLoop + repeatTimes * BURST_RESULT_SIZE],
                               resultLocal[j * onceComputeBaseNum + repeatTimes * BURST_SIZE], lastNum, 1, 1, 1,
                               srcRepStride);
            }
        }
    }
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::MoveResultToGM(const uint32_t &loopOffset,
                                                                          const uint32_t &curBaseNumAlign16)
{
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::MoveResultMaxToGM(const uint32_t &loopOffset)
{
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::MoveMatmulRetFromGM()
{
    mm.WaitIterateAll();
    DataCopyParams copyPara{
        1,  // 1片连续内存
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::DoMask(const uint32_t &loopOffset)
{
    ...
    for (uint32_t i = 0; i < queryNum; i++)
    {
        DataCopy(maskLocal[i * onceComputeBaseNumMaskLen],
                 maskGlobal[i * static_cast<uint64_t>(blockMaskLen) + loopOffset / MASK_BIT_NUM],
                 onceComputeBaseNumMaskLen);
    }
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::ComputeOneLoop(const uint32_t &loopIndex,
                                                                          const uint32_t &curBaseNumAlign16,
                                                                          ...
                                                                          const bool &calcNext)
{
    ...
    CalcResultMax(resultMaxLocal, loopIndex);
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::MoveAMatrixFromGM(LocalTensor<int8_t> &aMatrixLocal)
{
    Nd2NzParams copyParam = {
        ...
        static_cast<uint16_t>(
            tiling.M),  // 目的nz矩阵中，来自源操作数同一行的多行数据相邻行起始地址间的偏移：按128行对齐，每行是32B
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::CalcABFirstTime(LocalTensor<int8_t> &aMatrixLocal)
{
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::MoveQueryNormFromGM()
{
    DataCopyParams copyPara{
        1,  // 1片连续内存
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::CalcResultInLoops()
{
    uint32_t loopNum = computeNum / onceComputeBaseNum;
    uint32_t remainNum = computeNum % onceComputeBaseNum;
    bool calcNext = true;
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::Process()
{
...
__aicore__ inline void AscendcDistanceInt8CosMaxsWithMask::SetFlag()
{
...
extern "C" __global__ __aicore__ void ascendc_distance_int8_cos_maxs_with_mask(
    ...
    GM_ADDR result, GM_ADDR resultMax, GM_ADDR flag, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    IndexOps::AscendcDistanceInt8CosMaxsWithMask op(tilingData);
