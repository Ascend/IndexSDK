...
class AscendcDistanceBatchMaskGeneratorWithExtra
{
   public:
    ...
    __aicore__ inline void Init(GM_ADDR queryTimeStamp, GM_ADDR queryTokenSet, GM_ADDR dbTimeStamp, GM_ADDR dbDivisor,
                                GM_ADDR dbRemainder, GM_ADDR extraMask, GM_ADDR extraMaskAttr, GM_ADDR distanceMask,
                                GM_ADDR workspace,
    ...
    __aicore__ inline void CompareExtraMask(uint32_t bid, uint32_t loop)
    {
        ...
        if (subUseExtraMask != 0 && currentOffset < extraMaskLen)
        {
            ...
            if (currentExtraMaskOffset < this->distanceMaskLenAlign)
            {
                ...
                SetFlag<HardEvent::S_V>(2);
                WaitFlag<HardEvent::S_V>(2);
                Duplicate(distanceMaskInt16Local[curDataBlockId * 16], (int16_t)0,
            ...
            DataCopyPad(this->extraMaskLocal, this->extraMaskGm[bid * extraMaskLen + currentOffset],
        ...
        {
            Duplicate(distanceMaskInt16Local, (int16_t)0, (int32_t)this->distanceMaskLenAlign / 2);
        }
    }
};
}  // namespace IndexOps
    ...
    GM_ADDR extraMask, GM_ADDR extraMaskAttr, GM_ADDR distanceMask, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    op.Init(queryTimeStamp, queryTokenSet, dbTimeStamp, dbDivisor, dbRemainder, extraMask, extraMaskAttr, distanceMask,
