...
class AscendcDistanceBatchMaskGenerator
{
   public:
    ...
    __aicore__ inline void Init(GM_ADDR queryTimeStamp, GM_ADDR queryTokenSet, GM_ADDR dbTimeStamp, GM_ADDR dbDivisor,
                                GM_ADDR dbRemainder, GM_ADDR distanceMask, GM_ADDR workspace,
                                const AscendcDistanceBatchMaskGeneratorTilingData* tilingData, TPipe* pipe)
    {
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t bid = 0; bid < this->batchSize; ++bid)
        {
            ...
            for (uint32_t loop = 0; loop < this->repeatNum; ++loop)
            {
                ...
                DataCopy(this->distanceMaskGm[bid * this->m + loop * this->distanceMaskLenAlign],
   ...
   private:
    ...
    __aicore__ inline void CompareTimeStamp(uint32_t bid)
    {
        ...
        Cast(this->dbTimeStampFloat32Local, this->dbTimeStampLocal, AscendC::RoundMode::CAST_NONE,
             this->dbTimeStampLenAlign);
        CompareScalar(this->timeStampCmpResTmpLocal1, this->dbTimeStampFloat32Local, (float32_t)(0.0),
                      AscendC::CMPMODE::GE, this->dbTimeStampLenAlign);
        Adds(this->dbTimeStampLocal, this->dbTimeStampLocal, (endTime - startTime), this->dbTimeStampLenAlign);
        Cast(this->dbTimeStampFloat32Local, this->dbTimeStampLocal, AscendC::RoundMode::CAST_NONE,
             this->dbTimeStampLenAlign);
        CompareScalar(this->timeStampCmpResTmpLocal2, this->dbTimeStampFloat32Local, (float32_t)(0.0),
                      AscendC::CMPMODE::LE, this->dbTimeStampLenAlign);
    ...
    __aicore__ inline void CompareTokenId()
    {
        ...
        CompareScalar(this->tokenCmpResLocal, resRemainderFloat16Local, (float16_t)(2.0), AscendC::CMPMODE::GT,
                      this->dbDivisorLenAlign);
...
extern "C" __global__ __aicore__ void ascendc_distance_batch_mask_generator(GM_ADDR queryTimeStamp,
                                                                            GM_ADDR queryTokenSet, GM_ADDR dbTimeStamp,
                                                                            GM_ADDR dbDivisor, GM_ADDR dbRemainder,
                                                                            GM_ADDR distanceMask, GM_ADDR workspace,
                                                                            GM_ADDR tiling)
{
    GET_TILING_DATA(tilingData, tiling);
    TPipe pipe;
    IndexOps::AscendcDistanceBatchMaskGenerator op;
    op.Init(queryTimeStamp, queryTokenSet, dbTimeStamp, dbDivisor, dbRemainder, distanceMask, workspace, &tilingData,
            &pipe);
    op.Process();
}
