...
constexpr uint32_t IVFPQ_DIST_REDUCE_NUM_M32 = 16;
...
__aicore__ inline void SearchGatherV1(__ubuf__ int32_t *topkMergeEndDisIndex, __gm__ int64_t *labelOffsetGm,
                                      __gm__ uint64_t *labelBaseGm, __gm__ uint64_t *topkMergeEndLabel, uint32_t topk)
{
...
class AscendcIvfpqSearchDistanceTopK
{
   ...
   private:
    ...
    __aicore__ inline void SingleBlockTopKAndIndexAlign(LocalTensor<float> dst_local_value,
                                                        ...
                                                        int innerBlockindex);
    __aicore__ inline void DistCompute(LocalTensor<float> queryPQUb, LocalTensor<float> distResultUb,
                                       uint32_t innerBlockindex, uint32_t blockIndex, uint32_t batchIndex);
    ...
    const AscendcIvfpqSearchDistanceTopKTilingData *__restrict tilingData)
{
    ...
    if (this->subSpaceNum >= 32)
    {
        this->dist_compute_reduce_num = IVFPQ_DIST_REDUCE_NUM_M32;
    }
...
__aicore__ inline void AscendcIvfpqSearchDistanceTopK::InitReduceUbTensor()
{
    ...
    // M>=32 uses scalar LUT sum only; skip Gather/Reduce temps to free UB for the 32KB LUT.
    if (this->subSpaceNum < 32)
    {
        ...
        pipe.InitBuffer(dist_gather_offset_tmp_buf,
                        this->dist_compute_reduce_num * this->subSpaceNum * sizeof(int32_t));
    ...
    LocalTensor<uint8_t> tmp_local, int innerBlockindex)
{
    ...
    for (uint32_t index = 0; index < this->single_core_total_block; index++)
    {
        AscendC::Adds(dst_local_index[index * this->topk], dst_local_index[index * this->topk],
                      int32_t(index * this->topk_outter_num + innerBlockindex * this->perCoreInnerBlockDealSize),
                      this->topk);
    }
    AscendC::PipeBarrier<PIPE_V>();
}
    ...
    LocalTensor<int32_t> src_local_index, LocalTensor<uint8_t> tmp_local, int innerBlockindex)
{
    if (innerBlockindex > 0)
    {
        ...
        AscendC::TopK<float, true, false, false, AscendC::TopKMode::TOPK_NORMAL>(
            dist_local_value, dist_local_index, src_local_value, src_local_index, src_local_finish_whole, tmp_local,
            topk, this->topkTilingDataWhole, topKInfoWhole, this->isLargest);
    }
    else
    {
        // 只对比当前轮次
        ...
        AscendC::TopK<float, true, false, false, AscendC::TopKMode::TOPK_NORMAL>(
            dist_local_value, dist_local_index, src_local_value, src_local_index, src_local_finish_whole, tmp_local,
            topk, this->topkTilingDataSingle, topKInfoWhole, this->isLargest);
    }
...
__aicore__ inline void AscendcIvfpqSearchDistanceTopK::DistCompute(LocalTensor<float> queryPQUb,
                                                                   ...
                                                                   uint32_t innerBlockindex, uint32_t blockIndex,
                                                                   uint32_t batchIndex)
{
    ...
    uint32_t curBlockNum =
        static_cast<uint32_t>(this->codeSizeGm.GetValue(batchIndex * this->codeBlockNum + blockIndex));
    ...
    uint64_t curBlockOffset =
        static_cast<uint64_t>(this->codeOffsetGm.GetValue(batchIndex * this->codeBlockNum + blockIndex));

    // batch分批处理
    uint32_t batchNum = curTaskNum / this->dist_compute_reduce_num;
    // M=32 temporary ReduceSum/Gather path overflows UB on current tiling; use scalar LUT sum
    if (this->subSpaceNum >= 32)
    {
        batchNum = 0;
    }
        ...
        for (uint32_t batchidx = 0; batchidx < batchNum; batchidx++)
        {
            ...
            if (this->subSpaceNum >= 8)
            {
                AscendC::ReduceSum<float, AscendC::Pattern::Reduce::AR, true>(
                    curBatchDistResult, dist_gather_result_tmp, dist_code_tmp, reduceShape, true);
                AscendC::PipeBarrier<PIPE_V>();
            }
            else
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    {
                        Add(dist_add_value_tmp[i * 16], dist_add_value_tmp[i * 16],
                            dist_trans_tmp[i * 16 * this->subSpaceNum + j * 16], 16);
                    }
                }
                AscendC::PipeBarrier<PIPE_V>();
    ...
    while (processedNum < curTaskNum)
    {
        ...
        {
            distResultUb(processedNum) = SearchTableSumV1<32>(queryPQ, codeBase, curBlockOffset, taskIdx, this->ksub);
        }
        processedNum++;
    }
    AscendC::PipeBarrier<PIPE_ALL>();
...
__aicore__ inline void AscendcIvfpqSearchDistanceTopK::Process()
{
    ...
    LocalTensor<int32_t> dist_add_const_tmp;
    if (this->subSpaceNum < 32)
    {
        dist_add_const_tmp = dist_add_const_tmp_buf.Get<int32_t>();
    ...
    for (int batchIndex = 0; batchIndex < batch; batchIndex++)
    {
        ...
        for (int blockIndex = 0; blockIndex < perCoreBlockNum; blockIndex++)
        {
            ...
            perCoreInnerBlockLoopNum =
                (codeBlockSizeTmp + this->perCoreInnerBlockDealSize - 1) / this->perCoreInnerBlockDealSize;
...
__aicore__ inline void AscendcIvfpqSearchDistanceTopK::ProcessMerge()
{
    ...
    for (int batchIndex = blockIdx; batchIndex < batch; batchIndex += usedAivNum)
    {
            ...
            if (codeBlockSizeTmp != 0)
            {
                ...
                for (int indexId = 0; indexId < mergeBeginTopNumInLoop; indexId++)
                {
                    AscendC::Adds(top_k_merge_src_index_deque[indexId * this->topk],
                                  ...
                                  static_cast<int32_t>(IVFPQ_CODE_BLOCK_SIZE * (i * mergeBeginTopNumInLoop + indexId)),
                                  this->topk);
                }
                AscendC::PipeBarrier<PIPE_V>();
                // 首轮、尾轮实际的内轴长度不一致
        ...
        SearchGatherV1(top_k_merge_end_dst_index_buf_local_addr, labelOffsetGmAddr, labelBaseGmAddr,
                       topkLabelFinalGmAddr, topk);
        AscendC::PipeBarrier<PIPE_V>();
        // 最终归并结果拷贝到GM
