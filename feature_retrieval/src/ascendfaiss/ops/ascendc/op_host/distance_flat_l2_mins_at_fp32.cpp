...
ge::graphStatus DoLibApiTiling(DistanceFlatL2MinsAtFP32TilingData &tiling, uint64_t l1_size, uint64_t l0c_size)
{
    ...
    gemm_qb_tiling.SetOrgShape(tiling.get_formerTileLength(), tiling.get_baseCodesNumLength(), tiling.get_dimLength());
...
{
    // 计算每个tile的长度
    return (ub_size - 2048 - 8192 - baseCodesNumLength * FLOAT32_BYTES) /
           (dimLength * FLOAT32_BYTES + baseCodesNumLength * FLOAT32_BYTES * 2 + REDUCE_SIZE * FLOAT32_BYTES +
            baseCodesNumLength * FLOAT32_BYTES / 64 * 2);
}
...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    size_t usrSize =
        static_cast<size_t>(aivec_num) * static_cast<size_t>(bufferSize) * 2 * static_cast<size_t>(FLOAT32_BYTES);
