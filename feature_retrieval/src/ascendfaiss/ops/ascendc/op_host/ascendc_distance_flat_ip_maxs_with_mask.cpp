...
#include "ascendc_distance_flat_ip_maxs_with_mask_tiling.h"

namespace optiling {
using namespace matmul_tiling;

static ge::graphStatus TilingSetInputShapeInfo(gert::TilingContext* context,
    AscendcDistanceFlatIPMaxsWithMaskTilingData &tiling)
{
    if (context == nullptr) {
...
static ge::graphStatus TilingSetCubeTiling(gert::TilingContext* context,
    AscendcDistanceFlatIPMaxsWithMaskTilingData &tiling)
{
...
namespace ops {
class AscendcDistanceFlatIPMaxsWithMask : public OpDef {
public:
    explicit AscendcDistanceFlatIPMaxsWithMask(const char* name) : OpDef(name)
    {
        this->Input("queries")
            .ParamType(REQUIRED)
...
OP_ADD(AscendcDistanceFlatIPMaxsWithMask);
}
