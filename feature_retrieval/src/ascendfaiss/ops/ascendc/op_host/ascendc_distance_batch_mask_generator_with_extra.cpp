...
const uint32_t EXTRA_MASK_INPUT_DIM = 5;
const uint32_t EXTRA_MASK_ATTR_INPUT_DIM = 6;
...
const uint32_t EXTRA_MASK_LEN_DIM = 1;
const uint32_t EXTRA_MASK_ATTR_SIZE_DIM = 0;
...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    const auto extraMaskPtr = context->GetInputShape(EXTRA_MASK_INPUT_DIM);
    const auto extraMaskAttrPtr = context->GetInputShape(EXTRA_MASK_ATTR_INPUT_DIM);

    if ((queryTimeStampPtr == nullptr) || (dbTimeStampPtr == nullptr) || (queryTokenSetPtr == nullptr))
    {
        return ge::GRAPH_FAILED;
    }
    if ((extraMaskPtr == nullptr) || (extraMaskAttrPtr == nullptr))
    {
        return ge::GRAPH_FAILED;
    }
    ...
    auto extraMaskShape = extraMaskPtr->GetStorageShape();
    auto extraMaskAttrShape = extraMaskAttrPtr->GetStorageShape();
    ...
    uint32_t extraMaskLen;
    uint32_t extraMaskAttrLen;
    ...
    if (dbLen == 0)
    {
        ERROR_LOG("total_db_num must be greater than 0.");
        return ge::GRAPH_FAILED;
    }
    ...
    {
        ERROR_LOG("extra_mask.shape[1] must be less than or equal to %u.", MAX_K);
        return ge::GRAPH_FAILED;
    }
    extraMaskAttrLen = extraMaskAttrShape.GetDim(EXTRA_MASK_ATTR_SIZE_DIM);
    if (extraMaskAttrLen != LEN_OF_EIGHT)
    {
        ERROR_LOG("extra_mask_attr.shape[0] must be %u.", LEN_OF_EIGHT);
        return ge::GRAPH_FAILED;
    }
