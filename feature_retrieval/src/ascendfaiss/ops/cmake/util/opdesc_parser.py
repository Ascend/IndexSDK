...
class OpInfo:
    def __init__(self: any, file: str, batch_list: list, iterator_list: list, builder: any,
                 op_type: list, auto_gen_dir: str = None):
        self.file = file
        self.batch_list = batch_list
        self.iterator_list = iterator_list
        self.builder = builder
        self.op_type = op_type
        self.auto_gen_dir = auto_gen_dir
...
def _trans_soc_ver_to_short(soc_ver: str):
    ...
    if low_soc_ver not in SOC_TO_SHORT_SOC_MAP:
        logging.info("WARNING: caution: %s will trans into ascend910, \
            if not your intention, use ascend910b1~4 instead", soc_ver)
...
def _get_op_custom_options(op_descs: list, auto_gen_dir: str):
    ...
    if not os.path.exists(file):
        logging.info("WARNING: cannot find %s/custom_compile_options.ini", auto_gen_dir)
        return
    with open(file, 'r') as fd:
        ...
        for line in lines:
            ...
            if op_type.upper() == 'ALL':
                op_type = OP_ALL
            if op_type != OP_ALL and _is_op_type_in_opdesc(op_descs, op_type) is False:
                logging.info("WARNING: op: %s are not exists in this project", op_type)
                continue
...
def get_op_desc(op_info: OpInfo) -> list:
    op_descs = []
    op_match = False
    with open(op_info.file, 'r') as fd:
        lines = fd.readlines()
        for line in lines:
            line = line.strip()
            if line.startswith('['):
                name = line[1:-1]
                if op_info.op_type is None or name in op_info.op_type:
                    op_match = True
                    op_desc = op_info.builder(name)
                    op_desc.parse_replay_val(op_info.batch_list, op_info.iterator_list)
                    op_descs.append(op_desc)
                else:
                    op_match = False
                    if op_info.op_type is not None and len(op_descs) == len(op_info.op_type):
                        return op_descs
                continue
            if not op_match:
                continue
    ...
    _get_op_custom_options(op_descs, op_info.auto_gen_dir)
    return op_descs
