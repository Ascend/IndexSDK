...
print(
    "[%.3f] Init AscendIndexIVFPQ, dim = %d, sub_quantizers = %d, metric_tpye = %s, byResidual = %s, devices = %s"
    % (time.time() - start_time, dim, m, args.metric_name, str(args.by_residual), str(devices))
)

ascend_index_ivfpq = ascendfaiss.AscendIndexIVFPQ(dim, ascendfaiss.METRIC_L2, nlist, m, 8, config)
...
for i in [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]:
    ascendfaiss.AscendParameterSpace().set_index_parameter(ascend_index_ivfpq, 'nprobe', i)
    t, r = datasets.evaluate(ascend_index_ivfpq, query, ground_truth, k)
    print(
        "@%3d qps: %.3f, r@1: %.4f, r@10: %.4f, r@100: %.4f, r@1000: %.4f"
