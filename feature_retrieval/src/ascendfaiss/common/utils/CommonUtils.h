...
template <typename T>
APP_ERROR AddCodeNDFormat(const AscendTensor<T, DIMS_2> &rawData, size_t ntotal, size_t blockSize,
                          std::vector<std::unique_ptr<DeviceVector<T>>> &baseShaped)
{
    ...
    for (size_t i = 0; i < n;)
    {
        ...
        auto ret = aclrtMemcpy(dst, leftInBlock * dims * sizeof(T), src, copyCount * dims * sizeof(T),
                               ACL_MEMCPY_HOST_TO_DEVICE);
        APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "Mem operator h2d error %d", ret);
...
template <typename T>
APP_ERROR GetVectorsNDFormat(uint32_t offset, uint32_t num, uint32_t blockSize,
                             const std::vector<std::unique_ptr<DeviceVector<T>>> &baseShaped, std::vector<T> &vectors)
{
    if (num == 0)
    {
        return APP_ERR_OK;
    ...
    for (uint32_t i = 0; i < num;)
    {
        ...
        auto ret = aclrtMemcpy(dst, leftInData * dims * sizeof(T), src, copyCount * dims * sizeof(T),
                               ACL_MEMCPY_DEVICE_TO_HOST);
        APPERR_RETURN_IF_NOT_FMT(ret == ACL_SUCCESS, APP_ERR_INNER_ERROR, "Mem operator d2h error %d", ret);
...
{
    return aclrtMemcpy(
        baseShaped[param.dstBlockIdx]->data() + param.dstBlockOffset * static_cast<size_t>(dim),
        ...
        static_cast<size_t>(dim) * sizeof(T), ACL_MEMCPY_DEVICE_TO_DEVICE);
}
