...
AscendIndexIVFFlatImpl::AscendIndexIVFFlatImpl(AscendIndexIVFFlat *intf, int dims, int nlist, faiss::MetricType metric,
                                               AscendIndexIVFFlatConfig config)
...
void AscendIndexIVFFlatImpl::copyFromCentroids(const faiss::IndexIVFFlat *index)
{
    ...
    for (size_t i = 0; i < indexConfig.deviceList.size(); i++)
    {
        ...
        auto ret = aclrtMemcpy(pIndex->centroidsOnDevice->data(), centroidsSize * sizeof(float), centroids,
...
void AscendIndexIVFFlatImpl::copyFromIVF(const faiss::IndexIVFFlat *index)
{
    ...
    for (int listId = 0; listId < nlist; listId++)
    {
        ...
        for (size_t i = 0; i < listSize; i++)
        {
            ...
            assignCounts.at(listId).Add(const_cast<float *>(vectors + i * dim), ids ? (ids + i) : nullptr);
...
void AscendIndexIVFFlatImpl::indexIVFFlatGetListCodes(int deviceId, int nlist, InvertedLists *ivf) const
{
    ...
    for (int listId = 0; listId < nlist; listId++)
    {
        ...
        auto ret = aclrtMemcpy(ids.data(), ids.size() * sizeof(::ascend::Index::idx_t), idsVec.data(),
                               idsVec.size() * sizeof(faiss::idx_t), ACL_MEMCPY_DEVICE_TO_HOST);
...
void AscendIndexIVFFlatImpl::addPaged(int n, const float *x, const idx_t *ids)
{
    ...
    if (totalSize > addPageSize || static_cast<size_t>(n) > ADD_VEC_SIZE * 10)
    {
        ...
        for (size_t i = 0; i < static_cast<size_t>(n); i += tileSize)
        {
            ...
            {
                APP_LOG_INFO("AscendIndexIVFFlatImpl::add: adding %zu:%zu / %d\n", i, i + curNum, n);
            }
    ...
    else
    {
        if (this->intf_->verbose)
        {
            APP_LOG_INFO("AscendIndexIVFFlatImpl::add: adding 0:%d / %d\n", n, n);
        }
        addImpl(n, x, ids);
    }
...
size_t AscendIndexIVFFlatImpl::getAddElementSize() const { return static_cast<size_t>(intf_->d) * sizeof(float); }
}  // namespace ascend
}  // namespace faiss
