...
AscendIndexImpl::AscendIndexImpl(int dims, faiss::MetricType metric, AscendIndexConfig config, AscendIndex *intf)
...
#pragma omp parallel for reduction(+ : removeCnt) num_threads(deviceCnt)
    ...
    FAISS_THROW_IF_NOT_FMT(this->intf_->ntotal >= removeCnt, "removeCnt should be in range [0, %ld], actual=%d.",
                           this->intf_->ntotal, removeCnt);
...
void AscendIndexImpl::searchPaged(int n, const float *x, int k, float *distances, idx_t *labels) const
{
    ...
    if (totalSize > SEARCH_PAGE_SIZE || static_cast<size_t>(n) > SEARCH_VEC_SIZE || totalOutSize > SEARCH_PAGE_SIZE)
    {
        ...
        for (size_t i = 0; i < static_cast<size_t>(n); i += tileSize)
        {
            ...
            searchImpl(curNum, x + i * static_cast<size_t>(this->intf_->d), k, distances + i * static_cast<size_t>(k),
    ...
    if (n > 0)
    {
        ...
        if (totalSize > SEARCH_PAGE_SIZE || (size_t)n > SEARCH_VEC_SIZE || totalOutSize > SEARCH_PAGE_SIZE)
        {
            ...
            for (size_t i = 0; i < (size_t)n; i += tileSize)
            {
                ...
                searchImpl(curNum, x + i * (size_t)this->intf_->d, k, distances + i * (size_t)k,
                           labels + i * (size_t)k);
            }
        }
        else
...
void AscendIndexImpl::getBase(int deviceId, char *xb) const
{
    ...
    FAISS_THROW_IF_NOT_FMT(indexes.find(deviceId) != indexes.end(), "deviceId is out of range, deviceId=%d.", deviceId);
...
void AscendIndexImpl::reset()
{
    ...
    for (size_t i = 0; i < deviceNum; ++i)
    {
        ...
        FAISS_THROW_IF_NOT_FMT(ret == ::ascend::APP_ERR_OK, "Failed to reset index result: %d, device id: %d\n", ret,
                               deviceId);
    }
    idxDeviceMap.clear();
                                        ...
                                        float *distances, idx_t *labels) const
{
    ...
    auto isInvalidLabel = [](ascend_idx_t labelValue)
    { return static_cast<idx_t>(labelValue) == -1 || labelValue == std::numeric_limits<ascend_idx_t>::max(); };
...
#pragma omp parallel for if (n > 100) num_threads(::ascend::CommonUtils::GetThreadMaxNums())
    for (idx_t i = 0; i < n; i++)
    {
        ...
        {
            size_t id = devices;
            float disMerged = std::numeric_limits<float>::max();
            ascend_idx_t labelMerged = static_cast<ascend_idx_t>(-1);
            for (size_t j = 0; j < devices; j++)
            {
                while (posit[j] < k && isInvalidLabel(label[j][offset + posit[j]]))
                {
                    posit[j]++;
                }
                if (posit[j] >= k)
                ...
                if (id == devices || compFunc(dist[j][pos], disMerged))
                {
            ...
            if (id == devices)
            {
                *(distances + offset + num) = std::numeric_limits<float>::max();
                *(labels + offset + num) = -1;
                num++;
                continue;
            }
                                        ...
                                        float *distances, idx_t *labels) const
{
    ...
    for (size_t i = 0; i < deviceCnt; i++)
    {
        transform(begin(label[i]), end(label[i]), begin(transLabel[i]),
                  [&](ascend_idx_t temp)
                  {
...
void AscendIndexImpl::checkParameters(int dims, faiss::MetricType metric, AscendIndexConfig, AscendIndex *intf) const
{
    VALUE_UNUSED(metric);
    ...
    FAISS_THROW_IF_NOT_FMT(dims > 0 && dims <= MAX_DIM, "Invalid number of dimensions(%d), should be (0, %d]", dims,
                           MAX_DIM);
    FAISS_THROW_IF_NOT_FMT(dims % DIM_DIVISOR == 0, "Invalid number of dimensions(%d), should be divisible by %d", dims,
                           DIM_DIVISOR);
    ...
    FAISS_THROW_IF_NOT_FMT(
        indexConfig.resourceSize == -1 || (indexConfig.resourceSize >= 0 && indexConfig.resourceSize <= INDEX_MAX_MEM),
...
void AscendIndexImpl::indexSearch(IndexParam<uint16_t, uint16_t, ascend_idx_t> &param) const
{
    ...
    auto ret = aclrtMemcpy(tensorDevQueries.data(), tensorDevQueries.getSizeInBytes(), param.query,
                           param.n * param.dim * sizeof(uint16_t), ACL_MEMCPY_HOST_TO_DEVICE);
    ...
    FAISS_THROW_IF_NOT_FMT(ret == APP_ERR_OK, "Failed to search index,deviceId is %d, result is: %d\n", param.deviceId,
                           ret);
}
