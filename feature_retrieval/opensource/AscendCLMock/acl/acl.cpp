...
extern "C"
{
    ...
    {
        // Align size up to a multiple of 32 bytes (no extra +32 like aclrtMalloc).
        size_t alignedSize = ((size + 31U) / 32U) * 32U;
        return aclrtMalloc(devPtr, alignedSize, policy);
    }
    ...
    aclError aclrtGetMemInfo(aclrtMemAttr UNUSED(attr), size_t *free, size_t *total)
    {
        if (free == nullptr || total == nullptr)
        {
            return ACL_ERROR_FAILURE;
        }
        // Stub HBM/DDR capacity for UT (no real device memory).
        constexpr size_t kMockTotalBytes = 32ULL * 1024ULL * 1024ULL * 1024ULL;  // 32 GiB
        *total = kMockTotalBytes;
        *free = kMockTotalBytes;
        return ACL_SUCCESS;
    }
    ...
    const char *aclrtGetSocName() { return ENV().getSocName(); }

    aclError aclopSetModelDir(const char *UNUSED(modelDir)) { return ACL_SUCCESS; }
    ...
    size_t aclGetTensorDescNumDims(const aclTensorDesc *desc) { return desc->numDims; }

    static uint8_t getBytesByTensorType(aclDataType type)
    {
        static std::map<aclDataType, uint8_t> typeMapBytes = {
            {ACL_INT8, sizeof(int8_t)},     {ACL_INT16, sizeof(int16_t)},   {ACL_INT32, sizeof(int32_t)},
            {ACL_INT64, sizeof(int64_t)},   {ACL_UINT8, sizeof(uint8_t)},   {ACL_UINT16, sizeof(uint16_t)},
            {ACL_UINT32, sizeof(uint32_t)}, {ACL_UINT64, sizeof(uint64_t)}, {ACL_FLOAT16, sizeof(int16_t)},
            {ACL_FLOAT, sizeof(float)},     {ACL_DOUBLE, sizeof(double)},   {ACL_BF16, sizeof(unsigned short)},
                                 ...
                                 int numOutputs, aclDataBuffer *const outputs[], aclrtStream stream)
    {
        ...
        {
            aclAppLog(ACL_ERROR, __FUNCTION__, __FILE__, __LINE__, "op:%s not register pls register first",
                      handle->opName);
            return ACL_ERROR_UNINITIALIZE;
        }
    ...
    aclError aclrtProcessReport(int32_t timeout) { return 0; }
    ...
    aclDataType aclGetTensorDescType(const aclTensorDesc *desc) { return (aclDataType)0; }

    aclError aclopCreateKernel(const char *opType, const char *kernelId, const char *kernelName, void *binData,
    ...
    aclError aclopRegisterCompileFunc(const char *opType, aclopCompileFunc func) { return 0; }

    aclError aclopUpdateParams(const char *opType, int numInputs, const aclTensorDesc *const inputDesc[],
