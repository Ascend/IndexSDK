...
thread_local std::array<aclrtContext, MAX_DEVICE> AscendSimuDevice::m_activeContexts{};
...
AscendSimuDevice::~AscendSimuDevice() { ClearAll(); }
...
size_t AscendSimuDevice::GetRefCnt() const { return m_refCnt; }
...
aclrtContext AscendSimuDevice::GetActiveContext() { return m_activeContexts[m_deviceId]; }

void AscendSimuDevice::SetActiveContext(aclrtContext ctxt) { m_activeContexts[m_deviceId] = ctxt; }
...
void AscendSimuDevice::DestoryStream(aclrtStream stream)
{
    ...
    stream->ctxt->streams.erase(std::find_if(stream->ctxt->streams.begin(), stream->ctxt->streams.end(),
                                             [stream](aclrtStream _stream) { return stream == _stream; }));
}

void AscendSimuDevice::WaitStreamExecFlowIdle(aclrtStream stream) { m_flowMap[stream]->waitForEmpty(); }
