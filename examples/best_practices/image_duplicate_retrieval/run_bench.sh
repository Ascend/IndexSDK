...
cd "${PROJECT_DIR}"
mkdir -p "${RESULT_DIR}"
BENCHMARK="${PROJECT_DIR}/build/image_dedup_benchmark"
for variant in flat int8flat ivfflat_single ivfflat_dual ivfrabitq; do
    "${BENCHMARK}" "${variant}"
done
for variant in flat int8flat ivfflat_single ivfflat_dual ivfrabitq; do
    "${BENCHMARK}" "${variant}" --hbm
done
python3 "${PROJECT_DIR}/python/verify_bench.py"
