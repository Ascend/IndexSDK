...
IndexGreat集成了我司自研的昇腾侧和鲲鹏侧向量检索算法,为用户提供鲲鹏+昇腾高维大底库近似检索能力。
...
2. 所使用的python环境需为`Python 3.9.11`或以上，并安装:
    * `torch`: 2.0.1；
...
1. `索引创建`阶段，底库大小`nb`需要满足`10000 <= nb`，不建议`nb > 1e8`；
...
#### 注意：仅针对`AKMode`下创建的`Vstar`索引，在`AInitParams`内传入的`verbose`的值会覆盖掉`IndexGreatInitParams`内的`verbose`值。（`AKMode`下的`KMode`仍遵循`IndexGreatInitParams`设置的`verbose`值）
...
* `convPQM`: PQ量化向量分段数。默认值：128。限制范围：1）`16 <= convPQM <= dim`； 2）`convPQM % 8 == 0`；3） `dim % convPQM == 0`。
...
1. 训练码本的脚本可见`HSP_Training_final.py`。
...
* `mask`: 一个填充uint8数据的`vector`容器。对于每个uint8数据，将其转化为二进制表达形式，再将该形式转化为小端序（代表1的bit在最左边）后, 对于为0的bit，在检索过程中无视拥有与该bit位位置对应的id的底库向量。
