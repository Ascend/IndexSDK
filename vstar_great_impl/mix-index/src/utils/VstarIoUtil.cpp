...
void initializeFileDescription(int &fd, const std::string &fname)
{
    ...
    char *resolvedPathPtr = realpath(realPath.c_str(), resolvedPath);
    if (lstat(realPath.c_str(), &st) == 0) { // 文件路径已经存在
        ASCEND_THROW_IF_NOT_MSG(resolvedPathPtr != nullptr, "Expect resolvedPath to be valid since file exists.\n");
        fd = open(resolvedPath, O_NOFOLLOW | O_WRONLY);
        ...
        ASCEND_THROW_IF_NOT_MSG(resolvedPathPtr == nullptr, "Expect resolvedPath to be NULL since file not exists.\n");
        // 以期待创建一个新文件的方式打开文件，并设置640权限（如果任何父路径为软连接，不会打开或创建）
        fd = open(resolvedPath, O_NOFOLLOW | O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP);
        ASCEND_THROW_IF_NOT_MSG(fd >= 0, // 文件未打开成功，无需关闭
            "fname or one of its parent directory is a softlink, or error encoutered during opening fname.\n");
...
{
    struct stat stTmp;
    std::string::size_type lastPos = realPath.rfind('/');
...
VstarIOWriter::VstarIOWriter(const std::string &fname) : name(fname)
{
    initializeFileDescription(fd, name);
}

VstarIOWriter::~VstarIOWriter()
{
    closeFileDescription(fd);
}
