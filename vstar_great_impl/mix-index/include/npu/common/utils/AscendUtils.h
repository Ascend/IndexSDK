...
#define CALL_PARALLEL_FUNCTOR(pcounter, pmin, forCounter, threadPool, functor) do { \
    if (pcounter >= pmin) {                                                \
...
#define CALL_PARALLEL_FUNCTOR_NOEXCEPTION(pcounter, pmin, forCounter, threadPool, functor) do { \
    if (pcounter >= pmin) {                                                            \
        std::vector<std::future<void> > functorRets;                                   \
        for (int i = 0; i < forCounter; i++) {                                         \
            functorRets.emplace_back(threadPool->Enqueue(functor, i));                 \
...
#define WAITING_FLAG_READY(flag, checkTicks, timeout) do { \
    int waitTicks_ = 0;                                                                            \
...
#define ACL_REQUIRE_OK(x) do { \
    auto err = (x);                                                           \
    ASCEND_THROW_IF_NOT_FMT(err == ACL_ERROR_NONE, "ACL error %d", (int)err); \
} while (0)

// Wrapper to test return status of ACL functions and return
#define ACL_REQUIRE_OK_RET_CODE(x, errcode) do { \
    auto err = (x);                                                                         \
    ASCEND_EXC_IF_NOT_FMT(err == ACL_ERROR_NONE, return errcode, "ACL error %d", (int)err); \
} while (0)
