...
#define ASCEND_THROW_MSG(MSG) do { \
    throw ::ascendSearchacc::AscendException(MSG, __PRETTY_FUNCTION__, __FILE__, __LINE__); \
} while (false)

#define ASCEND_THROW_FMT(FMT, ...) do { \
    ...
    throw ::ascendSearchacc::AscendException(__s, __PRETTY_FUNCTION__, __FILE__, __LINE__); \
} while (false)

// Exceptions thrown upon a conditional failure
#define ASCEND_THROW_IF_NOT(X) do { \
    if (!(X)) {                                                                           \
...
#define ASCEND_THROW_IF_NOT_CODE(X, CODE) do { \
    if (!(X)) {                                                                           \
        CODE;                                                                             \
        aclAppLog(ACL_ERROR, __FUNCTION__, __FILE__, __LINE__, "Error: '%s' failed", #X); \
        ASCEND_THROW_FMT("Error: '%s' failed", #X);                                       \
...
#define ASCEND_THROW_IF_NOT_MSG(X, MSG) do { \
    if (!(X)) {                                                                           \
        aclAppLog(ACL_ERROR, __FUNCTION__, __FILE__, __LINE__, "Error: '%s' failed", #X); \
...
#define ASCEND_THROW_IF_NOT_FMT(X, FMT, ...) do { \
    if (!(X)) {                                                                                        \
...
#define ASCEND_EXC_IF_NOT(X, CODE) do { \
    if (!(X)) {                \
        CODE;                  \
    }                          \
} while (false)

#define ASCEND_EXC_IF_NOT_LOG(X, CODE, MSG) do { \
    if (!(X)) {                                                      \
...
#define ASCEND_EXC_IF_NOT_FMT(X, CODE, FMT, ...) do { \
    if (!(X)) {                                                                                            \
