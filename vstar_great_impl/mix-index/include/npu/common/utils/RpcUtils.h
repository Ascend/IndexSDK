...
#define RPC_RETURN_IF_NOT_OK(X) do { \
    auto ret = X;                                  \
    if (ret) {                                     \
...
#define RPC_RETURN_IF_NOT(X, ERRCODE) do { \
    if (!(X)) {                                        \
...
#define RPC_RETURN_IF_NOT_LOG(X, ERRCODE, MSG) do { \
    if (!(X)) {                                                      \
...
#define RPC_RETURN_IF_NOT_FMT(X, ERRCODE, FMT, ...) do { \
    if (!(X)) {                                                                                            \
...
#define RPC_RETURN_FMT(ERRCODE, FMT, ...) do { \
    aclAppLog(ACL_ERROR, __FUNCTION__, __FILE__, __LINE__, "Err failed: " FMT, __VA_ARGS__); \
    resp.mutable_result()->set_errorcode(ERRCODE);                                           \
    return 0;                                                                                \
} while (false)

#define HDC_RETURN_IF_NOT(X) do { \
    if (!(X)) {                     \
        return HDC_RPC_ERROR_ERROR; \
    }                               \
} while (false)

#define HDC_RETURN_IF_NOT_LOG(X, MSG) do { \
    if (!(X)) {                                                      \
        aclAppLog(ACL_ERROR, __FUNCTION__, __FILE__, __LINE__, MSG); \
        return HDC_RPC_ERROR_ERROR;                                  \
    }                                                                \
...
#define HDC_RETURN_IF_NOT_FMT(X, FMT, ...) do { \
    if (!(X)) {                                                                                            \
        aclAppLog(ACL_ERROR, __FUNCTION__, __FILE__, __LINE__, "Err: '%s' failed: " FMT, #X, __VA_ARGS__); \
        return HDC_RPC_ERROR_ERROR;                                                                        \
    }                                                                                                      \
