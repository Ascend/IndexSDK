...
public:
                                ...
                                uint32_t blockDim)
    {
        ...
        pipe.InitBuffer(outQueueCO1,
                        bufferNumL0B, cubeAlign * ((sizeCodeWordL0BBuffer / subDim2 / sizeof(half))) * sizeof(half));
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int32_t queryIdx = 0; queryIdx < n; queryIdx ++) {
            ...
            for (int32_t loop = 0; loop < loopCount; loop++) {
                ...
                // 全局桶id (nlist1 * nlist2个)除以nlist2（每个L1桶内的L2桶个数），得到所在粗桶id
                int32_t bucketIdx = l2IndicesLocal.GetValue(l2IndicesIdx) / nlist2;
...
//              判断是否是此query在该核上应处理的最后一个子桶
                if (loop == loopCount - 1 || curL2IndicesIdxAccuSegmentNum >= segmentNum ||
                    ...
                    if (preL2IndicesIdxAccuSegmentNum >= segmentNum) break;
                    endFlag = 1;
                ...
                if (codeWordEndOffset == codeWordBeginOffset) continue;
//              基于前面的计算得到了真正需要算的endOffset和beginOffset以及粗桶id bucketIdx
                ...
                if (tailSize==0) tailSize = sizeCodeWordUBBuffer;
...
private:
    ...
    __aicore__ inline void CopyInB(const int32_t curComputeSize)
    {
        ...
        DataCopy(b1Local, codeWordFp16Local, curComputeSize / sizeof(half));
    ...
    __aicore__ inline void ComputeAndCopyOut(const uint64_t dstOffset, const uint64_t vmDstOffset,
                                             const int32_t curCopyNum)
    {
        LocalTensor<half> computeLocal = outQueueCompute.DeQue<half>();
        LocalTensor<half> vcMinLocal = outQueueVcMin.AllocTensor<half>();
        ...
        if (curCopyVcMinSize != curCopyVcMinSizeAlign) {
            Duplicate(vcMinLocal, zero, curCopyVcMinSizeAlign / sizeof(half));
        }
        pipe_barrier(PIPE_ALL);

        WholeReduceMin<half>(vcMinLocal, computeLocal, segSizeVcMin, curCopyNum / segSizeVcMin, 1, 1,
                             segSizeVcMin * sizeof(half) / 32); // WholeRediceMin需要32bit对齐
        pipe_barrier(PIPE_ALL);
// 原子加
        for (int i = 0; i < curCopyVcMinSizeAlign / sizeof(half); i++) {
            float vcMinTemp = vcMinLocal.GetValue(i);
        ...
        SetAtomicAdd<half>();
        DataCopy(vcMinGM[vmDstOffset * 2 / segSizeVcMin], vcMinLocal, curCopyVcMinSizeAlign / sizeof(half));
        SetAtomicNone();
        pipe_barrier(PIPE_ALL);

        DataCopy(outDistsGM[dstOffset], computeLocal, curCopyNum);

        outQueueVcMin.FreeTensor(vcMinLocal);
        outQueueCompute.FreeTensor(computeLocal);
    }
...
extern "C" __global__ __aicore__ void vsc3(GM_ADDR queries, GM_ADDR shaped, GM_ADDR l2Indices, GM_ADDR diff1,
                                           GM_ADDR diff2, GM_ADDR precompute, GM_ADDR attr_nlistl1,
                                           GM_ADDR attr_nlistl2, GM_ADDR attr_segmentl3, GM_ADDR dist, GM_ADDR opflag,
                                           GM_ADDR vcmin, GM_ADDR workspace, GM_ADDR tiling)
{
    GET_TILING_DATA(tiling_data, tiling);
    KernelVSC3 op;
    op.Init(queries, shaped, l2Indices,  diff1, diff2, precompute, dist, opflag, vcmin,
            tiling_data.subDim1,
            tiling_data.subDim2,
            tiling_data.nlist1,
            tiling_data.nlist2,
...
#else
extern "C" __global__ __aicore__ void vsc3(GM_ADDR queryCode,
                                                          GM_ADDR codeWord,
                                                          GM_ADDR l2Indices,
                                                          GM_ADDR diff1, GM_ADDR diff2, GM_ADDR precompute,
                                                          GM_ADDR attr_nlistl1, GM_ADDR attr_nlistl2,
                                                          GM_ADDR attr_segmentl3,
                                                          GM_ADDR outDists, GM_ADDR opFlag, GM_ADDR vcMin)
{
    KernelVSC3 op;
    op.Init(queryCode, codeWord, l2Indices, diff1, diff2, precompute, outDists, opFlag, vcMin,
            32,
            16,
            1024,
            32,
