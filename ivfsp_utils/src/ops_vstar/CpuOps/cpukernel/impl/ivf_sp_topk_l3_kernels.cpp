...
#include "ivf_sp_topk_l3_kernels.h"
...
namespace {
    const char *IVF_SP_TOPK_L3 = "IvfSpTopkL3";
    const uint32_t THREAD_CNT = 6;
}

namespace aicpu {
    uint32_t IvfSpTopkL3CpuKernel::Compute(CpuKernelContext &ctx)
    {
        Inputs inputs;
        Outputs outputs;
    ...
    uint32_t IvfSpTopkL3CpuKernel::GetInOutAndCheck(const CpuKernelContext &ctx, Inputs &inputs, Outputs &outputs) const
    {
        KERNEL_LOG_INFO("IvfSpTopkL13puKernel GetInOutAndCheck begin");
    ...
    uint32_t IvfSpTopkL3CpuKernel::CheckInputShapes(const Inputs &inputs)
    {
        ...
        KERNEL_CHECK_TRUE(shapeVmdists->GetDims() == INPUT_NUM2, KERNEL_STATUS_PARAM_INVALID,
                          "Dims of input[0][vmdists] must be 2");
        KERNEL_CHECK_TRUE(shapeIdaddress->GetDims() == INPUT_NUM2, KERNEL_STATUS_PARAM_INVALID,
                          "Dims of input[0][idaddress] must be 2");
    ...
    void IvfSpTopkL3CpuKernel::UpdateOutputsShape(Outputs &outputs)
    {
    ...
    void IvfSpTopkL3CpuKernel::DoCompute(size_t start, size_t end, const Inputs &inputs, Outputs &outputs, C &&cmp)
    {
        ...
        for (size_t j = start; j < end; j++) {
            ComputeBlock<T, C>(j, indists, vmdists, idaddress, outdists, outlabels, reorder, cmp);
        }
    }

    template<typename T>
    void IvfSpTopkL3CpuKernel::InitTopkHeap(Outputs &outputs) const
    {
    ...
    template <typename T, typename C>
    void IvfSpTopkL3CpuKernel::ComputeBlock(size_t n, KernelTensor<float16_t> &indistsTensor,
                                            ...
                                            KernelTensor<T> &outlabelsTensor, bool reorder, C &&cmp)
    {
        ...
        float16_t *vmdists = vmdistsTensor.GetSubTensorDim0(n);
        uint64_t *idaddress = idaddressTensor.GetSubTensorDim0(n);
        float16_t *outdists = outdistsTensor.GetSubTensorDim0(n);
        T *outlabel = outlabelsTensor.GetSubTensorDim0(n);

        auto ret = 0;

        std::vector<int64_t> realIds(l3SegNum_ * l3SegSize_, 3);
        int curTotal = 0;
        for (int i = 0; i < nprobeL2_; i++) {
            ...
            size_t cpySize = std::min(int(offset), int(l3SegNum_ * l3SegSize_ - curTotal));
            ret = memcpy_s(realIds.data() + curTotal, sizeof(int64_t) * cpySize, idStart, sizeof(int64_t) * cpySize);
            if (ret != 0) {
            ...
            if (curProbeSegNum * l3SegSize_ - offset > 0) {
                ...
                ret = memcpy_s(indists + (curTotal + offset),
            ...
            curTotal += curProbeSegNum * l3SegSize_;

            if (curTotal >= l3SegNum_ * l3SegSize_) {
                break;
            }
        }

        if (curTotal < l3SegNum_ * l3SegSize_) {
            std::vector<float16_t> fillDist(l3SegNum_ * l3SegSize_ - curTotal, 99.99);
            ret = memcpy_s(indists + curTotal, sizeof(float16_t) * (l3SegNum_ * l3SegSize_ - curTotal), fillDist.data(),
                           sizeof(float16_t) * (l3SegNum_ * l3SegSize_ - curTotal));
            if (ret != 0) {
                KERNEL_LOG_ERROR("Copy data shaped error %d", ret);
                return;
            }
            ...
            for (int64_t i = burstIdx; i < burstSize; ++i) {
                ...
                for (int64_t j = 0; j < burstLen_ && idx < ntotal; ++j, ++idx) {
                    if (cmp(outdists[0], indists[idx])) {
                        ...
                        outlabel[0] = static_cast<T>(realIds[baseOffset + idx]);
    ...
    REGISTER_CPU_KERNEL(IVF_SP_TOPK_L3, IvfSpTopkL3CpuKernel);
} // namespace aicpu