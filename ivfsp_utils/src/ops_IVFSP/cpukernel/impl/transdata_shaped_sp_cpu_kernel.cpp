...
#include "transdata_shaped_sp_cpu_kernel.h"
...
namespace {
const char* TRANSDATA_SHAPED = "TransdataShapedSp";
}

namespace aicpu {
uint32_t TransdataShapedSpCpuKernel::Compute(CpuKernelContext &ctx)
{
    Inputs inputs;
    Outputs outputs;
...
uint32_t TransdataShapedSpCpuKernel::GetInOutAndCheck(const CpuKernelContext &ctx,
    Inputs &inputs, Outputs &outputs) const
{
    KERNEL_LOG_INFO("TransdataShapedSpCpuKernel GetInOutAndCheck begin");
...
uint32_t TransdataShapedSpCpuKernel::CheckInOutShapes(const Inputs &inputs, const Outputs &outputs)
{
    KERNEL_LOG_INFO("TransdataShapedSpCpuKernel CheckInputShapes begin");
    ...
    KERNEL_CHECK_TRUE(attrCount == TRANSDATA_SHAPED_SP_ATTR_IDX_COUNT,
        KERNEL_STATUS_PARAM_INVALID, "Num of attrs must be %d", TRANSDATA_SHAPED_SP_ATTR_IDX_COUNT);

    auto attr = static_cast<int64_t *>(inputs.attrs->GetData());
    ntotal_ = *(attr + TRANSDATA_SHAPED_SP_ATTR_NTOTAL_IDX);

    return KERNEL_STATUS_OK;
}

template<typename T>
void TransdataShapedSpCpuKernel::DoCompute(size_t start, size_t end, const Inputs &inputs, Outputs &outputs)
{
...
REGISTER_CPU_KERNEL(TRANSDATA_SHAPED, TransdataShapedSpCpuKernel);
} // namespace aicpu
