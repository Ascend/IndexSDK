...
#include "topk_ivf_sp_cpu_kernel.h"
...
namespace {
const char* TOPK_IVF = "TopkIvfSp";
const uint32_t THREAD_CNT = 6;
}

namespace aicpu {
uint32_t TopkIvfSpCpuKernel::Compute(CpuKernelContext &ctx)
{
    Inputs inputs;
    Outputs outputs;
...
uint32_t TopkIvfSpCpuKernel::GetInOutAndCheck(const CpuKernelContext &ctx, Inputs &inputs, Outputs &outputs) const
{
    KERNEL_LOG_INFO("TopkIvfSpCpuKernel GetInOutAndCheck begin");
...
uint32_t TopkIvfSpCpuKernel::CheckInputShapes(const Inputs &inputs)
{
    KERNEL_LOG_INFO("TopkIvfSpCpuKernel CheckInputShapes begin");
    ...
    KERNEL_CHECK_TRUE(shapeOpflag->GetDims() == INPUT_NUM4,
    ...
    KERNEL_CHECK_TRUE(attrCount == TOPK_IVF_SP_ATTR_IDX_COUNT,
        KERNEL_STATUS_PARAM_INVALID, "Num of attrs must be %d", TOPK_IVF_SP_ATTR_IDX_COUNT);

    auto attr = static_cast<int64_t *>(inputs.attr->GetData());
    asc_ = *(attr + TOPK_IVF_SP_ATTR_ASC_IDX);
    k_ = *(attr + TOPK_IVF_SP_ATTR_K_IDX);
    burstLen_ = *(attr + TOPK_IVF_SP_ATTR_BURST_LEN_IDX);
    blockNum_ = *(attr + TOPK_IVF_SP_ATTR_BLOCK_NUM_IDX);
    flagNum_ = *(attr + TOPK_IVF_SP_ATTR_FLAG_NUM_IDX);
...
void TopkIvfSpCpuKernel::UpdateInOutShape(Inputs &inputs, Outputs &outputs) const
{
    KERNEL_LOG_INFO("TopkIvfSpCpuKernel UpdateInOutShape begin");
...
void TopkIvfSpCpuKernel::InitTopkHeap(Outputs &outputs) const
{
...
template <typename C>
void TopkIvfSpCpuKernel::DoCompute(size_t tcnt, size_t tid, const Inputs &inputs, Outputs &outputs, C &&cmp)
{
    ...
    for (int64_t qidx = tid; qidx < nq_; qidx += tcnt) {
        ...
        for (int64_t bidx = 0; bidx < blockNum_; ++bidx) {
            ...
            for (int64_t hidx = 0; hidx < handleBatch_; ++hidx) {
                ...
                ComputeQuery(qidx, bidx, hidx, indists, vmdists, ids, size, outdists, outlabels, reorder, cmp);
            }
        }
        if (!reorder) {
            float16_t *outdists1 = outdists.GetSubTensorDim0(qidx);
            int64_t *outlabel1 = outlabels.GetSubTensorDim0(qidx);
...
template <typename C>
void TopkIvfSpCpuKernel::ComputeQuery(int64_t qidx,
    int64_t bidx,
    int64_t hidx,
    ...
    C &&cmp)
{
    float16_t *indists = indistsTensor.GetSubTensorDim2(qidx, bidx, hidx);
    float16_t *vmdists = vmdistsTensor.GetSubTensorDim2(qidx, bidx, hidx);
...
REGISTER_CPU_KERNEL(TOPK_IVF, TopkIvfSpCpuKernel);
} // namespace aicpu