...
#include "topk_ivfsp_l1_cpu_kernel.h"
...
namespace {
const char *TOPK_IVFSP_L1 = "TopkIvfSpL1";
const uint32_t THREAD_CNT = 6;
}

namespace aicpu {
uint32_t TopkIvfSpL1CpuKernel::Compute(CpuKernelContext &ctx)
{
    Inputs inputs;
    Outputs outputs;
...
uint32_t TopkIvfSpL1CpuKernel::GetInOutAndCheck(const CpuKernelContext &ctx, Inputs &inputs, Outputs &outputs) const
{
...
uint32_t TopkIvfSpL1CpuKernel::CheckInputShapes(const Inputs &inputs)
{
    ...
    KERNEL_CHECK_TRUE(attrCount == TOPK_IVFSP_L1_ATTR_IDX_COUNT, KERNEL_STATUS_PARAM_INVALID, "Num of attrs must be %d",
...
void TopkIvfSpL1CpuKernel::UpdateOutputsShape(Outputs &outputs)
{
...
void TopkIvfSpL1CpuKernel::DoCompute(size_t start, size_t end, const Inputs &inputs, Outputs &outputs, C &&cmp)
{
...
template<typename T>
void TopkIvfSpL1CpuKernel::InitTopkHeap(Outputs &outputs) const
{
...
template <typename T, typename C>
void TopkIvfSpL1CpuKernel::ComputeBlock(size_t n, int64_t blockIdx, KernelTensor<float16_t> &indistsTensor,
...
template <typename T, typename C>
void TopkIvfSpL1CpuKernel::UpdateHeap(float16_t *dists, T *label, int64_t len,
    float16_t pushDist, int64_t index, C &&cmp)
{
...
REGISTER_CPU_KERNEL(TOPK_IVFSP_L1, TopkIvfSpL1CpuKernel);
} // namespace aicpu