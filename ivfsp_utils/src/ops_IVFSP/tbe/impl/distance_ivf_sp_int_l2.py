...
class DistanceIVFSpIntL2Mins:
    def __init__(self,
        ...
        self.nprobe_align = (self.nprobe + 7)//8*8
    ...
    def set_tik_instance(self):
        ...
        self.aicore_use = min(self.nprobe, 8)
    ...
    def cal_num_each_core(self):
        ...
        with self.tik_instance.new_stmt_scope():
            ...
            self.tik_instance.data_move(base_offset_ub, self.input_base_offset_gm, 0, 1, self.nprobe_align//4, 0, 0)
            ...
            self.tik_instance.data_move(actual_num_ub, self.input_actual_code_num_gm, 0, 1, self.nprobe_align//8, 0, 0)
    
            
            for i in range(self.nprobe):
                self.actual_num[i].set_as(actual_num_ub[i])
           
            codebook_offset = self.tik_instance.Tensor("uint64", (self.nprobe_align,),
                                                            name="codebook_offset_ub",
                                                            scope=tik.scope_ubuf)
            self.tik_instance.data_move(codebook_offset, self.input_codebook_offset_gm, 0, 1, 
                                            self.nprobe_align//4, 0, 0)
    ...
    def cal_xd_v2(self, queries_ub, queries_l1, queries_l2_ub, add_scalar_list, data_xd_l1, move_num, queries_align):
    ...
    def cal_xd_lowdim_v2(self, queries_ub, queries_l1, queries_l2_ub, add_scalar, data_xd_l1, move_num, queries_align):
        ...
        with self.tik_instance.new_stmt_scope():
            ...
            # move diff adn min from out to UB
            self.tik_instance.data_move(diff_ub, self.input_diff_gm, 0, 1, self.dim2 // 16, 0, 0)
            self.tik_instance.data_move(min_ub, self.input_min_gm, 0, 1, self.dim2 // 16, 0, 0)
            diff2_ub = self.tik_instance.Tensor("float16",
                                                (self.dim2,),
                                                name="diff2_ub", scope=tik.scope_ubuf)
            # calculate 0.5 * d
    ...
    def distance_compute_each_loop_v2(self, list_idx):
        ...
        with self.tik_instance.if_scope(centroids_loop_time > 0):
            with self.tik_instance.for_range(0, centroids_loop_time) as loop_centroids:
                self.cube_compute_each_loop(list_idx, data_xd_l1, add_scalar, self.base_offset[list_idx],
    ...
    def compute_and_min_v2(self):
        ...
        with self.tik_instance.for_range(0, self.aicore_use, block_num=self.aicore_use) as block_index:
            for list_idx in range(self.aicore_use):
                with self.tik_instance.if_scope(list_idx == block_index):
                    self.distance_compute_each_loop_v2(list_idx)
    ...
    def get_tik_instance_v2(self):
        """
        obtain tik instance
        """
        self.compute_and_min_v2()

        self.tik_instance.BuildCCE(
            kernel_name=self.kernel_name,
            inputs=[
                self.input_queries_gm, self.input_codebook_gm, self.input_code_gm,
...
def distance_ivf_sp_int_l2(input_queries,
                                input_codebook,
                                input_code,
                                input_precomputed,
                                input_diff,
                                ...
                                output_distances,
                                output_flag,
                                kernel_name="distance_ivf_sp_int_l2"):
    """
    ...
    None
    """
    distance_sq8 = DistanceIVFSpIntL2Mins(input_queries, input_codebook, input_code,
                                          ...
                                          output_distances, output_flag,
                                          kernel_name)
    tik_instance = distance_sq8.get_tik_instance_v3()
    return tik_instance