...
def check_custom_op_opinfo(required_custom_op_info_keys, ops, op_key):
    ...
    if len(missing_keys) > 0:
        ...
        logging.error(f"op: {op_key} opInfo missing: {missing_keys_join}")
...
def check_op_input_output(info, key, ops):
    ...
    for op_sets in ops[key]:
        if op_sets not in ('format', 'type', 'name'):
            logging.error(f"{info} should has format type or \
                            name as the key, but getting {op_sets}")
    ...
    for op_key in aicpu_ops:
        ...
        for key in ops:
            if key == "opInfo":
                ...
                logging.error("Only opInfo, input[0-9], output[0-9] can be used as a "
                              f"key, but op {op_key} has the key {key}")
                raise KeyError("bad key value")
