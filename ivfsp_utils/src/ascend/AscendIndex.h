...
#ifndef ASCENDSEARCH_ASCEND_ASCENDINDEX_H
#define ASCENDSEARCH_ASCEND_ASCENDINDEX_H
...
const int64_t INDEX_MAX_MEM_INT8 = 0x80000000; // 0x80000000 mean 2048MB
...
struct AscendIndexConfig {
    ...
    inline explicit AscendIndexConfig(std::initializer_list<int> devices, int64_t resources = INDEX_DEFAULT_MEM,
        uint32_t blockSize = DEFAULT_BLOCK_SIZE)
        : deviceList(devices), resourceSize(resources), dBlockSize(blockSize) {}

    inline explicit AscendIndexConfig(std::vector<int> devices, int64_t resources = INDEX_DEFAULT_MEM,
        uint32_t blockSize = DEFAULT_BLOCK_SIZE)
        : deviceList(devices), resourceSize(resources), dBlockSize(blockSize) {}
