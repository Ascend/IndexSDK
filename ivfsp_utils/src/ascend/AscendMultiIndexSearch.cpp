    ...
    float *distances, idx_t *labels, bool merged)
{
    ...
    FAISS_THROW_IF_NOT_MSG(x, "Invalid x: x cannot be nullptr.");
    FAISS_THROW_IF_NOT_MSG(distances, "Invalid distances: distances cannot be nullptr.");
    FAISS_THROW_IF_NOT_MSG(labels, "Invalid labels: labels cannot be nullptr.");

    for (size_t i = 0; i < indexes.size(); ++i) {
        FAISS_THROW_IF_NOT_FMT(indexes[i], "Invalid index %zu from given indexes: nullptr.", i);
        FAISS_THROW_IF_NOT_FMT(indexes[i]->is_trained, "Index %zu not trained", i);
        if (i > 0) {
    ...
    for (size_t i = 0; i < indexes.size(); ++i) {
        ...
        for (const auto device : deviceList) {
            rpcContext ctx = indexes[i]->impl_->contextMap.at(device);
            ctxMap[device] = ctx;
            deviceIndexIdsMap[device].push_back(indexes[i]->impl_->indexMap[ctx]);
            hostIndexIdMap[device].push_back(i);
        }
    }

    std::vector<int> deviceIdsVec;
    for (auto it = deviceIndexIdsMap.begin(); it != deviceIndexIdsMap.end(); ++it) {
        deviceIdsVec.push_back(it->first);
    }
    ...
    auto searchFunctor = [&](int idx) {
        ...
        transform(begin(distHalf[idx]), end(distHalf[idx]), begin(dist[idx]),
            [](uint16_t temp) { return static_cast<float>(fp16(temp)); });
