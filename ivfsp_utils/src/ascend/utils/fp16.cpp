...
const uint16_t HALF_MAN_MASK = 0x03FF;     // mantissa mask of fp16(        11111 11111)
const uint16_t HALF_MAN_HIDE_BIT = 0x0400; // hide bit of mantissa of fp16(   1 00000 00000)
#define HALF_SIGN_VALUE(x) ((x).Bits.sign)
#define HALF_EXP_VALUE(x) ((x).Bits.exp)
#define HALF_MAN_VALUE(x) ((x).Bits.man | static_cast<uint16_t>(((x).Bits.exp > 0 ? 1 : 0) * HALF_MAN_HIDE_BIT))
#define HALF_IS_ZERO(x) (((x) & 0x7FFF) == 0)
#define HALF_IS_NAN(x) (((x).Bits.exp == 0x1f) && ((x).Bits.man > 0))
#define HALF_IS_INF(x) (((x).Bits.exp == 0x1f) && !((x).Bits.man > 0))
...
#define FLOAT_MAN_VALUE(x) ((x).Bits.man | static_cast<uint32_t>(((x).Bits.exp > 0 ? 1u : 0u) * FLOAT_MAN_HIDE_BIT))
#define FLOAT_IS_NAN(x) (((x).Bits.exp == 0xff) && ((x).Bits.man > 0))
#define FLOAT_IS_INF(x) (((x).Bits.exp == 0xff) && !((x).Bits.man > 0))
...
static uint16_t FloatToFp16(const float &val)
{
    ...
    if (exp >= 0x8Fu) { // 0x8Fu:142=127+15
        ...
        if (exp >= 0x67) { // 0x67:103=127-24 Denormal
            ...
            uint64_t tmp = (static_cast<uint64_t>(fMan)) << (exp - 0x67);

            bool needRound = IsRoundOne(tmp, SHIFT_LEN);
            retMan = static_cast<uint16_t>(tmp >> SHIFT_LEN);
            if (needRound) {
                retMan++;
            }
        ...
        retExp = static_cast<int16_t>(exp - 0x70u);

        bool needRound = IsRoundOne(man, shift);
        retMan = static_cast<uint16_t>(man >> shift);
        if (needRound) {
            retMan++;
        }

        if ((retMan & HALF_MAN_HIDE_BIT) != 0) {
            retExp++;
        }
    }
...
float Fp16ToFloat(const uint16_t &val)
{
    ...
    while ((man > 0) && ((man & HALF_MAN_HIDE_BIT) == 0)) {
        man <<= 1;
        exp--;
    }
...
fp16::fp16(const int16_t &val) : data(static_cast<uint16_t>(val)) {}

fp16::fp16(const fp16 &fp) : data(fp.data) {}

fp16::fp16(const int32_t &val) : data(static_cast<uint16_t>(val)) {}

fp16::fp16(const uint32_t &val) : data(static_cast<uint16_t>(val)) {}
