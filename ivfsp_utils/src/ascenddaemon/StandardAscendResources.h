...
public:
    ...
    static std::shared_ptr<StandardAscendResources> getInstance()
    {
        ...
        int deviceIdTmp = 0;
        auto ret = aclrtGetDevice(&deviceIdTmp);
        if (ret != ACL_SUCCESS) {
        ...
        if (resources.find(deviceIdTmp) == resources.end()) {
            auto res = std::make_shared<StandardAscendResources>();
            res->SetDevice(deviceIdTmp);
            resources[deviceIdTmp] = res;
        }
        return resources[deviceIdTmp];
    }
    ...
    void SetDevice(int deviceIdTmp);
