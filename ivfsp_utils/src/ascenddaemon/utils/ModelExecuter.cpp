...
size_t ModelExecuter::getInputNumDims(int index) const
{
    aclmdlIODims dims;
...
size_t ModelExecuter::getOutputNumDims(int index) const
{
    aclmdlIODims dims;
...
int64_t ModelExecuter::getInputDim(size_t index, size_t dimIndex) const
{
    aclmdlIODims dims;
    auto ret = aclmdlGetInputDims(modelDesc, index, &dims);
    ASCEND_THROW_IF_NOT_FMT(ret == ACL_ERROR_NONE, "get input dims failed, index is %zu", index);
    ASCEND_THROW_IF_NOT_FMT(dimIndex < dims.dimCount, "get input dim failed, the dimCount=%zu", dims.dimCount);

    return dims.dims[dimIndex];
}

int64_t ModelExecuter::getOutputDim(size_t index, size_t dimIndex) const
{
    aclmdlIODims dims;
    aclError ret = aclmdlGetOutputDims(modelDesc, index, &dims);
    ASCEND_THROW_IF_NOT_FMT(ret == ACL_ERROR_NONE, "get output dims failed, index is %zu", index);
    ASCEND_THROW_IF_NOT_FMT(dimIndex < dims.dimCount, "get output dim failed, the dimCount=%zu", dims.dimCount);
...
void ModelExecuter::releaseResource(aclmdlDataset *input, aclmdlDataset *output,
                                    aclDataBuffer *inputDb, aclDataBuffer *outputDb) const
{
    if (inputDb != nullptr) {
