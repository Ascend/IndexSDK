...
inline void CheckInputSize(size_t inputSize, std::ifstream &fin)
{
    ...
    ASCEND_THROW_IF_NOT_MSG(static_cast<size_t>(bytesLeft) >= inputSize,
...
/**
 * @brief 对输入路径的父目录做1）存在校验；2）软连接校验；
 *
 * @param realPath 全局路径
 * @param fd 文件句柄
...
size_t WriteBatchImpl(const int &fd, const T *ptr, size_t typeSize, size_t expectedWrite)
{
    ...
    ASCEND_THROW_IF_NOT_FMT(CSTYLE_IO_LIMIT % typeSize == 0,
                            "CSTYLE_IO_LIMIT[%lu] should be a multiple of size[%lu].", CSTYLE_IO_LIMIT, typeSize);
    size_t writeCounts = expectedWrite / CSTYLE_IO_LIMIT; // 写次数
    const size_t writeItemsMax = CSTYLE_IO_LIMIT / typeSize;
    ...
    for (size_t i = 0; i < writeCounts; ++i) {
        ...
        if (bytesWritePerCount == -1) { // 文件句柄打开失败，无必要继续迭代，直接返回
            return bytesWrite / typeSize;
        }
        bytesWrite += bytesWritePerCount;
    }
    ...
    bytesWrite += remainBytesWrite; // bytesWrite不可能小于-1，因此bytesWrite / size一定 >= 0, 返回值size_t不会翻转
    return bytesWrite / typeSize;
}
...
size_t ReadBatchImpl(const int &fd, T *ptr, size_t typeSize, size_t expectedRead)
{
    ...
    const size_t readItemsMax = CSTYLE_IO_LIMIT / typeSize; // 指针偏移量（CSTYLE_IO_LIMIT % 64 = 0，因此readItemsMax一定为整数)
    ssize_t bytesRead = 0;
    for (size_t i = 0; i < readCounts; ++i) {
        ssize_t bytesReadPerCount = read(fd, ptr + i * readItemsMax, CSTYLE_IO_LIMIT);
        bytesRead += bytesReadPerCount;
        if (bytesReadPerCount == -1) { // 文件句柄打开失败，无必要继续迭代，直接返回
            return bytesRead / typeSize;
        }
    }
    ...
    bytesRead += remainBytesRead; // bytesRead不可能小于-1，因此bytesRead / size一定 >= 0, 返回值size_t不会翻转
    return bytesRead / typeSize;
}
 ...
 */
struct FSPIOWriter {
    explicit FSPIOWriter(const std::string &fname);
...
struct FSPIOReader {
    explicit FSPIOReader(const std::string &fname);
    ...
    void ReadAndCheck(T *dataPtr, size_t dataSize) const
    {
        ...
        ASCEND_THROW_IF_NOT_FMT(bytesRead == dataSize,
                                "Read index failed; Expect to write %lu bytes data, actually write %lu bytes data.\n",
                                dataSize, bytesRead);
    }
