...
{
    return (b == 0) ? std::numeric_limits<U>::max() :
        (static_cast<decltype(a + b)>(a) / static_cast<decltype(a + b)>(b));
}

template<typename U, typename V>
constexpr auto divUp(U a, V b) -> decltype(a + b)
{
    return (b == 0) ? std::numeric_limits<U>::max() : ((static_cast<decltype(a + b)>(a) +
        static_cast<decltype(a + b)>(b) - 1) / static_cast<decltype(a + b)>(b));
}
...
{
    const int power = 2;
    return pow(power, static_cast<T>(n));
}
...
{
    return (isPowerOf2(v) ? static_cast<T>(2) * v : (static_cast<T>(1) << (log2(v) + 1))); // 2 means scale
}
